...
<head>
  <title>欢迎使用IT服务管理系统</title>
    <%@include file="/includefiles.jsp"%>
  	<script type="text/javascript" src="${r"${pageContext.request.contextPath}"}/${model.filePath}.js"></script> 
	...
	Ext.onReady(function(){	
	   ...
	   var citId="";
	   if("${r"${param.configItemTypeId}"}"!=""){
	   		citId="${r"${param.configItemTypeId}"}";
	   }
	   if("${r"${param.dataId}"}"==""){
	     var pageTemplates = new PageTemplates({dataId:"0",configItemTypeId:citId});
	   }
	   else{
	   var pageTemplates = new PageTemplates({dataId:"${r"${param.dataId}"}",configItemTypeId:citId});
	   }
