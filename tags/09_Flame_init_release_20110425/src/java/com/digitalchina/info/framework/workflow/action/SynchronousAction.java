		...
		if(counterSignAuditMegs.size()!=0&&counterSignAuditMegs!=null&&!"".equals(counterSignAuditMegs)){
			...
			while(counterMegs.hasNext()){
				...
				if(configUnitMail!=null&&!"".equals(configUnitMail)){//相应节点配置了相应邮件内容
					...
					String url = "<a href=" + PropertiesUtil.getProperties("system.web.url","localhost:8080") + "/infoAdmin/workflow/configPage/auditFromMail.jsp?"+"taskId="+taskId+"&dataId="+dataId+"&reqClass="+"&goStartState="+goStartState+"&taskName="+"&applyType="+applyType+"&browseFlag="+false+">"+"审批链接</a>";
		...
		if(counterSignAuditMegs.size()!=0&&counterSignAuditMegs!=null&&!"".equals(counterSignAuditMegs)){
			...
			while(counterMegs.hasNext()){
				...
				if(configUnitMail!=null&&!"".equals(configUnitMail)){//相应节点配置了相应邮件内容
					...
					if(subject==null||"".equals(subject)){
						subject="IT温馨提示:"+userMeg+"提交了"+reqFlag+vDesc+"，请您及时审批。";//"审批通知";
					}
				}
				for(int i=0;i<users.length;i++){
					UserInfo userInfo = (UserInfo)service.findUnique(UserInfo.class, "userName", users[i]);
