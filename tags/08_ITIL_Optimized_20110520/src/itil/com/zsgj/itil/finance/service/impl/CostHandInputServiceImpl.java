...
public class CostHandInputServiceImpl implements CostHandInputService{
	...
	public boolean saveFinanceCostSchedules(JSONObject jo)  {
		...
		FinanceCostType fct = (FinanceCostType)costHandInputDao.findObjectByProperty("com.zsgj.itil.finance.entity.FinanceCostType", id , null, null);
		...
		if(!"".equals(tempReimbursement)&&tempReimbursement!=null){
			...
			reimbursement = (UserInfo)costHandInputDao.findObjectByProperty("com.zsgj.info.framework.security.entity.UserInfo", userId , null, null);
		}
		...
		FinanceCostCenter costCenter = (FinanceCostCenter)costHandInputDao.findObjectByProperty("com.zsgj.itil.finance.entity.FinanceCostCenter", costCenterId , null, null);
		...
		costAuditUser = (UserInfo)costHandInputDao.findObjectByProperty("com.zsgj.info.framework.security.entity.UserInfo", auditUserId , null, null);
	...
	public String findList(Map<String, String> map, int start,
		...
		for(int i=0; i< list.size(); i++){
			...
			if("配置项".equals(costReduceType)){
				ConfigItem ci = (ConfigItem)costHandInputDao.findObjectByProperty("com.zsgj.itil.config.entity.ConfigItem", null, "cisn", costItem);
				costItem = ci.getName();
			}else{
				ServiceItem si = (ServiceItem)costHandInputDao.findObjectByProperty("com.zsgj.itil.service.entity.ServiceItem", null, "serviceItemCode", costItem);
				costItem = si.getName();
			}
