/*
 * Ext JS Library 2.0.1 Copyright(c) 2006-2007, Ext JS, LLC. licensing@extjs.com
 * 
...
Ext.DomHelper = function() {
	...
	var A = function(T) {
		...
		P += "<" + T.tag;
		for (var O in T) {
			if (O == "tag" || O == "children" || O == "cn" || O == "html"
					|| typeof T[O] == "function") {
				continue
			}
			if (O == "style") {
...
Ext.form.Field = Ext.extend(Ext.BoxComponent, {
	...
	onRender : function(C, A) {
		...
		if (this.readOnly) {
			...
			this.el.addClass("x-form-field-readyOnly")
		}
...
Ext.form.TriggerTipField = Ext.extend(Ext.form.TextField, {
	defaultAutoCreate : {
		tag : "input",
		type : "text",
		size : "16",
	...
	onRender : function(B, A) {
		...
		this.triggerConfig = {
			...
			cn : [{
				...
				tag : "img",
				src : Ext.BLANK_IMAGE_URL,
				cls : "x-form-tip-trigger " + this.trigger2Class
			}]
		}
		this.trigger = this.wrap.createChild(this.triggerConfig || {
			tag : "img",
	...
	initTrigger : function() {
		...
		this.trigger.on('mouseover', this.onTargetOver, this);
		this.trigger.addClassOnOver("x-form-trigger-over");
		this.trigger.addClassOnClick("x-form-trigger-click")
	},
	onDestroy : function() {
	...
	onTargetOver : function(e){
        new Ext.ToolTip({
        target: this.trigger,
//        title: '<font color=red>温馨提示</font>',
        anchor: 'left',
        autoHide: true,
        closable: true,
//        autoHeight:true,
...
//带提示图标的文本框
Ext.form.TipField = Ext.extend(Ext.form.TextField, {
	defaultAutoCreate : {
		tag : "input",
		type : "text",
		size : "16",
	...
	onRender : function(B, A) {
		...
		this.trigger = this.wrap.createChild(this.triggerConfig || {
			tag : "img",
			src : Ext.BLANK_IMAGE_URL,
			cls : "x-form-tip-trigger " + this.triggerClass
		});
		if (this.hideTrigger) {
			this.trigger.setDisplayed(false)
		}
...
Ext.reg("tipField", Ext.form.TipField);
...
//带提示信息的文本域
Ext.form.TriggerArea = Ext.extend(Ext.form.TextField, {
	defaultAutoCreate : {
		tag : "input",
		type : "text",
		size : "16",
...
//带提示用的组合框
Ext.form.TipComboBox = Ext.extend(Ext.form.TriggerTipField, {
	defaultAutoCreate : {
		tag : "input",
		type : "text",
		size : "24",
	...
	onRender : function(B, A) {
		Ext.form.TipComboBox.superclass.onRender.call(this, B, A);
		if (this.hiddenName) {
	...
	initEvents : function() {
		Ext.form.TipComboBox.superclass.initEvents.call(this);
	...
	onDestroy : function() {
		...
		this.bindStore(null);
		Ext.form.TipComboBox.superclass.onDestroy.call(this)
	},
	...
	onResize : function(A, B) {
		Ext.form.TipComboBox.superclass.onResize.apply(this, arguments);
	...
	onEnable : function() {
		Ext.form.TipComboBox.superclass.onEnable.apply(this, arguments);
		if (this.hiddenField) {
			this.hiddenField.disabled = false
		}
	},
	onDisable : function() {
		Ext.form.TipComboBox.superclass.onDisable.apply(this, arguments);
		if (this.hiddenField) {
	...
	getValue : function() {
		if (this.valueField) {
			...
			return Ext.form.TipComboBox.superclass.getValue.call(this)
		}
	},
	clearValue : function() {
		if (this.hiddenField) {
	...
	setValue : function(A) {
		...
		Ext.form.TipComboBox.superclass.setValue.call(this, C);
		this.value = A
	},
	onTargetOver : function(e){
        new Ext.ToolTip({
...
//        anchorOffset: 85, // center the anchor on the tooltip
	...
	restrictHeight : function() {
		...
		this.list.setHeight(this.innerList.getHeight() + C
...
Ext.reg("tipcombo", Ext.form.TipComboBox);
...
Ext.extend(Ext.grid.GridView, Ext.util.Observable, {
	...
	handleHdOver : function(D, A) {
		...
		if (C && !this.headersDisabled) {
			...
			if (this.cm.isSortable(this.activeHdIndex)
					&& !this.cm.isFixed(this.activeHdIndex)) {
	...
	onEditorKey : function(F, E) {
		...
		if (C == E.TAB) {
			...
			if (C == E.ENTER) {
				...
				if (A) {
					G = D
							.walkCells(B.row - 1, B.col, -1, this.acceptsNav,
									this)
				} else {
