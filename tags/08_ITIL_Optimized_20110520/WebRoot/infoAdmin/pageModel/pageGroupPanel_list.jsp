...
function loadNewTables(){ 
    var serverPath = "${pageContext.request.contextPath}/pageModel/pageGroupPanelManage.do?methodCall=toPageGroupPanelEditForm";
...
<TABLE cellSpacing=1 cellPadding=3 width="100%" bgColor=#6298e1 border=0>
  ...
  <FORM name="formSearch" action="${pageContext.request.contextPath}/pageModel/pageGroupPanelManage.do" method="post">
...
<TABLE cellSpacing=1 cellPadding=0 width="100%" border=0>
 ...
 <TR>
 <TD height=30>系统后台管理&nbsp;-&gt;&nbsp;
  ${not empty requestScope.module? requestScope.module.name : '全部'}页面面板</TD>
 </TR></TBODY></TABLE>


<TABLE cellSpacing=1 cellPadding=3 width="100%" bgColor=#6298e1 border=0>
  <FORM name="formDel" action="${pageContext.request.contextPath}/pageModel/pageGroupPanelManage.do" method="post">
  ...
  <TR>
    ...
    <STRONG><FONT color=#ffffff>标题名称</FONT></STRONG>
    </TD>
     <TD noWrap  bgColor=#8db5e9>
    <FONT color=#ffffff><strong>关键字</strong></FONT></TD>
   
    <TD noWrap bgColor=#8db5e9>
    <FONT color=#ffffff><strong>默认布局面板</strong></FONT>
    </TD>
    <TD noWrap bgColor=#8db5e9>
 ...
 <c:forEach  var="item" items="${requestScope.page.data}" varStatus="status">   
  ...
  <TR onmouseover="this.style.backgroundColor = '#FFFFFF'" style="CURSOR: hand" 
    ...
    <A onclick='changeAdminFlag("修改导航栏目")' 
      href="${pageContext.request.contextPath}/pageModel/pageGroupPanelManage.do?methodCall=toPageGroupPanelEditForm&id=${item.id}">
      ${item.title }
    </A> 
    </TD>

    <TD noWrap>${item.name}</TD>
    <TD noWrap>${item.xtype.cnName}</TD>
    <TD noWrap>
