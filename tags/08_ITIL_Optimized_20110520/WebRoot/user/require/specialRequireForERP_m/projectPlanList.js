		...
		if (curppId != '0') {
			...
			for(var i=0;i<data.length;i++){
				if(data[i].id=='SRProjectPlan$parentPlanCombo'){
					...
					url:webContext+'/extjs/comboDataAction?clazz=com.zsgj.itil.config.extlist.entity.SRProjectPlan',
					fields:['id','planName'],
...
//				alert(newId);
		...
		var refreshButton=new Ext.Button({
			...
			scope : this,
			handler:function(){
				window.location = webContext + '/user/require/specialRequireForERP/makeProjectPlan.jsp?dataId='+this.dataId+'&tabNumber='+'projectPlanListPanel';
				}
			});
