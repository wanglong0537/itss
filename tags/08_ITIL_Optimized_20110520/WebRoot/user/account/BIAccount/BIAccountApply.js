PageTemplates = Ext.extend(Ext.Panel, {
	...
	getFormpanel_PersonAccountApply_Input : function() {
		...
		this.formpanel_PersonAccountApply_Input = new Ext.form.FormPanel({
			...
			{
				html : '<font color=red>*</font> 注册服务权限描述：为了能准确反馈您需要的权限，准备了一个权限注册描述事例，请按照该方式填写：<br><font style="font-family:楷体">((例如：我在北京平台系统科技公司XXX事业部，希望查看XXX事业部销售、欠款等数据))</font>',
				cls : 'common-text',
				colspan : 2,
				rowspan : 0,
			 ...
			 {
				html : ' 如果需要申请的权限与现有的某一个BI帐号权限一致请填写该帐号:<br> (例如：我需要申请与现有BI帐号XXX一致的权限）',
				cls : 'common-text',
				colspan : 2,
				rowspan : 0,
				style : 'border:1px dotted #b0acac;width:600;text-align:left;margin:0px 0px 0px 75px'
...
//            title : '申请帐号信息',
		    ...
		    animCollapse:true,
			collapsible :false,
		    style : 'border:0px dotted #b0acac;margin:0px 0px 0px 75px',
			autoHeight : true,
		    defaults : {
				bodyStyle : 'padding:4px'
			},
			...
			items:[
			{
				html : '<font color="red">*</font>是否涉及薪酬:',
				cls : 'common-text',
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
							 ...
							 if(responseArray.success){	
							 	Ext.MessageBox.alert("提示","申请人已存在审批中的BI帐号申请,不能够再提该申请！",function(btn){
				   ...
				   Ext.Ajax.request({
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
									.decode(response.responseText);
						    if(responseArray.success){
						    	Ext.MessageBox.alert("提示","申请人已存在可使用的BI帐号,请确认后在提申请！",function(btn){
		                      	window.history.back(-1);
		                      	});
		                      }else{
