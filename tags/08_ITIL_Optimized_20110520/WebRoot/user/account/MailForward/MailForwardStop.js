PageTemplates = Ext.extend(Ext.Panel, {
	...
	getTabpanel : function(tab, tabTitle) {
		this.tabPanel = new Ext.TabPanel({
			...
			items : tab,
			tbar : [new Ext.Toolbar.TextItem('<font color=red>提示：</font><font color=blue>1.页面中带红色<font color=red>*</font>号的必填项，请在填写完整后再提交申请！2.<font color=red>资费说明：</font>转发费用一个月5元，操作之后一小时生效！</font>')]
	...
	getFormpanel_MailForwardApply_Input : function() {
		...
		this.formpanel_MailForwardApply_Input = new Ext.form.FormPanel({
			...
			items:[
			{
				html : '<font color=red>*</font>停止转发帐号名称:',
				cls : 'common-text',
				style : 'width:135;text-align:right'
			}, new Ext.form.ComboBox({forceSelection:true,
				xtype : 'combo',
				...
				{
				html : '<font color=red>*</font>邮件停发申请原因:',
				cls : 'common-text',
				style : 'width:135;text-align:right'
			}, new Ext.form.TextField({
				fieldLabel : '申请原因',
			...
			buttons : [{
				...
				id:'save',
				handler : function() {
					if(!Ext.getCmp('panel_MailForwardApply_Input').form.isValid()){
					...
					var accountName=Ext.getCmp('MailForwardApply$accountNameCombo').getRawValue();
					Ext.Ajax.request({
						url : webContext
								+ '/accountAction_saveMailForwardStopDraft.action',
						params : {
							info : info,
				...
				if(!Ext.getCmp('panel_MailForwardApply_Input').form.isValid()){
					...
					Ext.Ajax.request({
						...
						failure : function(response, options) {
							Ext.MessageBox.alert("提示", "提交申请失败,请检查停止转发帐号是否选择正确!");
							 Ext.getCmp("save").enable();
					                Ext.getCmp("submit").enable();
					                Ext.getCmp("back").enable();
						}
