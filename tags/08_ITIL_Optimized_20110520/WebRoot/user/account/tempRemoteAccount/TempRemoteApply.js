		...
		this.formpanel_SpecailAccountApply_Input = new Ext.form.FormPanel({
			...
			/*{
				...
				listeners : {
					...
					var mailvalue=Ext.getCmp('itil_ac_SpecialAccount$drawSpaceCombo').getRawValue();
					if(mailvalue=='无'){
					Ext.MessageBox.alert("提示","请您选择具体的领卡地点,谢谢您的合作！",function(btn){
					Ext.getCmp('itil_ac_SpecialAccount$drawSpaceCombo').setValue("");
					return false;
					});	
					}
				}
			...
			
			 {
				...
				listeners : {
					...
					Ext.Ajax.request({
						    ...
						    Ext.MessageBox.alert("提示","已存在该帐号名的临时人员远程接入帐号,请确认后再提申请！",function(btn){
			...
			{
			xtype : 'fieldset',
		    title : '<h3><font color=red>远程接入帐号申请说明:</font></h3> ',
			layout : 'table',
		    anchor : '100%',
		    animCollapse:true,
			collapsible :true,
			...
			buttons : [{
				...
				handler : function() {
					...
					/*var drawSpace=Ext.getCmp('itil_ac_SpecialAccount$drawSpaceCombo').getRawValue();
					if(drawSpace=='无'){
						Ext.MessageBox.alert("提示","领卡地点不能够选择无，请选择具体的领卡地点！");
						return false;
					}*/
				...
				handler : function() {
					...
					Ext.Ajax.request({
								url : webContext
										+ '/accountAction_getTempAccountApply.action',
								params : {
									accountType : 'TempVPNAccount',
								...
								success : function(response, options) {
								    ...
								    if(responseArray.success){
								    	if(responseArray.type=='hasaccount'){
									    		Ext.MessageBox.alert("提示","已存在该帐号名的临时人员远程接入帐号,请确认后再提申请！",function(btn){
						                         Ext.getCmp('itil_ac_SpecialAccount$accountNameCombo').setValue();
						                        Ext.getCmp("save").enable();
												Ext.getCmp("submit").enable();
									    		...
									    		Ext.MessageBox.alert("提示","该帐号名已存在审批中的临时人员远程接入帐号申请,请确认后再提申请！",function(btn){
						                         Ext.getCmp('itil_ac_SpecialAccount$accountNameCombo').setValue();
						                         Ext.getCmp("save").enable();
												 Ext.getCmp("submit").enable();
												 Ext.getCmp("back").enable();
											...
											Ext.Ajax.request({
												...
												success : function(response, options) {
													...
													Ext.Ajax.request({
														...
														params : {
															dataId : curId,
															userInfo:userInfo,
															drawSpace:'1',//默认是北京_上地的帐号管理员作为第一级管理员审批,工作地点 1 表示北京上地
															description:description,
															bzparam : "{dataId :'" + curId
																	+ "',applyId : '" + curId
																	+ "',accountName:'" + curName
																	+ "',applyType: 'acproject'," 
																	+ "applyTypeName: '临时人员远程接入帐号申请',"
														...
														failure : function(response, options) {
														xt.MessageBox.alert("提示", "提交申请失败,请检查审批人是否选择正确!");
															Ext.getCmp("save").enable();
											                Ext.getCmp("submit").enable();
											                Ext.getCmp("back").enable();
														}
												...
												failure : function(response, options) {
													xt.MessageBox.alert("提示", "提交申请失败,请检查帐号名称是否选择正确!");
													Ext.getCmp("save").enable();
											                Ext.getCmp("submit").enable();
											                Ext.getCmp("back").enable();
												}
...
//														"提示",
//														"使用人还没有临时人员邮件帐号,请先提交临时人员邮件帐号申请,是否创建该申请？",
