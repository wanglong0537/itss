PageTemplates = Ext.extend(Ext.Panel, {
		...
		this.formpanel_SpecailAccountApply_Input = new Ext.form.FormPanel({
...

				{
					...
					listeners : {
						...
						'select' : function(combo, record, index) {
							...
							if (mailvalue == '无') {
								Ext.MessageBox.alert("提示",
										"请您选择具体的还卡地点,谢谢您的合作！", function(btn) {
											Ext
													.getCmp('itil_ac_SpecialAccount$drawSpaceCombo')
													.setValue("");
											return false;
			...
			buttons : [{
				...
				handler : function() {
					...
					if (records.length < 1) {
						Ext.MessageBox.alert('警告', '您不存在可删除的帐号!');
						return false;
					}
					var record = Ext.getCmp('account').getSelectionModel()
							.getSelected();
					...
					var num=0;
					//add by liuying at 20100531 for 记录删除帐号是否为软令牌 end
					for (var i = 0; i < records.length; i++) {
					...
					if(num==records.length){
						var curDataId = this.dataId;
									var info = Ext
											.encode(getFormParam('panel_SpecailAccountApply_Input'));
									var drawSpace = Ext
											.getCmp('itil_ac_SpecialAccount$drawSpaceCombo')
											.getValue();
									Ext.Ajax.request({
										...
										success : function(response, options) {
											...
											var applyReason = new Array();
											var accountName = new Array();
											//add by liuying at 20100531 for 记录删除帐号是否为软令牌 start
											var vpnTypes = new Array();
											var num=0;
											//add by liuying at 20100531 for 记录删除帐号是否为软令牌 end
											for (var i = 0; i < records.length; i++) {
											...
											if(num==0){//表示删除的帐号全是硬令牌
											...
											}else if(num==records.length){//表示删除的帐号全是软令牌 
												drawSpace='1';
												Ext.Ajax.request({
													url : webContext
															+ '/accountWorkflow_applyRemoteAccessAccount.action',
												...
												Ext.Ajax.request({
													url : webContext
															+ '/accountWorkflow_applyVPNAccountDelete.action',
													params : {
														dataId : curId,
														userInfo : userInfo,
														drawSpace : drawSpace,
		...
		if (this.dataId == "0" || this.dataId == "null") {
			...
			this.formpanel_SpecailAccountApply_Input.load({
				...
				success : function(action, form) {
					...
					Ext.getCmp("AccountApplyMainTable$confirmUserCombo")
							.initComponent();
					Ext.getCmp("itil_ac_SpecialAccount$drawSpaceCombo")
							.initComponent();
				}
			});
	...
	initComponent : function() {
		...
		this.grid2 = new Ext.grid.EditorGridPanel({
			...
			bbar : [new Ext.Toolbar.TextItem('<font color=red>若删除多个帐号,可以选中帐号前面的复选框</font>')],
			columns : [sm, {
				header : '自动编号',
				dataIndex : 'id',
				align : 'left',
