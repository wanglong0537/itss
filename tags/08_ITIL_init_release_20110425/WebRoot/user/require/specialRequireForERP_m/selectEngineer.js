PageTemplates = Ext.extend(Ext.Panel, {
	...
	saveAndSubmit : function() {
		...
		if(!Ext.getCmp('panel_erpSR_it_m_input').form.isValid()){
			Ext.MessageBox.alert("提示","页面中带红色波浪线的为必填项,请填写完整。谢谢您合作！");	
			return false;
		}
		var formParam = Ext.getCmp('panel_erpSR_it_m_input').form.getValues(false);
		var vp = null;
		if (formParam != null) {
	...
	getFormpanel_erpSR_it_m_input : function() {
		...
		if (this.dataId != '0') {
			data = da.getPanelElementsForEdit("esr_m_selectEngineer",
	...
	initComponent : function() {
		...
		this.model = "esr_m_selectEngineer";
		var buttonUtil = new ButtonUtil();
		this.mybuttons = buttonUtil.getButtonForModel("esr_m_selectEngineer",this);
