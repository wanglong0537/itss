HistroyForm = Ext.extend(Ext.Panel, {
	...
	foredit : true,
	width : 800,
	clazz : "com.digitalchina.itil.service.entity.ServiceItemApplyAuditHis",
	frame : true,
	defaults : {
	...
	initComponent : function() {
		...
		this.hisGrid = this.getGrid(this.reqIdHis,
				"com.digitalchina.itil.service.entity.ServiceItemApplyAuditHis",
				this.picPanel);
	...
	testfun : function() {
		...
		Ext.getCmp('hisPanelId').setTitle("审批历史——"+record.get('serviceItem'));
		if (record == undefined) {
	...
	getGrid : function(kplanId, clazz, picPanel) {
		...
		for (var i = 0; i < obj.length; i++) {
			...
			var isHidden = false;
			var columnWidth = 120;
			if (propertyName == 'id'||propertyName == 'serviceItem'||propertyName == 'definitionName'||propertyName == 'processId') {
				isHidden = true;
			}
		...
		var str=g.getStore().getAt(row).get('comment');
		if(str==undefined){
		}else{
		if(str.indexOf('\\r\\n')>0){
				var str1=str.split('\\r\\n');
				...
				message=message+str1[i]+'<br>';
				}
				Ext.Msg.show({
				   title:'审批意见',
				   msg:'审 批 人 : '+g.getStore().getAt(row).get('approver')+'<br>'+'结点名称 : '+g.getStore().getAt(row).get('nodeName')+'<br>'+'审批意见 ： '+message,
				   buttons: Ext.Msg.OK,
				   animEl: 'elId'
				...
				Ext.Msg.show({
				   title:'审批意见',
				   msg:'审 批 人 : '+g.getStore().getAt(row).get('approver')+'<br>'+'结点名称 : '+g.getStore().getAt(row).get('nodeName')+'<br>'+'&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp审批意见 ： '+str,
				   buttons: Ext.Msg.OK,
				   animEl: 'elId',
				   icon: Ext.MessageBox.INFO
				});
