PageTemplates = Ext.extend(Ext.Panel, {
	...
	save : function() {
		...
		if(!Ext.getCmp('panel_SpecialRequire1_m_Input').form.isValid()){
		...
		var curscid = this.serviceItemId;
		var formParam = Ext.getCmp('panel_SpecialRequire1_m_Input').form.getValues(false);
		var vp = null;
		if (formParam != null) {
		...
		var curUrl = webContext
				...
				+ 'pagePanel=panel_SpecialRequire1_m_Input'
		...
		Ext.Ajax.request({
			...
			success : function(response, options) {
				...
				window.location = webContext
						...
						+ "&serviceItemId=" + curscid + "&processType=1";
			},
 ...
 getFormpanel_SpecialRequire1_m_Input: function() {
 	...
 	var curSi = this.serviceItemId;
 	  var da = new DataAction();
		...
		// 判断是新增还是修改
		var biddata = "";
		data = da.getModifyRequireFormPanelElementsForEdit("panel_SpecialRequire1_m_Input",this.dataId,this.oldId);
			...
			if (idStr.indexOf('$applyNum') > 0
				...
				if(idStr=='SpecialRequirement$oldApplyCombo'){
					data[i].store = new Ext.data.JsonStore({
						url:webContext+'/extjs/comboDataAction?clazz=com.digitalchina.itil.require.entity.SpecialRequirement',
						fields:['id','name'],
						listeners:{
		...
		this.formpanel_SpecialRequire1_m_Input = new Ext.form.FormPanel({
			id : 'panel_SpecialRequire1_m_Input',
			layout : 'table',
			height : 'auto',
			width : 800,
			frame : true,
		...
		return this.formpanel_SpecialRequire1_m_Input;
	},
  items : this.items,
	/*
	...
	initComponent : function() {
		...
		var histroyForm = new HistroyForm({
	     ...
	     this.getFormpanel_SpecialRequire1_m_Input();
		  ...
		  this.tab = new Ext.TabPanel({
			...
			items : [this.formpanel_SpecialRequire1_m_Input, histroyForm]
		});

		if (this.status != 0) {
			this.items = [this.tab];
		} else {
			this.items = [this.formpanel_SpecialRequire1_m_Input];
		}
