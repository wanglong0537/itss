...
PageModelComponentPanel = Ext.extend(Ext.Panel, {
	...
	worksubmit : function() {
		Ext.Ajax.request({
			url : webContext + '/serviceItemWorkflow_apply.action',
			params : {
				dataId : this.dataId,
				model : this.model,
				bzparam : "{dataId :'" + this.dataId
						...
						+ "',applyId : '" + this.dataId
						+ "',applyType: 'cproject',applyTypeName: '服务目录审批',customer:'',alterFlag:'审批','workflowHistory':'com.digitalchina.itil.service.entity.ServiceCatalogueAuditHis'}",
				...
				if (meg.id != undefined) {
					Ext.Msg.alert("提示", "启动工作流成功", function() {
						...
						window.location = webContext
								+ '/user/serviceIssue/serviceCatalogueIssueList.jsp';
					});
				} else {
	...
	save : function() {
		...
		if (info.ServiceCatalogue$name == ""
				|| info.ServiceCatalogue$customerType == ""
				|| info.ServiceCatalogue$customer == ""
				|| info.ServiceCatalogue$beginDate == ""
				|| info.ServiceCatalogue$endDate == "") {
	...
	getPanel : function() {
		...
		var cpanel = new Ext.form.FormPanel({
			...
			// buttons : this.button,
			title : "创建服务目录 <font color=red><提示:请先保存,再维护服务目录关系></font>",
			items : biddata
		});
		return cpanel;
	},
	...
	getGridPanel : function() {
		...
		for (var i = 0; i < columns.length; i++) {
			if (columns[i].dataIndex == 'ServiceItemSLA$serviceCatalogue'
					|| columns[i].dataIndex == 'ServiceItemSLA$serviceItem') {
