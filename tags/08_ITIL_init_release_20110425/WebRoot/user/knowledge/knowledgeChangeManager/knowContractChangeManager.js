	...
	remove : function() {
		...
		var firm  =Ext.Msg.confirm('提示', '您确定要进行删除操作吗？', function(button) {
			if (button == 'yes') {
				...
				Ext.Ajax.request({
						url : webContext
								+ '/extjs/dataAction?method=remove&clazz=com.digitalchina.itil.knowledge.entity.KnowContract',
						params : {id:id[i]},
						timeout:100000, 
		  ...
		  var oldpanel={  
	              ...
	              autoLoad : {
					url : webContext + "/tabFrame.jsp?url=" + webContext
							+ "/user/knowledge/knowledgeChangeManager/oldknowContract.jsp?dataId="+oldKnowContract,
					text : "页面正在加载中......",
					method : 'post',
					scope : this
			      }
...
//													Ext.getCmp("pagetoolbar").formValue = param;
	...
	initComponent : function() {
		var da = new DataAction();
		var DataHead = da.getListPanelElementsForHead("KnowLedgeContractQuery_pagepanel");
        ...
        this.store.on("load",function(store,records, opt) {
					for(var i=0;i<records.length;i++){
						...
						}else if(records[i].get("KnowContract$status")==1){
							records[i].set("KnowContract$status","正式");
						}else if(records[i].get("KnowContract$status")==2){
							records[i].set("KnowContract$status","审批中");
			  			}else if(records[i].get("KnowContract$status")==3){
							records[i].set("KnowContract$status","过期");
			  			}else if(records[i].get("KnowContract$status")==4){
							records[i].set("KnowContract$status","变更草稿");
			  			}
					}
		});
