	...
	getFormpanel_SpecailAccountApply_Input : function() {
		...
		this.formpanel_SpecailAccountApply_Input = new Ext.form.FormPanel({
			...
			buttons : [{
				...
				handler : function() {
					...
					if (accountName == '') {
						Ext.MessageBox
								.alert("提示", "页面中带红色星号的为必填项,请填写完整。谢谢您合作！");
					}
					Ext.getCmp("save").disable();
					Ext.getCmp("submit").disable();
					Ext.getCmp("back").disable();
							...
							Ext.Ajax.request({
								...
								params : {
									...
									bzparam : "{dataId :'"
											...
											+ curName
											+ "',applyType: 'acproject',"
											+ "applyTypeName: '临时人员远程接入帐号权限变更申请',"
		...
		if (this.dataId == "0" || this.dataId == "null") {
			...
			Ext.Ajax.request({
				...
				success : function(response, options) {
					...
					if (responseArray.success) {
						...
						Ext.MessageBox.alert("提示",
								"申请人不存在可变更的临时人员远程接入帐号,请核实后再提交申请。谢谢您的合作！",
								function(btn) {
			...
			this.formpanel_SpecailAccountApply_Input.load({
				url : webContext
						+ '/accountAction_getTempVPNDraftData.action?panelName=panel_SpecailAccountApply_Input&dataId='
						+ this.dataId,
				timeout : 30,
				success : function(action, form) {
