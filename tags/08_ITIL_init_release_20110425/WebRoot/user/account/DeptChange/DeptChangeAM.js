...
//modify by liuying for 修改原邮件服务器为文本框 at 20100514 end				
			...
			buttons : [{
				...
				handler : function() {
					...
					Ext.Ajax.request({
						url : webContext + '/accountAction_saveDeptChangeResult.action',
						params : {
							id : id,
							rights : rights,
							wwwValue : wwwValue
	...
	initComponent : function() {
		...
		this.store1 = new Ext.data.JsonStore({
			url : webContext
					+ "/accountAction_listDeptChangeAccountList.action",
			fields : ['id', 'accountName', 'accountType', 'ifHold',
					'remarkDesc', 'rightsDesc'],
			root : 'data',
			totalProperty : 'rowCount',
			sortInfo : {
				field : "id",
		...
		this.grid2 = new Ext.grid.EditorGridPanel({
			title : '员工帐号信息(注<font color=red>*</font>帐号管理员请在相应帐号权限说明处双击填写审批帐号说明,该说明不能为空!谢谢您的合作!)',
			id : 'account',
			name : 'account',
			border : true,
			store : this.store1,
			...
			columns : [{
				...
				header : '<font color=red>权限说明(帐号管理员填写)</font>',
