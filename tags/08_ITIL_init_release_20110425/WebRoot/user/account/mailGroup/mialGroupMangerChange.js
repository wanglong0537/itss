	...
	getFormpanel_MailGroup : function() {
		...
		this.formpanel_MailGroup = new Ext.form.FormPanel({
			...
			items : [{
				...
				items : [{
					html : '<font color=red>*</font> 群组名称:(多个群组变更请使用分号分隔，最多十个群组，群组管理员必须相同 )',
					cls : 'common-text',
					colspan : 4,
					rowspan : 0,
				...
				new Ext.Button({
					...
					handler : function() {
						...
						Ext.Ajax.request({
						       url : webContext
								+ '/accountAction_getMailGroupMangerByName.action',
						params : {
							groupNewName:temp
						},
						...
						if(responseArray.success){	
							var groupManger=responseArray.groupManger;
							Ext.getCmp('MailGroup$groupManger').setValue(groupManger);
						}
						else{
		                  }
		               },
				failure : function(response, options) {
					  ...
					  Ext.Ajax.request({
						...
						if(responseArray.success){	
							var groupManger=responseArray.groupManger;
						    var groupMangers=Ext.getCmp('MailGroup$groupManger').getValue();
						    if(groupManger!=groupMangers){
						    		Ext.Msg.alert("提示", "多个群组变更,群组管理员必须相同.");
						    	return false;
						    }
				...
				failure : function(response, options) {
					...
					html : ' <font color=red>*</font>群组原管理员:(群组管理员将负责添加和删除群组内的成员)',
					cls : 'common-text',
					colspan : 4,
					rowspan : 0,
					style : 'border:1px dotted #b0acac;width:600;text-align:left;margin:0px 0px 0px 75px'
				...
				{
					html : '<font color=red>*</font>群组新管理员（组管理员将负责添加和删除群组内的成员，最多为十人,多人用分号分隔）',
					cls : 'common-text',
					colspan : 4,
					rowspan : 0,
					style : 'border:1px dotted #b0acac;width:600;text-align:left;margin:0px 0px 0px 75px'
					...
					xtype : 'textarea',
					id : 'MailGroup$changeGroupManger',
					colspan : 4,
					rowspan : 0,
					name : 'MailGroup$changeGroupManger',
					width : 603,
				...
				{
					html : ' <font color=red>可选择的群组管理员:</font>',
					cls : 'common-text',
					style : 'margin:0px 0px 0px 75p'
				}, 
				new Ext.form.ComboBox({forceSelection:true,
				xtype : 'combo',
				hiddenName : 'AccountApplyMainTable$delegateApplyUsers',
				id : 'AccountApplyMainTable$delegateApplyUserCombos',
				width : 200,
				style : '',
				fieldLabel : '代申请人',
				colspan : 0,
				...
				allowBlank : true,
				listWidth:500,
				name : 'AccountApplyMainTable$delegateApplyUsers',
				triggerAction : 'all',
				minChars : 50,
				queryDelay : 700,
				store : new Ext.data.JsonStore({
					...
					listeners : {
						beforeload : function(store, opt) {
							if (opt.params['AccountApplyMainTable$delegateApplyUsers'] == undefined) {
								opt.params['userName'] = Ext
										.getCmp('AccountApplyMainTable$delegateApplyUserCombos').defaultParam;
							}
						}
					},
					totalProperty : 'rowCount',
				...
				initComponent : function() {
					...
					handler : function() {
						...
						var manger = Ext.getCmp("AccountApplyMainTable$delegateApplyUserCombos").getValue();
						if (manger == "") {
							Ext.Msg.alert("提示", "请选择管理员后再添加");
							return;
						}
						var varTMP = Ext.getCmp("AccountApplyMainTable$delegateApplyUserCombos").getRawValue();
						var start = varTMP.indexOf("/");
						var end = varTMP.lastIndexOf("/");
						var tmp = varTMP.substring(start + 1, end);

						if (tmp == "") {
							Ext.Msg.alert("提示", "管理员名称不能为空");
							return false;
						}
						var mangerGroup = Ext.getCmp('MailGroup$changeGroupManger')
								.getValue();
						var number = Ext.getCmp('MailGroup$changeGroupManger')
								.getValue().split(';');
						if (number.length >= 10) {
							Ext.Msg.alert("提示", "该管理员最多为十个，不可以再继续添加！");
							return false;
						}
						if (mangerGroup.indexOf(tmp) >= 0) {
							Ext.Msg.alert("提示", "该管理员已经添加，不能重复添加");
							return false;
						}
						if (mangerGroup == "") {
							Ext.getCmp('MailGroup$changeGroupManger').setValue(tmp);

						} else {

							Ext.getCmp('MailGroup$changeGroupManger')
									.setValue(mangerGroup + ";" + tmp);
						}
						Ext.getCmp("AccountApplyMainTable$delegateApplyUserCombos").setValue("");
					}
				}), new Ext.Button({
					text : '重置',
					pressed : true,
					...
					scope : this,
					handler : function() {
						Ext.getCmp('MailGroup$changeGroupManger').setValue("");
					}
				}),
				...
				{
					html : '<font color=red>*</font>群组变更原因',
					cls : 'common-text',
					colspan : 4,
					rowspan : 0,
					style : 'border:1px dotted #b0acac;width:600;text-align:left;margin:0px 0px 0px 75px'
			...
			buttons : [{
				...
				handler : function() {
					...
					Ext.Ajax.request({
						url : webContext
								+ '/accountAction_saveMailGroupMangerChangeDraft.action',
						params : {
							info : info,
