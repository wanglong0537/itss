	   ...
	   new Ext.form.Hidden({
			...
			buttons : [{
				...
				if(!Ext.getCmp('panel_PersonAccountApply_Input').form.isValid()){
					...
					var end=Ext.getCmp('itil_ac_PersonFormalAccount$endDate').getValue();
					var now=new Date();
					if(end<now){
						Ext.MessageBox.alert("提示","到期日期不能早于今天!");
						return false;
					}
					...
					var userInfo=Ext.getCmp('AccountApplyMainTable$applyUserCombo').getValue();
                        ...
                        success : function(response, options) {
						  ...
						  if (r.success) {
                        	Ext.MessageBox.alert("提示","该用户远程接入帐号已存在,请核实简要表!");
                        	return  false;
						  }else{
						Ext.Ajax.request({
