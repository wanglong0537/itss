...
//		       emptyText : 'password123',
			...
			new Ext.form.ComboBox({forceSelection:true,
				...
				store : new Ext.data.JsonStore({
					...
					listeners : {
						beforeload : function(store, opt) {
							if (opt.params['mailServer'] == undefined) {
								opt.params['name'] = Ext
										.getCmp('mailServerCombo').defaultParam;
							}
						}
					},
					totalProperty : 'rowCount',
				...
				initComponent : function() {
					this.store.load({
						params : {
							id : Ext.getCmp('mailServerCombo')
									.getValue(),
							start : 0
						},
						callback : function(r, options, success) {
							Ext
									.getCmp('mailServerCombo')
									.setValue(Ext
											.getCmp('mailServerCombo')
											.getValue());
						}
			...
			ud.getUpDownLoadFileSu(attachmentFlag,'7735','com.digitalchina.info.appframework.metadata.entity.SystemFile','DeptMailattachment');
			}else{
			var ud=new UpDownLoadFile();
			ud.getUpDownLoadFileSu(attachmentFlag,'7735','com.digitalchina.info.appframework.metadata.entity.SystemFile','DeptMailattachment');
			}}
         ...
         buttons : [{
				...
				handler : function() {
				    ...
				    var password=Ext.getCmp('itil_ac_SpecialAccount$password').getValue();
				    
				    var mailServer=Ext.getCmp('mailServerCombo').getRawValue();
					 ...
					 Ext.getCmp("AccountApplyMainTable$applyUserCombo")
					  ...
					  Ext.getCmp("AccountApplyMainTable$platFormHRCountSignCombo")
							...
							.initComponent();
							 ...
							 Ext.getCmp("itil_ac_SpecialAccount$password").setValue("password123");
							  Ext.getCmp("mailServerCombo").setValue("BJMS0");
