			...
			items:[{
				...
				listeners : {
					...
					'select' : function(combo, record, index) {
						  ...
						  if (!r.success) {
                                   Ext.MessageBox.alert("提示","申请人不存在可删除的LMS帐号！");
								var userInfo = Ext
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
							 ...
							 if(responseArray.success){	
							 	Ext.MessageBox.alert("提示","申请人已存在审批中的LMS帐号删除申请,不能够再提该申请！",function(btn){
		...
		if (this.dataId == "0" || this.dataId == "null") {
				...
				Ext.Ajax.request({
					...
					success : function(response, options) {
					  var r=Ext.decode(response.responseText);
					  if(!r.success){
			              Ext.MessageBox.alert("提示","您不存在可删除的LMS帐号！");
