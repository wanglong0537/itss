PagePanel = Ext.extend(Ext.Panel, {
	...
	getSearchForm : function() {
		...
		var data = da.getPanelElementsForQuery("panel_trainPlan"); //da.getElementsForQuery("com.digitalchina.itil.train.entity.TrainPlan");
...
//				this.pageBar.formValue = this.formValue;
	...
	removeTrain : function(){
		var store = this.store;
				...
				var clazz = "com.digitalchina.itil.train.entity.TrainPlan";		
					if(!record){
			...
			if (button == 'yes') {	
				for(var i=0;i<records.length;i++){
					...
					var scId = records[i].get("TrainPlan$id");
							Ext.Ajax.request({
	...
	initComponent : function(){
        ...
        this.mybuttons=buttonUtil.getButtonForModel("page_train_plan_list",this);
		this.fp = this.getSearchForm();
		 this.modelTableName="TrainPlan";
		var obj = da.getListPanelElementsForHead("panel_trainPlan");
		...
		for (var i = 0; i < obj.length; i++) {
			...
			if(propertyName=='TrainPlan$deleteFlag'){
				isHiddenColumn = true;
			}
		...
		this.store = da.getPanelJsonStore("panel_trainPlan",obj); //add header 
