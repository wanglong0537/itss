PagePanel = Ext.extend(Ext.Panel, {
	...
	search : function() {//查询方法
		var tempApp = Ext.getCmp('department').getValue();
		var param = {
					department : tempApp,
						'start' : 0
		};
	...
	create : function() {
		var clazz = "com.digitalchina.itil.config.extlist.entity.PlatFormHRCountSign";//指定实体类型
...
//						
	...
	initComponent : function() {//初始化面板
		...
		this.store = new Ext.data.JsonStore({//设置下方列表的格式
			url : webContext + "/accountSystemAdminAction_listPlatFormHrSign.action",
			fields: ['id','department','realName','itcode'],
			root:'data',
		...
		this.grid = new Ext.grid.GridPanel({//设定grid
			...
			columns : [sm, {header : '编号',dataIndex : 'id',align : 'left',sortable : true,hidden : true}, 
			            {header : '部门名称',dataIndex : 'department',align : 'left',sortable : true,hidden : false,width:250},
						{header : '加签人真实性名',dataIndex : 'realName',align : 'left',sortable : true,hidden : false,width:200},
						{header : '加签人ItCode',dataIndex : 'itcode',align : 'left',sortable : true,hidden : false,width:250}
