...
function loadNewTables(){ 
	var url = "${pageContext.request.contextPath}/infoAdmin/userListMainTable.do?methodCall=toAdd";
...
<TABLE cellSpacing=1 cellPadding=3 width="100%" bgColor=#6298e1 border=0>
  ...
  <FORM name="formSearch" action="${pageContext.request.contextPath}/infoAdmin/userListMainTable.do" method="post">
        ...
        所属模块：
         ...
         <INPUT class="button" type="button" onclick="query(1);" value="检索" > 
          ...
          <TD noWrap align=right>
            <A href="javascript:query(1);">全部用户自定义主表</A>
...
<TABLE cellSpacing=1 cellPadding=0 width="100%" border=0>
 ...
 <TR>
 <TD height=30>
  ${not empty requestScope.module? requestScope.module.name : '全部'}用户自定义主表
  </TD></TR></TBODY></TABLE>


<TABLE cellSpacing=1 cellPadding=3 width="100%" bgColor=#6298e1 border=0>
  <FORM name="formDel" action="${pageContext.request.contextPath}/infoAdmin/userListMainTable.do" method="post">
 ...
 <c:forEach  var="item" items="${requestScope.page.data}" varStatus="status">   
  ...
  <TR onmouseover="this.style.backgroundColor = '#FFFFFF'" style="CURSOR: hand" 
    ...
    <A onclick='changeAdminFlag("修改导航栏目")' 
      href="${pageContext.request.contextPath}/infoAdmin/userListMainTable.do?methodCall=toForm&id=${item.id}">
      ${item.tableName }
    </A> 
    </TD>
    ...
    <TD ><a href="${pageContext.request.contextPath}/infoAdmin/userExpand/userDataInput.jsp?className=${item.className}">数据录入</a></TD>
  ...
  <TR>
    ...
    <INPUT class=button  onclick="loadNewTables();" type=button value=增加用户自定义类型 >&nbsp;
