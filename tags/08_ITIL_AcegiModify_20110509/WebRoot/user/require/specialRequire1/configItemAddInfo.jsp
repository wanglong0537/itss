...
<head>
  <title>欢迎使用IT服务系统</title>
	...
	Ext.onReady(function(){	
	   ...
	   if("${param.dataId}"==""){
	     var configItemAddInfo = new ConfigItemAddInfo({dataId:"0",configItemTypeId:citId,parentId:parentId,order:order,shipPicId:shipPicId,reqId:reqId,reqCls:reqCls,readOnly:"${param.readOnly}"});
	   }
	   else{
	   var configItemAddInfo = new ConfigItemAddInfo({dataId:"${param.dataId}",configItemTypeId:citId,parentId:parentId,order:order,shipPicId:shipPicId,reqId:reqId,reqCls:reqCls,readOnly:"${param.readOnly}"});
	   }
