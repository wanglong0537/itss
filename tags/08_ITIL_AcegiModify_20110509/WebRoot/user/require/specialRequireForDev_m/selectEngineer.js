PageTemplates = Ext.extend(Ext.Panel, {
	...
	saveAndSubmit : function() {
		if (!Ext.getCmp('panel_SpecialRequireDevConfirm_m_Input').form
				.isValid()) {
		...
		var tempEngineerId = Ext.getCmp('SpecialRequirement$mainEngineerCombo')
				.getValue();// 交付经理
		if (tempEngineerId == "") {
		...
		Ext.Ajax.request({
			...
			success : function(response, options) {
				var realEngineerId = Ext.util.JSON
						.decode(response.responseText).mainEngineer;// 交付经理
				Ext.Ajax.request({
					url : webContext + '/extjs/workflow?method=getData&taskId='
							+ taskId + '&users=makeInfoByEngineer:'
							+ realEngineerId + '$executeByEngineer:'
							+ realEngineerId + '$issueByEngineer:'
							+ realEngineerId,
					method : 'post',
					success : function(response, options) {
	...
	getFormpanel_SpecialRequireDevConfirm_m_Input : function() {
		...
		if (this.dataId != '0') {
			data = da.getPanelElementsForEdit("devsr_m_selectEngineer",
	...
	initComponent : function() {
		...
		this.model = "devsr_m_selectEngineer";
		var buttonUtil = new ButtonUtil();
		this.mybuttons = buttonUtil.getButtonForModel("devsr_m_selectEngineer",
				this);
