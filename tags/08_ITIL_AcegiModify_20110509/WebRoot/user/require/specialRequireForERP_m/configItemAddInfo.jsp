...
<head>
  <title>欢迎使用IT服务系统</title>
	...
	<script type="text/javascript" src="${pageContext.request.contextPath}/user/require/requireAuditHis.js"></script>	                                                        
  	<script type="text/javascript" src="${pageContext.request.contextPath}/user/require/specialRequireForERP/configItemAddInfo.js"></script> 
	<style type="text/css"> 
		.x-head{
	...
	Ext.onReady(function(){	
	   ...
	   var shipPicId = "${param.shipPicId}";
	   var reqId ="${param.dtId}";
	   var reqCls = "${param.reqClass}";
	   if("${param.configItemTypeId}"!=""){
	   		citId="${param.configItemTypeId}";
	   }
	   if("${param.dataId}"==""){
	     var configItemAddInfo = new ConfigItemAddInfo({dataId:"0",configItemTypeId:citId,parentId:parentId,order:order,shipPicId:shipPicId,reqId:reqId,reqCls:reqCls});
	   }
	   else{
	   var configItemAddInfo = new ConfigItemAddInfo({dataId:"${param.dataId}",configItemTypeId:citId,parentId:parentId,order:order,shipPicId:shipPicId,reqId:reqId,reqCls:reqCls});
	   }
	   configItemAddInfo.render("configItemEnv");
	   new Ext.Viewport({
	   	 layout:'fit',
	   	 items:[configItemAddInfo]
	   });
