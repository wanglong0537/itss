	...
	getFormpanel_PersonAccountApply_Input : function() {
		...
		this.formpanel_PersonAccountApply_Input = new Ext.form.FormPanel({
			...
			reader : new Ext.data.JsonReader({
				...
				name : 'itil_ac_PersonFormalAccount$departTelephone',
				mapping : 'itil_ac_PersonFormalAccount$departTelephone'
			}, {
			...
			items : [{
				...
				items : [{
				 ...
				 }), 
				 {
					...
					listeners : {
						'expand' : function() {
									var did=Ext.getCmp('AccountApplyMainTable$id').getValue();
									if (did == "0" || did == "null"||did==''){
										Ext.getCmp('itil_ac_PersonFormalAccount$telephoneNumberCombo').setValue();
									}
									Ext.getCmp('itil_ac_PersonFormalAccount$telephoneNumberCombo').getEl().dom.readOnly=true;
									Ext.getCmp('noItcodeApply').collapse(false);
									Ext.getCmp('itil_ac_PersonFormalAccount$departTelephone').setValue('0');
						}
					},
					style : 'border:1px dotted #b0acac;',
					autoHeight : true,
									...
									.getValue();
										...
										Ext.Ajax.request({
											...
											success : function(response,
												...
												if (!r.success) {
													Ext.MessageBox.alert("提示","申请不存在可用的座机！");
													 Ext.getCmp('AccountApplyMainTable$applyUserCombo').setValue("");
													return false;
													
												} else {
													Ext.getCmp('itil_ac_PersonFormalAccount$telephoneNumberCombo').setValue(r.telnum);
				...
				
				{
					...
					listeners : {
						'expand' : function() {
									...
									Ext.getCmp('itil_ac_PersonFormalAccount$telephoneNumberCombo').getEl().dom.readOnly=false;
									Ext.getCmp('haveItcodeApply').collapse(false);
									Ext.getCmp('AccountApplyMainTable$applyUserCombo').setValue('');
									...
									Ext.getCmp('itil_ac_PersonFormalAccount$departTelephone').setValue('1');
						}
					},
					style : 'border:1px dotted #b0acac;',
					autoHeight : true,
			...
			},
			{
...

				{
					...
					store : new Ext.data.JsonStore({
						url : webContext
								+ '/accountAction_listAllDeptTelephoneAccount.action',
						fields : ['id', 'number'],
						listeners : {
							beforeload : function(store, opt) {
				...
				new Ext.form.Hidden({
					...
					xtype : 'hidden',
					id : 'itil_ac_PersonFormalAccount$departTelephone',
					colspan : 0,
					rowspan : 0,
					name : 'itil_ac_PersonFormalAccount$departTelephone',
					width : 200,
			...
			buttons : [{
				...
				handler : function() {
					...
					var telephoneNumber = Ext.getCmp('itil_ac_PersonFormalAccount$telephoneNumberCombo').getValue();
					if(telephoneNumber==''||telephoneNumber==null){
					 Ext.MessageBox.alert("提示","电话号码必须从下拉列表中选择,谢谢您的合作!");
		                return false;
					}
					if(confirmUser==''){
						Ext.MessageBox.alert("提示","审批人人必须从下拉列表中选择,谢谢您的合作!");
						return false;
					}
				    ...
				    var drawSpace = Ext.getCmp('itil_ac_PersonFormalAccount$workSpaceCombo').getValue();
				    var isdepttel = Ext.getCmp('itil_ac_PersonFormalAccount$departTelephone').getValue();
					Ext.Ajax.request({
						url : webContext
								+ '/accountAction_saveTelephoneDeleteDraft.action',
						params : {
							info : info,
						...
						failure : function(response, options) {
							Ext.MessageBox.alert("提示","保存失败,请检查变更的电话号码是否填写正确!");
							Ext.getCmp("save").enable();
							Ext.getCmp("submit").enable();
							Ext.getCmp("back").enable();
						}
				...
				handler : function() {
							           ...
							           if(Ext.getDom('noItcode').checked){
										...
										var stationNumber = Ext.getCmp('itil_ac_PersonFormalAccount$stationNumber').getValue();
					                    var drawSpace = Ext.getCmp('itil_ac_PersonFormalAccount$workSpaceCombo').getValue();
										Ext.MessageBox.alert("提示","若您存在硬电话,请您将其归还到指定的平台工程师处,谢谢您的合作!",function(btn){
										var telephoneNumber = Ext.getCmp('itil_ac_PersonFormalAccount$telephoneNumberCombo').getValue();
										 var isdepttel = Ext.getCmp('itil_ac_PersonFormalAccount$departTelephone').getValue();
										Ext.Ajax.request({
											url : webContext
