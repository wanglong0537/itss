...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
  ...
  initComponent : function()
  {
    ...
    var tt = {
      ...
      // --------------
        'window-render' : [
          {
            action: [this.createForm, this.createProgressBar,this.createProgressBar, this.createGrid],
            state: 'rendering'
          }
        ],
        'destroy' : [
  ...
  createForm : function()
  {
  	...
  	var formField = "<table width=100%><tr>";
  	formField += "<td style='font-size:9pt' nowrap>文件名称：</td><td><input name='configName'style='width:180pt'></td>";
  	formField += "<td style='width:10pt'>&nbsp;&nbsp;</td>";
  	formField += "</tr></table>";
  ...
  onUploadButtonClick : function()
  {
    if (this.is_uploading) {
      this.fsa.postEvent('stop-upload');
    	...
    	if(this.form.configName.value==""){
  			Ext.MessageBox.alert("提示信息","配置名称不能为空！");
  			this.form.configName.focus();
    	}
    	else{
