...
<script language="javascript" src="${pageContext.request.contextPath}/js/Admin.js"></script>
	...
	function sortTo(targetOrderFlag){ 
  		  //infoAdmin/sysTableQuery.do?methodCall=listQueryColumn&queryId=3
   		   ...
   		   if(confirm("您确认移动所勾选的系统角色可见字段吗? ")){
   		     var sRoleId = "${param.sRoleId}";
   		     var sourceItemIds="";
   		     var validate=false;
   		     var settingType = "${param.settingType}";
   		     <c:forEach var="item" items="${requestScope.sysTableRoleColumns}" varStatus="status">
     		 ...
     		 if(sort_checked.checked==true){
   		    ...
   		    if(!validate){
   		      alert("请勾选要移动的系统角色可见字段!");
   		      return false;
   		    }
   		     Ext.Ajax.request({
	            url:'${pageContext.request.contextPath}/infoAdmin/sysTableRole.do?methodCall=sortTableRoleColumn',
	            params:{
	                sRoleId: sRoleId, targetOrderFlag:targetOrderFlag, sourceOrderFlags: sourceItemIds, settingType: settingType
	            },
	            method:'POST',
	            timeout:100000,
    ...
    <TD noWrap align=middle bgColor=#ebf2f9 height=36>
      <TABLE cellSpacing=0 width="100%" border=0>
        ...
        <TR>
         ...
         <a href="${pageContext.request.contextPath}/infoAdmin/sysTableSetting.do?methodCall=list&smtId=${str.systemMainTable.id}">系统可见字段设置</a>
	    &nbsp;
	     <a href="${pageContext.request.contextPath}/infoAdmin/sysTableQuery.do?methodCall=list&smtId=${str.systemMainTable.id}">系统主表查询设置</a>
	    &nbsp;
	     <a href="${pageContext.request.contextPath}/infoAdmin/sysTableRole.do?methodCall=list&smtId=${str.systemMainTable.id}">系统角色可见字段设置</a>  
...
<TABLE cellSpacing=1 cellPadding=0 width="100%" border=0>
 <TBODY>
 <TR>
 <TD height=30>系统后台管理&nbsp;-&gt;&nbsp;系统主表&nbsp;-&gt;&nbsp;系统角色可见字段：
 
  </TD>
  <td align="right" style="color:red;">${str.systemMainTable.tableCnName}&nbsp;
...
<TABLE cellSpacing=1 cellPadding=3 width="100%" bgColor=#6298e1 border=0>
  ...
  <INPUT name="sRoleId" type="hidden" value="${str.id}">
  <TBODY>
  <TR>
 ...
 <c:forEach  var="item" items="${requestScope.sysTableRoleColumns}" varStatus="status">   
  ...
  <TR id="tr${item.id}" style="CURSOR: hand" 
    ...
    <TD noWrap>
    <input name="settingType"  id="settingType"   type="hidden" value="${item.settingType}">
  ...
  <TR style="CURSOR: hand"  bgColor=#ebf2f9>
   ...
   <TD noWrap></TD>
    ...
    <TD noWrap colspan="4">
     ...
     <INPUT class=button onclick="saveToUser();this.disabled=true;" type=button value="同步到${str.role.name }" style="float:left">	
    </TD>
