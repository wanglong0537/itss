PageTemplates = Ext.extend(Ext.Panel, {
	...
	getFormpanel_SpecialRequire3_Input : function() {
		...
		if (this.dataId != '0') {
			data = da.getPanelElementsForEdit("nsr1_confirmByTechnicManager",
		...
		for (var i = 0; i < data.length; i++) {
			if (data[i].id == 'itil_SRServiceProviderInfo$bigTypeCombo') {
				data[i].on('select', function(combo, record, index) {
					Ext.getCmp('itil_SRServiceProviderInfo$smallTypeCombo').clearValue();
				})
			}
			if (data[i].id == 'itil_SRServiceProviderInfo$smallTypeCombo') {
				data[i].on('beforequery', function(queryEvent) {
					var pid = Ext.getCmp('itil_SRServiceProviderInfo$bigTypeCombo').getValue();
					if (pid == "") {
			...
			if (data[i].id == 'itil_SRServiceProviderInfo$serviceProviderInCombo') {
				data[i].on('select', function(combo, record, index) {
					Ext.getCmp('itil_SRServiceProviderInfo$mainEngineerCombo').clearValue();
					Ext.getCmp('itil_SRServiceProviderInfo$assistanEngineerCombo').clearValue();
				})
			}
	...
	initComponent : function() {
		...
		this.model = "nsr1_selectEngineer";
		var buttonUtil = new ButtonUtil();
		this.mybuttons = buttonUtil.getButtonForModel("nsr1_selectEngineer",
				this);
		if (this.mybuttons != "") {
			this.buttons = {
				layout : 'table',
