PageTemplates = Ext.extend(Ext.Panel, {
	...
	getFormpanel_SpecialRequireDevConfirm_Input : function() {
		...
		var biddata = "";
		if (this.dataId != '0') {
			data = da.getPanelElementsForEdit("devsr_makeShareInfo",
	...
	getFormpanel_srInfo_input1 : function() {
		...
		var biddata = "";
		if (this.dataId != '0') {
			data = da.getPanelElementsForEdit("devsr_makeShareInfo","panel_srInfo_input1", this.dataId);// 这是要随时变得
		} else {
	...
	getFormserviceItemBasePanel : function() {
		var siId = "0";
		Ext.Ajax.request({  
	        url: webContext+'/SRAction_getReqServiceItemId.action?reqId='+this.dataId,  
	        async: false,  
	        success: function(response, opts) { 
	        	var scidata = Ext.util.JSON.decode(response.responseText);
	        	siId = scidata.id;
	        }  
	    }); 
		var da = new DataAction();
...
//			collapsible : true,
	...
	initComponent : function() {
		...
		var temp = new Array();
		var items = new Array();
		this.model = "devsr_makeShareInfo";
		var buttonUtil = new ButtonUtil();
		this.mybuttons = buttonUtil.getButtonForModel("devsr_makeShareInfo",this);
