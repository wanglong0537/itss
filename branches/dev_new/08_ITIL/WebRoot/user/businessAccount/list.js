AccountList = Ext.extend(Ext.Panel, {
	...
	getPanelJsonStore : function(panelname, header) { 
		...
		var store = new Ext.data.JsonStore({
			id : Ext.id(),
			url : webContext + '/businessAccountAction_queryRequireByName.action', //改成pageQuery测试
			root : "data",
			fields : fields,
...
//		alert(dataId);
		window.location = webContext+ "/user/businessAccount/requirePlanInfo.jsp?dataId="+dataId;
	},
...
//		this.store = da.getPanelJsonStore("panel_businessAccount_list",obj); //add header 
		var gridStore = new Ext.data.JsonStore({
			url : webContext
					+ '/businessAccountAction_queryRequireByName.action',
			totalProperty : 'rowCount',
			root : 'data',
			id : 'id',
			fields : ["SpecialRequirement$id","SpecialRequirement$applyNum", "SpecialRequirement$name", "SpecialRequirement$oldApply", "SpecialRequirement$requireId",
		...
		this.searchConfig = function() {
			var params = {
					...
					requireId:Ext.getCmp("requireId").getValue()
					};
