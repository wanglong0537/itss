	...
	remove : function() {
		...
		var firm  =Ext.Msg.confirm('提示', '您确定要进行删除操作吗？', function(button) {
			if (button == 'yes') {
				...
				Ext.Ajax.request({
						url : webContext
								+ '/extjs/dataAction?method=remove&clazz=com.zsgj.itil.knowledge.entity.KnowFile',
						params : {id:id[i]},
						timeout:100000, 
	...
	initComponent : function() {
        ...
        this.store.on("load",function(store,records, opt) {
					for(var i=0;i<records.length;i++){
						...
						if(records[i].get("KnowFile$status")==0){
						 	records[i].set("KnowFile$status","草稿");
						}
					}
		});
			...
			tbar : [{
				...
				handler : function() {
					window.location = webContext
									+ "/user/knowledge/knowledgeManager/createNewKnowFile.jsp";
				},
				scope : this,
				iconCls : 'add'
			},'-',{
