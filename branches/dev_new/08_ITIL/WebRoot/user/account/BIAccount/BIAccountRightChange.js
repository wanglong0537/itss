PageTemplates = Ext.extend(Ext.Panel, {
	...
	getTabpanel : function(tab, tabTitle) {
		this.tabPanel = new Ext.TabPanel({
			...
			items : tab,
			tbar : [new Ext.Toolbar.TextItem('<font color=red>提示：</font><font color=blue>1.页面中带红色<font color=red>*</font>号的必填项，请在填写完整后再提交申请!2.审批人必须从下拉列表中选择</font>')]
	...
	getFormpanel_PersonAccountApply_Input : function() {
		...
		this.formpanel_PersonAccountApply_Input = new Ext.form.FormPanel({
			...
			items:[
			 {
				html : '<font color=red>*</font> BI服务权限变更原因:',
				cls : 'common-text',
				colspan : 2,
				rowspan : 0,
			...
			{
				html : '<font color=red>*</font> BI服务权限变更描述：<br>(为了能准确反馈您需要的权限，准备了一个权限注册描述事例，请按照该方式填写：我从系统科技公司HP事业部调到IBM事业部，希望查看IBM事业部销售、欠款等数据。)',
				cls : 'common-text',
				colspan : 2,
				rowspan : 0,
				style : 'border:1px dotted #b0acac;width:600;text-align:left;margin:0px 0px 0px 75px'
			 ...
			 {
				html : ' 如果需要申请的权限与现有的某一个BI帐号权限一致请填写该帐号<br>（例如：我需要申请与现有BI帐号sungy一致的权限）：',
				cls : 'common-text',
				colspan : 2,
				rowspan : 0,
				style : 'border:1px dotted #b0acac;width:600;text-align:left;margin:0px 0px 0px 75px'
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
							 ...
							 if(responseArray.success){	
							 	Ext.MessageBox.alert("提示","申请人已存在审批中的BI帐号权限变更申请,不能够再提该申请！",function(btn){
		...
		if (this.dataId == "0" || this.dataId == "null") {
			Ext.Ajax.request({
						...
						success : function(response, options) {
							...
							.initComponent();
							  Ext.getCmp("itil_ac_PersonFormalAccount$applyReason").setValue("");
				             ...
				             failure : function(response, options) {
							   Ext.MessageBox.alert("提示","申请人不存在可变更的BI帐号,请确认后在提申请！",function(btn){
		                      	window.history.back(-1);
		                      	});
				              }
		                      })
