		...
		this.formpanel_SpecailAccountApply_Input = new Ext.form.FormPanel({
													   ...
													   Ext.Ajax.request({
														...
														Ext.Msg.show({
																		   ...
																		   fn:function(button,text){
																		    if(button=='ok'){
																				Ext.getCmp('itil_ac_SpecialAccount$accountOldUserCombo').setValue('');
																		    }
																		   },
			...
			items:[
			
			 {
				...
				queryDelay : 700,
				store : new Ext.data.JsonStore({
					url : webContext + "/accountAction_listWin7AccountName.action",
				...
				listeners : {
					...
					if(olduser==null||olduser==''){
						 Ext.MessageBox.alert("提示","请先选择帐号原所有者,谢谢您的合作！");
						 return false;
					}
						var param = queryEvent.combo.getRawValue();
						this.defaultParam = param;
				...
				var responseArray = Ext.util.JSON.decode(response.responseText);
			        ...
			        var accountNewOwner=Ext.getCmp('AccountApplyMainTable$applyUserCombo').getValue();
			         var telephone=Ext.getCmp('itil_ac_SpecialAccount$telephone').getValue();
						var id = Ext.getCmp('itil_ac_SpecialAccount$accountNameCombo').getValue();
			...
			,
				...
				handler : function() {
		           ...
		           Ext.getCmp("save").disable();
		            ...
		            var accountId=Ext.getCmp('itil_ac_SpecialAccount$accountNameCombo').getValue();
					Ext.Ajax.request({
								url : webContext
										+ '/accountAction_getAccountNameApply.action',
								params : {
									accountType : 'Win7',
								...
								success : function(response, options) {
									var responseArray = Ext.util.JSON.decode(response.responseText);
								    if(responseArray.success){
								    	Ext.MessageBox.alert("提示","该帐号名已存在审批中的Win7帐号所有者变更申请,请确认后再提申请！",function(btn){
						                         Ext.getCmp('itil_ac_SpecialAccount$accountNameCombo').setValue();
						                         Ext.getCmp("save").enable();
												 Ext.getCmp("submit").enable();
												 Ext.getCmp("back").enable();
