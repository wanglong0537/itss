				...
				var responseArray = Ext.util.JSON.decode(response.responseText);
					...
					var accountName=new Array();
				     ...
				     Ext.Ajax.request({
						...
						params : {id:id[i],applyReason:applyReason[i],curId:curId,accountName:accountName[i],accountType:'DeptMail'},
						timeout:100000, 
...
//					}
					var records = Ext.getCmp('account').getStore().getRange(0,
					              ...
					              if (records.length <1) {
			                      Ext.MessageBox.alert('警告', '你还不存在可删除的部门邮件帐号!');
			                      return false;
					              }
				   var record =  Ext.getCmp('account').getSelectionModel().getSelected();
		           var records =  Ext.getCmp('account').getSelectionModel().getSelections();
...
//		            sm.selectAll();
	                this.grid2 = new Ext.grid.EditorGridPanel({
	                   ...
	                   name:'account',
			           store : this.store1,
			            tbar : [new Ext.Toolbar.TextItem('<font color=red>提示：</font><font color=blue>1.双击删除原因单元格可以进行编辑</font>')],
			...
			columns : [sm, {header : '自动编号',dataIndex : 'id',align : 'left',sortable : true,hidden : true}, 
						...
						{header : '<font color=red>删除原因</font>',dataIndex : 'applyReason',align : 'center',sortable : true,hidden : false,editor:new Ext.grid.GridEditor(new Ext.form.TextField({allowBlank:true}))},
