			...
			items:[{
				...
				listeners : {
					...
					'select' : function(combo, record, index) {
						  ...
						  if (!r.success) {
//                                Ext.MessageBox.alert("提示","申请人不存在可删除的"+ Ext.getCmp('PageTemplates').serviceName+"！");
                                Ext.MessageBox.alert("提示","申请人不存在可删除的"+ Ext.getCmp('PageTemplates').serviceName+"！");
								var userInfo = Ext
							...
							Ext.Ajax.request({
								...
								params : {
									...
									bzparam : "{dataId :'" + curId
											...
											+ "',accountName:'" + curName
											+ "',applyType: 'acproject'," 
											+ "applyTypeName: '"+ Ext.getCmp('PageTemplates').serviceName+"删除申请',"
		...
		if (this.dataId == "0" || this.dataId == "null") {
				...
				Ext.Ajax.request({
					...
					success : function(response, options) {
					  var r=Ext.decode(response.responseText);
					  if(!r.success){
//			              Ext.MessageBox.alert("提示","您不存在可删除的WWW帐号！");
