	   ...
	   new Ext.form.Hidden({
		...
		buttons : [{
				...
				handler : function() {
					...
					//var drawSpace=Ext.getCmp('itil_ac_PersonFormalAccount$drawSpaceCombo').getRawValue();
					//if(drawSpace=='无'){
				      ...
				      success : function(response, options) {
				       var responseArray = Ext.util.JSON
				        ...
				        if(responseArray.success){ 
					         Ext.MessageBox.alert("提示","申请人已存在审批中的远程接入帐号申请,不能够再提该申请！",function(btn){
											...
											Ext.Ajax.request({
												...
												success : function(response, options) {
														...
														Ext.Ajax.request({
															...
															failure : function(response, options) {
																Ext.MessageBox.alert("提示", "请检查审批人是否选择正确!");
																Ext.getCmp("save").enable();
												                Ext.getCmp("submit").enable();
												                Ext.getCmp("back").enable();
															}
