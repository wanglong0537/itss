...
function loadNewTables(){ 
	var url = "${pageContext.request.contextPath}/infoAdmin/userMainTable.do?methodCall=toAdd";
	window.location.href = url;
}


function exportAllCITypeData(){ 
    var serverPath = "${pageContext.request.contextPath}";
    //var citypeTableId = ${citype.id}; 

    Ext.Ajax.request({
       url: serverPath+"/infoAdmin/userMainTable.do?methodCall=genAllCITypeDataForRelation", 
       params:{
           citypeTableId: 'all'
       },
       method:'POST',
...
function genAllCICisnCode(){ 
    var serverPath = "${pageContext.request.contextPath}";
    
    Ext.Ajax.request({
       url: serverPath+"/infoAdmin/userMainTable.do?methodCall=genAllCICisnCode", 
       params:{
           citypeTableId: 'all'
       },
       method:'POST',
...
<TABLE cellSpacing=1 cellPadding=3 width="100%" bgColor=#6298e1 border=0>
  ...
  <TR>
    <TD noWrap height=24><FONT color=#ffffff><IMG height=18 
      ...
      border=0>&nbsp;<STRONG>新增数据模型：提供用户创建表功能，实现实体类和映射信息自动生成，生成对应面板</STRONG></FONT></TD></TR>
    <TR>
  ...
  <FORM name="formSearch" action="${pageContext.request.contextPath}/infoAdmin/userMainTable.do" method="post">
        ...
        所属模块：
         ...
         用户主表名称：<INPUT name="tableName" type="text" value="${param.tableName}">
         ...
         <INPUT class="button" type="button" onclick="query(1);" value="检索" > 
          ...
          <TD noWrap align=right>
            <A href="javascript:query(1);">全部用户主表表</A>
...
<TABLE cellSpacing=1 cellPadding=0 width="100%" border=0>
 ...
 <TR>
 <TD height=30>
  ${not empty requestScope.module? requestScope.module.name : '全部'}用户主表表
  </TD></TR></TBODY></TABLE>


<TABLE cellSpacing=1 cellPadding=3 width="100%" bgColor=#6298e1 border=0>
  <FORM name="formDel" action="${pageContext.request.contextPath}/infoAdmin/userMainTable.do" method="post">
  ...
  <TR>
    ...
    <STRONG><FONT color=#ffffff>用户表名</FONT></STRONG>
    </TD>
    <TD  noWrap bgColor=#8db5e9>
    ...
    <TD noWrap bgColor=#8db5e9>
    <FONT color=#ffffff><STRONG>数据录入</STRONG></FONT>
    </TD>
    <TD noWrap bgColor=#8db5e9>
 ...
 <c:forEach  var="item" items="${requestScope.page.data}" varStatus="status">   
  ...
  <TR onmouseover="this.style.backgroundColor = '#FFFFFF'" style="CURSOR: hand" 
    ...
    <A onclick='changeAdminFlag("修改导航栏目")' 
      href="${pageContext.request.contextPath}/infoAdmin/userMainTable.do?methodCall=toForm&id=${item.id}&pageNo=${param.pageNo}">
      ${item.tableName }
    </A> 
    </TD>
    
    <TD noWrap>${item.tableCnName }</TD>
    <TD noWrap>${item.className }</TD>
    <TD noWrap><a href="${pageContext.request.contextPath}/infoAdmin/userExpand/userDataInput.jsp?className=${item.className}">数据录入</a></TD>
  ...
  <TR>
    ...
    <INPUT class=button  onclick="loadNewTables();" type=button value=增加用户主表 >&nbsp;
