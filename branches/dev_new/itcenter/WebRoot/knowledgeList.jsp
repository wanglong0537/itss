...
<html>
    ...
    <td width="800" style="vertical-align: top" align="left">
   	 ...
   	 <c:forEach var="obj" items="${pagination.data}">
   	 	...
   	 	<tr>
	   	 	...
	   	 	<td width="2%" ></td>
	   	 	<td align="left"><a href="${pageContext.request.contextPath}/knowledgeAction_getContentInfoAction.action?id=${obj[0].id}" target="_blank" class="listLink">${obj[0].summary}</a><span class="bluetxt" style=" color:#666666">　[${obj[3].name}]</span></td>
  ...
  <td colspan="3" align="center"><span style="font-family:宋体;font-size: 14px;color:#666666;" >当前页数：${pagination.pageNum}　总页数：${pagination.pageCount}</span>　　
  		<c:if test="${pagination.pageNum!=1}">
			<c:url var="first" value="/knowledgeAction_getListInfoAction.action?infoLength=30&pageSize=10&columnType=${columnType}">
				<c:param name="pageNum" value="1" />
			</c:url>
			<c:url var="previous" value="/knowledgeAction_getListInfoAction.action?infoLength=30&pageSize=10&columnType=${columnType}">
		...
		<c:if test="${pagination.pageNum<pagination.pageCount}">
			<c:url var="next" value="/knowledgeAction_getListInfoAction.action?infoLength=30&pageSize=10&columnType=${columnType}">
				<c:param name="pageNum" value="${pagination.pageNum+1}" />
			</c:url>
			<c:url var="last" value="/knowledgeAction_getListInfoAction.action?infoLength=30&pageSize=10&columnType=${columnType}">
