...
<html>
    ...
    <td width="800" style="vertical-align: top" align="left">
   	 ...
   	 <c:forEach var="knowFile" items="${pagination.data}">
   	 	...
   	 	<tr>
	   	 	...
	   	 	<td width="2%" ></td>
	   	 	<td align="left"><a href="${pageContext.request.contextPath}/knowFileAction_getContentInfoAction.action?id=${knowFile.id}" target="_blank" class="listLink"><%request.setAttribute("redKeyValue", "<font color=red>"+request.getAttribute("keyValue").toString()+"</font>");%>${fn:replace(knowFile.name,keyValue,redKeyValue)}</a>　</td>
  ...
  <td colspan="3" align="center"><span style="font-family:宋体;font-size: 14px;color:#666666;" >当前页数：${pagination.pageNum}　总页数：${pagination.pageCount}</span>　　
  		<c:if test="${pagination.pageNum!=1}">
			<c:url var="first" value="/knowFileAction_getSearchInfoAction.action?infoLength=${infoLength}&pageSize=${pageSize}&columnType=${pagination.data[0].knowFileType}&keyValue=${keyValue}">
				<c:param name="pageNum" value="1" />
			</c:url>
			<c:url var="previous" value="/knowFileAction_getSearchInfoAction.action?infoLength=${infoLength}&pageSize=${pageSize}&columnType=${pagination.data[0].knowFileType}&keyValue=${keyValue}">
		...
		<c:if test="${pagination.pageNum<pagination.pageCount}">
			<c:url var="next" value="/knowFileAction_getSearchInfoAction.action?infoLength=${infoLength}&pageSize=${pageSize}&columnType=${pagination.data[0].knowFileType}&keyValue=${keyValue}">
				<c:param name="pageNum" value="${pagination.pageNum+1}" />
			</c:url>
			<c:url var="last" value="/knowFileAction_getSearchInfoAction.action?infoLength=${infoLength}&pageSize=${pageSize}&columnType=${pagination.data[0].knowFileType}&keyValue=${keyValue}">
