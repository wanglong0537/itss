ArchivesDistView = Ext.extend(Ext.Panel, {
	...
	initComponents : function() {
		...
		this.topbar = new Ext.Toolbar( {
			...
			items : [ {
				iconCls : "btn-add",
				text : "添加[ArchivesDist]",
				xtype : "button",
				handler : this.createRecord
			}, {
				iconCls : "btn-del",
				text : "删除[ArchivesDist]",
				xtype : "button",
		...
		this.gridPanel.addListener("rowdblclick", function(d, c, f) {
			d.getSelectionModel().each( function(e) {
				new ArchivesDistForm(e.data.archDistId).show();
			});
		});
	...
	createRecord : function() {
		new ArchivesDistForm().show();
	},
	delRecord : function(a) {
		Ext.Msg.confirm("信息确认", "您确认要删除该记录吗？", function(b) {
			if (b == "yes") {
				Ext.Ajax.request( {
					url : __ctxPath + "/archive/multiDelArchivesDist.do",
					params : {
						ids : a.data.archDistId
					},
					method : "POST",
					success : function(c, d) {
						Ext.ux.Toast.msg("操作信息", "成功删除该公文分类！");
						Ext.getCmp("ArchivesDistGrid").getStore().reload();
					},
	...
	editRecord : function(a) {
		new ArchivesDistForm( {
			archDistId : a.data.archDistId
		}).show();
	},
