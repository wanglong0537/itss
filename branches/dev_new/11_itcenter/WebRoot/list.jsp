...
<html>
    ...
    <td width="800" style="vertical-align: top" align="left">
   	 ...
   	 <c:forEach var="notice" items="${pagination.data}">
   	 	...
   	 	<tr>
	   	 	...
	   	 	<td width="2%" ></td>
	   	 	<td align="left"><a href="${pageContext.request.contextPath}/noticeAction_getContentInfoAction.action?id=${notice.id}" target="_blank" class="listLink">${notice.title}</a>　</td>
  ...
  <td colspan="3" align="center"><span style="font-family:宋体;font-size: 14px;color:#666666;" >当前页数：${pagination.pageNum}　总页数：${pagination.pageCount}</span>　　
  		<c:if test="${pagination.pageNum!=1}">
			<c:url var="first" value="/noticeAction_getListInfoAction.action?infoLength=30&pageSize=10&columnType=${pagination.data[0].newNoticeType}">
				<c:param name="pageNum" value="1" />
			</c:url>
			<c:url var="previous" value="/noticeAction_getListInfoAction.action?infoLength=30&pageSize=10&columnType=${pagination.data[0].newNoticeType}">
		...
		<c:if test="${pagination.pageNum<pagination.pageCount}">
			<c:url var="next" value="/noticeAction_getListInfoAction.action?infoLength=30&pageSize=10&columnType=${pagination.data[0].newNoticeType}">
				<c:param name="pageNum" value="${pagination.pageNum+1}" />
			</c:url>
			<c:url var="last" value="/noticeAction_getListInfoAction.action?infoLength=30&pageSize=10&columnType=${pagination.data[0].newNoticeType}">
