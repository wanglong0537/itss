...
public class ProcessRunServiceImpl extends BaseServiceImpl<ProcessRun>
	...
	public void saveProcessRun(ProcessRun processRun, ProcessForm processForm,
		...
		while (fieldNames.hasNext()) {
			...
			ParamField paramField = (ParamField) runInfo.getParamFields().get(
					fieldName);
			FormData fd = null;
			if (!isNewsForm) {
		...
		if (runInfo.isStartFlow()) {
			...
			variables.put("processName", processRun.getProDefinition()
					.getName());
	...
	public void saveAndNextStep(FlowRunInfo runInfo) {
		...
		if ("task".equals(nodeType)) {
			this.jbpmService.completeTask(runInfo.getTaskId(),
