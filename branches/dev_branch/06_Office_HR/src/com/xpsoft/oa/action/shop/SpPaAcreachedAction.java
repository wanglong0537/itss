...
public class SpPaAcreachedAction extends BaseAction{
	@Resource
	private SpPaAcreachedService spPaAcreachedService;
	private SpPaAcreached spPaAcreached;
	private long id;
	
	public SpPaAcreachedService getSpPaAcreachedService() {
		return spPaAcreachedService;
	}
	public void setSpPaAcreachedService(SpPaAcreachedService spPaAcreachedService) {
		this.spPaAcreachedService = spPaAcreachedService;
	}
	public SpPaAcreached getSpPaAcreached() {
		return spPaAcreached;
	}
	public void setSpPaAcreached(SpPaAcreached spPaAcreached) {
		this.spPaAcreached = spPaAcreached;
	}
	public long getId() {
		return id;
	}
	...
	public String save(){
		this.spPaAcreachedService.save(this.spPaAcreached);
	...
	public String readExcel() {
		...
		try {
			...
			if("1".equals(uploadFileType)) {//按品类导入
				...
				for(int i = 2; i < col; i++) {
					for(int j = 2; j < row; j++) {
						if(sheet.getCell(i, j) != null && !"".equals(sheet.getCell(i, j).getContents().trim())) {
							SpPaAcreached reach = new SpPaAcreached();
				...
				this.spPaAcreachedService.multiSave(reachList, templateId, deptId);
				...
				this.spPaAcreachedService.multiSave(reachList, templateId, null);
			}
