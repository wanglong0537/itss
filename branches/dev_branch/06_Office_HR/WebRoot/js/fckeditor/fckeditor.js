...
FCKeditor.prototype.CreateHtml = function() {
	...
	if (!this.CheckBrowser || this._IsCompatibleBrowser()) {
		c += '<input type="hidden" id="' + this.InstanceName + '" name="'
				+ this.InstanceName + '" value="'
				+ this._HTMLEncode(this.Value) + '" style="display:none" />';
		c += this._GetConfigHtml();
		c += this._GetIFrameHtml();
	} else {
		var b = this.Width.toString().indexOf("%") > 0 ? this.Width
				: this.Width + "px";
		var a = this.Height.toString().indexOf("%") > 0 ? this.Height
				: this.Height + "px";
		c += '<textarea name="' + this.InstanceName
				+ '" rows="4" cols="40" style="width:' + b + ";height:" + a;
		if (this.TabIndex) {
...
FCKeditor.prototype._GetConfigHtml = function() {
	...
	return '<input type="hidden" id="' + this.InstanceName
			+ '___Config" value="' + a + '" style="display:none" />';
};
FCKeditor.prototype._GetIFrameHtml = function() {
	...
	var b = '<iframe id="' + this.InstanceName + '___Frame" src="' + a
