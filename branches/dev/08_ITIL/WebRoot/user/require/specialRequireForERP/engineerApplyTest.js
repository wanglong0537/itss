...
PageTemplates = Ext.extend(Ext.Panel, {
	...
	saveAndSubmit : function() {
		if (!Ext.getCmp(panel1).form.isValid()) {
	...
	getFormpanel_erpSR_it_input : function() {
		...
		var biddata = "";
		if (this.dataId != '0') {
			data = da.getPanelElementsForEdit("esr_engineerApplyTest",
	...
	getFormpanel_srInfo_input2 : function() {
		...
		if (this.dataId != '0') {
			data = da.getPanelElementsForEdit("esr_engineerApplyTest",
					panel1, this.dataId);// 这是要随时变得
		} else {
			data = da.getPanelElementsForAdd(panel1);
		}
	...
	getGridpanel_SRTestReport : function() {
		...
		var getGridButtons = buttonUtil.getButtonForPanel("panel_SRTestReport",
				this);
		getGridButtons.push(new Ext.Toolbar.TextItem("<font color='red' style='padding-left:18px;font-size:12px;'>保存列表信息请单击暂存</font>"));
...
//			loadMask : true,
	...
	initComponent : function() {
		...
		var financeType = sra.selectFinanceType(this.dataId);
		if(financeType=='2'){
					panel1 = "panel_srInfo_input6";
		}else{
					panel1 = "panel_srInfo_input2" ;
		}		
		...
		var items = new Array();
		var temp = new Array();
		this.model = "esr_engineerApplyTest";
		var buttonUtil = new ButtonUtil();
		this.mybuttons = buttonUtil.getButtonForModel(
				"esr_engineerApplyTest", this);
