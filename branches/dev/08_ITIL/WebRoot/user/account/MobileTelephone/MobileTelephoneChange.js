				...
				
			{
				...
				listeners : {
				   ...
				   'select' : function(combo, record, index) {
							...
							Ext.Ajax.request({
								url : webContext
										+ '/accountAction_getUserMobileTelephone.action',
								params : {
									userInfo : userInfo,
									accountType:'手机'
                               },
							    ...
							    if (!r.success) {
									Ext.MessageBox.alert("提示","申请人还没有申请过手机补贴申请！");
					           var userInfo = Ext
...
//				hideTrigger:true,
				...
				}),
		  {
			...
			buttons : [{
				...
				iconCls : 'save',
				handler : function() {
					if(!Ext.getCmp('panel_MobileTelephoneApply_Input').form.isValid()){
					...
					Ext.getCmp('itil_ac_MobileTelephoneApply$startDate').setValue(Ext.getCmp('itil_ac_MobileTelephoneApply$startMonth').getValue());
					...
					Ext.Ajax.request({
						url : webContext
								+ '/accountAction_saveMobileTelephoneChangeDraft.action',
						params : {
							info : info,
						...
						success : function(response, options) {
							...
							Ext.getCmp('panel_MobileTelephoneApply_Input').load({
								url : webContext
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
							 ...
							 if(responseArray.success){	
							 	Ext.MessageBox.alert("提示","申请人已存在审批中的手机变更申请,不能够再提该申请！",function(btn){
					...
					Ext.Ajax.request({
						...
						success : function(response, options) {
				           ...
				           var department = Ext
									...
									.getValue();
							Ext.Ajax.request({
							//modify by liuying for 改变启动工作流方法 2010/1/7
								url : webContext
		...
		Ext.Ajax.request({
						...
						success : function(response, options) {
							...
							var curId = responseArray.id;
						    Ext.getCmp('panel_MobileTelephoneApply_Input').form.load({
				                url : webContext
