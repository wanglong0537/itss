PageTemplates = Ext.extend(Ext.Panel, {
	...
	getFormpanel_PersonAccountApply_Input : function() {
		...
		this.formpanel_PersonAccountApply_Input = new Ext.form.FormPanel({
			...
			{
				html : '<font color=red>*</font> 注册服务类型:',
				cls : 'common-text',
				style : 'width:135;text-align:right'
			}, new Ext.form.ComboBox({forceSelection:true,
				xtype : 'combo',
				hiddenName : 'itil_ac_PersonFormalAccount$registerServiceType',
				id : 'itil_ac_PersonFormalAccount$registerServiceTypeCombo',
				width : 200,
				style : '',
				fieldLabel : '注册服务类型',
				colspan : 0,
				...
				store : new Ext.data.JsonStore({
					url : webContext
							+ '/extjs/comboDataAction?clazz=com.digitalchina.itil.config.extlist.entity.ErpServiceType',
					fields : ['id', 'name'],
					listeners : {
						beforeload : function(store, opt) {
							if (opt.params['itil_ac_PersonFormalAccount$registerServiceType'] == undefined) {
								opt.params['name'] = Ext
										.getCmp('itil_ac_PersonFormalAccount$registerServiceTypeCombo').defaultParam;
							}
						}
					},
					totalProperty : 'rowCount',
				...
				listeners : {
					...
					'select' : function(combo, record, index) {
					var mailvalue=Ext.getCmp('itil_ac_PersonFormalAccount$registerServiceTypeCombo').getRawValue();
					if(mailvalue=='无'||mailvalue==''){
					Ext.MessageBox.alert("提示","请您选择具体的注册服务类型,谢谢您的合作！",function(btn){
					Ext.getCmp('itil_ac_PersonFormalAccount$registerServiceTypeCombo').setValue("");
					});
					}
					}
				},
				initComponent : function() {
					this.store.load({
						params : {
							id : Ext
									.getCmp('itil_ac_PersonFormalAccount$registerServiceTypeCombo')
									.getValue(),
							start : 0
						},
						callback : function(r, options, success) {
				...
				colspan : 0,
				rowspan : 0,
				id : 'itil_ac_PersonFormalAccount$dutyName',
				name : 'itil_ac_PersonFormalAccount$dutyName',
				style : '',
				width : 200,
				value : '',
				...
				html : '<font color=red>*</font> 事物代码或路径:',
				cls : 'common-text',
				style : 'width:135;text-align:right'
			}, new Ext.form.TextField({
				fieldLabel : '事物代码或路径',
				...
				colspan : 0,
				rowspan : 0,
				id : 'itil_ac_PersonFormalAccount$thingCode',
				name : 'itil_ac_PersonFormalAccount$thingCode',
				style : '',
				width : 200,
				value : '',
				...
				colspan : 0,
				rowspan : 0,
				emptyText:'（公司代码，销售组织，工厂等）',
				id : 'itil_ac_PersonFormalAccount$controlScope',
				name : 'itil_ac_PersonFormalAccount$controlScope',
				style : '',
				width : 200,
				value : '',
			...
			{
				html : '<font color=red>*</font> ERP服务权限补充描述',
				cls : 'common-text',
				colspan : 2,
				rowspan : 0,
			 ...
			 {
				html : ' 如果需要申请的权限与现有的某一个ERP帐号权限一致请填写该帐号:<br> （例如：我需要申请与现有ERP帐号it-manage一致的权限）',
				cls : 'common-text',
				colspan : 2,
				rowspan : 0,
				style : 'border:1px dotted #b0acac;width:700;text-align:left;margin:0px 0px 0px 25px'
		...
		buttons : [{
				...
				handler : function() {
					...
					//add by liuying at 20100125 for 增加验证 必填项为空格的验证 start
					var dutyName=Ext.getCmp('itil_ac_PersonFormalAccount$dutyName').getValue();
					var thingCode=Ext.getCmp('itil_ac_PersonFormalAccount$thingCode').getValue();
					var controlScope=Ext.getCmp('itil_ac_PersonFormalAccount$controlScope').getValue();
					var registerServiceRightDesc=Ext.getCmp('itil_ac_PersonFormalAccount$registerServiceRightDesc').getValue();
					if(dutyName.trim()==""||dutyName==null){
					Ext.MessageBox.alert("提示","岗位名称为必填项,请填写完整。谢谢您合作！");	
					return false;
					}
					if(thingCode.trim()==""||thingCode==null){
					Ext.MessageBox.alert("提示"," 事物代码或路径为必填项,请填写完整。谢谢您合作！");
					return false;
					}
					if(controlScope.trim()==""||controlScope==null){
					Ext.MessageBox.alert("提示","控制范围为必填项,请填写完整。谢谢您合作！");
					return false;
					}
					if(registerServiceRightDesc.trim()==""||registerServiceRightDesc==null){
					Ext.MessageBox.alert("提示","服务权限补充描述为必填项,请填写完整。谢谢您合作！");
					return false;
					}
					//add by liuying at 20100125 for 增加验证 必填项为空格的验证 end
					Ext.getCmp('itil_ac_PersonFormalAccount$registerServiceTypeCombo')
											.getValue()
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
							 ...
							 if(responseArray.success){	
							 	Ext.MessageBox.alert("提示","申请人已存在审批中的ERP帐号申请,不能够再提该申请！",function(btn){
			...
			Ext.Ajax.request({
			          ...
			          Ext.Ajax.request({
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON.decode(response.responseText);
						    if(responseArray.success){
						    	Ext.MessageBox.alert("提示","申请人已存在可使用的ERP帐号,请确认后再提申请！",function(btn){
		                      	window.history.back(-1);
		                      	});
		                      }else{
							...
							var curId = responseArray.id;
							 Ext.getCmp('panel_PersonAccountApply_Input').form.load({
				                url : webContext + '/accountAction_initPersonAccountApplyData.action',
				                params : {
