PageTemplates = Ext.extend(Ext.Panel, {
	...
	getTabpanel : function(tab, tabTitle) {
		this.tabPanel = new Ext.TabPanel({
			...
			items : tab,
			tbar : [new Ext.Toolbar.TextItem('<font color=red>提示：</font><font color=blue>1.页面中带红色<font color=red>*</font>号的必填项，请在填写完整后再提交申请!&nbsp<font color=red>2.审批人必须从下拉列表中选择</font>')]
...
//				mapping : 'AccountApplyMainTable$attachment'
			...
			{
				html : '<font color=red>*</font> ERP服务权限变更补充描述:',
				cls : 'common-text',
				colspan : 4,
				rowspan : 0,
					...
					if (attachmentFlag == '') {
						...
						ud
								.getUpDownLoadFileSu(attachmentFlag, '7735',
										'com.digitalchina.info.appframework.metadata.entity.SystemFile','NotesIDFile010attachment');
					} else {
						var ud = new UpDownLoadFile();
						ud
								.getUpDownLoadFileSu(attachmentFlag, '7735',
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
							 ...
							 if(responseArray.success){	
							 	Ext.MessageBox.alert("提示","申请人已存在审批中的ERP帐号权限变更申请,不能够再提该申请！",function(btn){
		                      	window.history.back(-1);
		                      	});
			}else{
			Ext.Ajax.request({
						...
						success : function(response, options) {
							...
							.initComponent();
							     Ext.getCmp("itil_ac_PersonFormalAccount$id").setValue("");
							     Ext.getCmp("itil_ac_PersonFormalAccount$dutyName").setValue("");
							     Ext.getCmp("itil_ac_PersonFormalAccount$thingCode").setValue("");
							     Ext.getCmp("itil_ac_PersonFormalAccount$controlScope").setValue("");
							     Ext.getCmp("itil_ac_PersonFormalAccount$registerServiceRightDesc").setValue("");
							      Ext.getCmp("itil_ac_PersonFormalAccount$sameRightAccount").setValue("");
						
						
							
							   },
				             failure : function(response, options) {
							   Ext.MessageBox.alert("提示","申请人不存在可变更的ERP帐号,请确认后在提申请！",function(btn){
		                      	window.history.back(-1);
		                      	});
				              }
		                      })
				...
				success : function(action, form) {
					 Ext.getCmp("AccountApplyMainTable$applyUserCombo")
							...
							.initComponent();
							 Ext.getCmp("sUserInfos$personnelScopeCombo")
							.initComponent();
					var desc=Ext.getCmp("itil_ac_PersonFormalAccount$registerServiceRightDesc").getValue();
					if(desc.indexOf('\\r\\n')>0){
						var str1=desc.split('\\r\\n');
						var message='';
						for(var i=0;i<str1.length;i++){
						message=message+str1[i]+'\r\n';
						}
						Ext.getCmp("itil_ac_PersonFormalAccount$registerServiceRightDesc").setValue(message);
					}
				}
			});
		}
