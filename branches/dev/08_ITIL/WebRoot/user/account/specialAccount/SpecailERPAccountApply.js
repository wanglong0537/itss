		...
		this.formpanel_SpecailAccountApply_Input = new Ext.form.FormPanel({
			...
			}),
			{
				...
				listeners : {
					'beforequery' : function(queryEvent) {if(this.readOnly==true){return false}
						...
						'select' : function(combo, record, index) {
					var mailvalue=Ext.getCmp('itil_ac_SpecialAccount$registerServiceTypeCombo').getRawValue();
					if(mailvalue=='无'||mailvalue==''){
					Ext.MessageBox.alert("提示","请您选择具体的ERP注册服务类型,谢谢您的合作！",function(btn){
					Ext.getCmp('itil_ac_SpecialAccount$registerServiceTypeCombo').setValue("");
					});
					}
					}
				},
				...
				{
				html : '<font color=red>*</font>岗位职责描述:',
				cls : 'common-text',
				style : 'width:135;text-align:right'
			}, new Ext.form.TextArea({
				fieldLabel : '岗位职责描述',
			...
			buttons : [{
				...
				handler : function() {
					...
					Ext.Ajax.request({
						...
						failure : function(response, options) {
							Ext.MessageBox.alert("保存失败,请检查页面数据是否选择正确!");
							Ext.getCmp("save").enable();
					        Ext.getCmp("submit").enable();
					        Ext.getCmp("back").enable();
						}
				...
				handler : function() {
					...
					Ext.getCmp("back").disable();
					var info = Ext.encode(getFormParam('panel_SpecailAccountApply_Input'));
					//modify by liuying at 20100114 for 特殊ERP账号提交申请限制 start
					Ext.Ajax.request({
						url : webContext
						...
						if(responseArray.success){	
								Ext.MessageBox.alert("提示","申请人已存在可使用的特殊ERP帐号,请确认后再提交申请！",function(btn){
			    	       ...
			    	       Ext.Ajax.request({
								...
								success : function(response, options) {
									var responseArray = Ext.util.JSON
											.decode(response.responseText);
									 if(responseArray.success){	
									 	Ext.MessageBox.alert("提示","申请人已存在审批中的特殊ERP帐号申请,不能够再提该申请！",function(btn){
				                      	window.history.back(-1);
				                      	});
									 }else{
							 
									 Ext.Ajax.request({
											url : webContext + '/accountAction_saveSpecailAccountApplyDraft.action',
											params : {
													info : info,
													serviceItemId : curscid,
													processInfoId:processInfoId,
											...
											success : function(response, options) {
												...
												Ext.Ajax.request({
													url : webContext + '/accountWorkflow_apply2.action',
													params : {	
														dataId : curId,
														userInfo:userInfo,
												...
												success : function(response, options) {
												    ...
												    Ext.Msg.alert("提示", "提交申请成功", function() {
												    	window.location = webContext
												    	+ "/requireSIAction_toRequireInfoByServiceItemId2.action?id="+ curscid;
												    	});
												},
												failure : function(response, options) {
										...
										failure : function(response, options) {					
											Ext.MessageBox.alert("提示", "提交申请失败,请检页面数据是否选择正确!");
											Ext.getCmp("save").enable();
									        Ext.getCmp("submit").enable();
									        Ext.getCmp("back").enable();
										}
				...
				//modify by liuying at 20100114 for 特殊ERP账号提交申请限制 end
				}
			}, {
				text : '返回',
				iconCls : 'back',
