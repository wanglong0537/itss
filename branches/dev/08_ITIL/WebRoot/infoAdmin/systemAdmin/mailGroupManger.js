PagePanel = Ext.extend(Ext.Panel, {
	...
	search : function() {//查询方法
		var tempApp = Ext.getCmp('groupNewName').getValue();
		var param = {
					groupNewName : tempApp,
					'start' : 0
		};
		/*给分页发的时候这样的参数格式*/
	...
	create : function() {
		var clazz = "com.digitalchina.itil.config.extlist.entity.MailGroup";//指定实体类型
...
//						
	...
	initComponent : function() {//初始化面板
		...
		this.store = new Ext.data.JsonStore({//设置下方列表的格式
			url : webContext + "/accountSystemAdminAction_listMailGroupManger.action",
			fields: ['id','groupNewName','groupManger'],
			root:'data',
		...
		this.grid = new Ext.grid.GridPanel({//设定grid
			...
			columns : [sm, {header : '编号',dataIndex : 'id',align : 'left',sortable : true,hidden : true}, 
						{header : '邮件群组名称',dataIndex : 'groupNewName',align : 'left',sortable : true,hidden : false,width:200},
						{header : '邮件群组管理员',dataIndex : 'groupManger',align : 'left',sortable : true,hidden : false,width:250}
