RoleForm = Ext.extend(Ext.Window, {
	formPanel : null,
	...
	initComponents : function() {
		...
		if(this.roleDN != null && this.roleDN != "undefined") {
			Ext.getCmp("cn").readOnly = true;
			this.formPanel.getForm().load({
				deferredRender : false,
				url : webContext + "/role?methodCall=getDetailByRoleDN&roleDN=" + this.roleDN,
				waitMsg : "正在载入数据……",
	...
	save : function(a, b) {
		if(b.getForm().isValid()) {
			if(a.isModify) {
				//修改
				b.getForm().submit({
					url : webContext + "/role?methodCall=modify",
					method : "post",
					waitMsg : "正在提交数据……",
				...
				//新增
				b.getForm().submit({
					url : webContext + "/role?methodCall=add",
					method : "post",
					waitMsg : "正在提交数据……",
					success : function(form, action) {
						a.close();
