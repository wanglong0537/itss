PageTemplates = Ext.extend(Ext.Panel, {
	...
	getFormpanel_PersonAccountApply_Input : function() {
		...
		this.formpanel_PersonAccountApply_Input = new Ext.form.FormPanel({
			...
			{
				html : '<font color=red>*</font>EB变更原因:',
				cls : 'common-text',
				style : 'width:135;text-align:right'
							...
							Ext.Ajax.request({
								...
								params : {
									...
									bzparam : "{dataId :'" + curId
											...
											+ "',accountName:'" + curName
											+ "',applyType: 'acproject'," 
											+ "applyTypeName: 'E-Bridge帐号权限变更申请',"
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
							 ...
							 if(responseArray.success){	
							 	Ext.MessageBox.alert("提示","申请人已存在审批中的EB帐号权限变更申请,不能够再提该申请！",function(btn){
		                      	window.history.back(-1);
		                      	});
							 }else{
			  Ext.Ajax.request({
					url : webContext + '/accountAction_getUserPersonAccount.action',//获得个人正式帐号
						...
						success : function(response, options) {
							...
							Ext.getCmp('itil_ac_PersonFormalAccount$telephone').setValue("");
							...
							Ext.getCmp('itil_ac_PersonFormalAccount$id').setValue("");
				             ...
				             failure : function(response, options) {
							   Ext.MessageBox.alert("提示","申请人不存在可变更的E-Bridge帐号,请确认后在提申请！",function(btn){
		                      	window.history.back(-1);
		                      	});
				              }
		                      })
