PageTemplates = Ext.extend(Ext.Panel, {
	...
	getFormpanel_HRSAccountApply_Input : function() {
		...
		this.formpanel_HRSAccountApply_Input = new Ext.form.FormPanel({
			...
			}),
			{
				...
				listeners : {
					...
					var mailvalue=Ext.getCmp('HRSAccountApply$confirmUserCombo').getRawValue();
					if(mailvalue=='无'){
					Ext.MessageBox.alert("提示","请您选择具体的部门经理,谢谢您的合作！",function(btn){
					Ext.getCmp('HRSAccountApply$confirmUserCombo').setValue("");
					});
					}
					}
				},
		...
		buttons : [{
				...
				iconCls : 'submit',
				handler : function() {
					if(!Ext.getCmp('panel_HRSAccountApply_Input').form.isValid()){
			...
			failure : function(response, options) {
				Ext.MessageBox.alert("提交审批失败,请检查部门经理是否填写真确!");
			}
		}, this);
				}
			},
