	...
	getFormpanel_MailGroup : function() {
		...
		this.formpanel_MailGroup = new Ext.form.FormPanel({
			...
			items : [{
				...
				items : [{
					html : '<font color=red>*</font> 群组原名称:',
					cls : 'common-text',
					colspan : 4,
					rowspan : 0,
...
//						}
						...
						if (mangerGroup == "") {
							...
							Ext.getCmp('MailGroup$groupName').setValue(mangerGroup+';'+temp);//modify by liuying at 20100129 
 //                         Ext.Msg.alert("提示", "一次申请只能变更一个群组帐号");
						}
				...
				{
					html : '<font color=red>*</font>群组新名称(名称中不要出现分号)',
					cls : 'common-text',
					colspan : 4,
					rowspan : 0,
					style : 'border:1px dotted #b0acac;width:600;text-align:left;margin:0px 0px 0px 75px'
					...
					xtype : 'textarea',
					id : 'MailGroup$groupNewName',
					colspan : 4,
					rowspan : 0,
					name : 'MailGroup$groupNewName',
					width : 603,
					readOnly:true, //modify by liuying at 20100129
					style : 'margin:0px 0px 0px 75p',
					value : '',
					allowBlank : false,
					validator : '',
...
//add by liuying at 20100129 start
 {
					html : ' <font color=red>新群组名:</font>',
					cls : 'common-text',
					style : 'margin:0px 0px 0px 75p'
				}, new Ext.form.TextField({
				...
				new Ext.Button({
					...
					handler : function() {
						...
						var newName = Ext.getCmp("groupNewName").getValue();
						if (newName == ""||newName==null) {
							Ext.Msg.alert("提示", "请输入新群组名后再添加");
							return false;
						}
						if(newName.indexOf(';')>=0||newName.indexOf('；')>=0){
							Ext.Msg.alert("提示","新群组名中不能包含';'字符!");
							return false;
						}
						
						var mangerGroup = Ext.getCmp('MailGroup$groupNewName')
								.getValue();
//						var number = Ext.getCmp('MailGroup$groupName')
//								.getValue().split(';');
//						if (number.length >= 10) {
...
//						}
						...
						if (mangerGroup.indexOf(newName) >= 0) {
							Ext.Msg.alert("提示", "该群组名已经添加，不能重复添加");
							return false;
						}
						if (mangerGroup == "") {
							Ext.getCmp('MailGroup$groupNewName').setValue(newName);

						} else {
							Ext.getCmp('MailGroup$groupNewName').setValue(mangerGroup+';'+newName);
 //                         Ext.Msg.alert("提示", "一次申请只能变更一个群组帐号");
						}
						Ext.getCmp("groupNewName").setValue("");
					}
				}), new Ext.Button({
					text : '重置',
					pressed : true,
					...
					scope : this,
					handler : function() {
						Ext.getCmp('MailGroup$groupNewName').setValue("");
					}
				}),
//add by liuying at 20100129 end
				{
			...
			buttons : [{
				...
				handler : function() {
					...
					var groupName = Ext.getCmp('MailGroup$groupName')
							.getValue();
					var groupNewName = Ext.getCmp('MailGroup$groupNewName')
							.getValue();
					var num1 = groupName.split(';').length;
					var num2 = groupNewName.split(';').length;
					if(num1!=num2){
						Ext.Msg.alert("提示","原群组名称数目和新群组名称数目不匹配！请您核对名称数目以及新旧名称对应情况！");
						return false;
					}
					//add by liuying at 20100129 end
					...
					Ext.Ajax.request({
						url : webContext
								+ '/accountAction_saveMailGroupNameChangeDraft.action',//modify by liuying at 20100129
						params : {
							info : info,
							...
							groupName : groupName,
							groupNewName:groupNewName,//modify by liuying at 20100129
				...
				handler : function() {
					...
					Ext.Ajax.request({
						...
						params : {
							...
							processType : processType,
							groupName : groupName,
							groupNewName:groupNewName,//add by liuying at 20100129
							panelName : 'panel_MailGroup'
						},
						success : function(response, options) {
							var responseArray = Ext.util.JSON
