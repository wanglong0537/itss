PageTemplates = Ext.extend(Ext.Panel, {
	...
	initComponent : function() {
		...
		this.store2 = new Ext.data.JsonStore({
			url : webContext + "/accountAction_listSpecailAllAccount.action",
			fields : ['id', 'accountType', 'accountName', 'applyReason',
					'ifHold', 'accountNewOwner'],
			root : 'data',
			totalProperty : 'rowCount',
			sortInfo : {
				field : "id",
			...
			baseParams : {
				...
				'load' : function(store, records, options) {
					if(records.length==0){
						Ext.getCmp('tab').hideTabStripItem('specailAccountInfo');
					}else{
						Ext.getCmp('tab').unhideTabStripItem('specailAccountInfo');
					}
					}
				}
		});
