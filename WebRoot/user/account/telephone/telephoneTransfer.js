		...
		this.formpanel_PersonAccountApply_Input = new Ext.form.FormPanel({
			...
			buttons : [{
				...
				handler : function() {
				    ...
				    if(telephoneNumber==null||telephoneNumber.trim().length<7){
				    	Ext.MessageBox.alert("提示","请填写完整的要变更的电话号码,谢谢您的合作!");
						return false;
				    }
					...
					Ext.Ajax.request({
						url : webContext
								+ '/accountAction_saveTelephoneTransferDraft.action',
						params : {
							info : info,
				...
				handler : function() {
			           ...
			           if(Ext.getDom('noItcode').checked){
						...
						Ext.Ajax.request({
							...
							failure : function(response, options) {
								Ext.MessageBox.alert("提示","提交申请失败,请检查变更的电话号码是否填写正确!");
								Ext.getCmp("save").enable();
								Ext.getCmp("submit").enable();
								Ext.getCmp("back").enable();
							}
