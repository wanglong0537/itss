PageTemplates = Ext.extend(Ext.Panel, {
	...
	getFormpanel_SpecailAccountApply_Input : function() {
		...
		this.formpanel_SpecailAccountApply_Input = new Ext.form.FormPanel({
								...
								success : function(response, options) {
									...
									if (r.success) {
										if(r.num==1){
											...
											Ext.Msg.show({
												   title:'提示',
												   msg: '此座机号码为多人共用的号码，是够继续变更这个号码的所有者？',
			...
			{
				html : '<font color=red>*</font>座机新所有者:',
				cls : 'common-text',
				style : 'width:135;text-align:right'
			}, new Ext.form.ComboBox({forceSelection:true,
					xtype : 'combo',
...
//															if(r.userType==7){
//																Ext.MessageBox.alert("系统提示","该属于派遣员工，尚没有权限做此申请！",function(btn){
//																Ext.getCmp('AccountApplyMainTable$applyUserCombo').setValue("");
//																return false;
//															})}
//																else if(r.userType==9){
//																Ext.MessageBox.alert("系统提示","该属于派遣(厂商)员工，尚没有权限做此申请！",function(btn){
//																Ext.getCmp('AccountApplyMainTable$applyUserCombo').setValue("");
//																return false;
//															})}
//																if(r.userType==4){
//																Ext.MessageBox.alert("系统提示","该属于临时员工，尚没有权限做此申请！",function(btn){
//																Ext.getCmp('AccountApplyMainTable$applyUserCombo').setValue("");
//																return false;
//															})}
//																else if(r.userType==2){
//																Ext.MessageBox.alert("系统提示","该属于特聘员工，尚没有权限做此申请！",function(btn){
//																Ext.getCmp('AccountApplyMainTable$applyUserCombo').setValue("");
//																return false;
//															})}
//																else if(r.userType==8){
//																Ext.MessageBox.alert("系统提示","该属于特殊员工，尚没有权限做此申请！",function(btn){
//																Ext.getCmp('AccountApplyMainTable$applyUserCombo').setValue("");
//																return false;
//															})}
			 ...
			 new Ext.form.Hidden({
				...
				handler : function() {
					...
					if(telNumber==''||telNumber==null){
					 Ext.MessageBox.alert("提示","座机号码必须从下拉列表中选择,谢谢您的合作!");
		                return false;
					}
		           ...
		           Ext.getCmp("save").disable();
					...
					Ext.Ajax.request({
								...
								success : function(response, options) {
									...
									Ext.MessageBox.alert("提示","该座机号码已存在审批中的所有者变更申请,不能够再提该申请！",function(btn){
								...
								failure : function(response, options) {
									Ext.MessageBox.alert("提示", "提交申请失败!");
									Ext.getCmp("save").enable();
					                Ext.getCmp("submit").enable();
					                Ext.getCmp("back").enable();
								}
			...
			,
				...
				handler : function() {
					...
					Ext.MessageBox.alert("提示","工作地点没有自动带出，请联系管理员！");	
					return false;
					}
					if(!Ext.getCmp('panel_PersonAccountApply_Input').form.isValid()){
					Ext.MessageBox.alert("提示","页面中带红色波浪线的为必填项,请填写完整。谢谢您合作！");	
