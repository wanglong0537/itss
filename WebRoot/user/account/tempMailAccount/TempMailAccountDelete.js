		...
		this.formpanel_SpecailAccountApply_Input = new Ext.form.FormPanel({
										...
										Ext.Ajax.request({
											...
											success : function(response,
												...
												if (!r.success) {
													...
													Ext.getCmp('account').store
															.removeAll();
													Ext.getCmp('account').store
															.load({
																params : param
															})
													Ext.getCmp('account').view
															.refresh();
			...
			buttons : [{
				...
				handler : function() {
					...
					var record = Ext.getCmp('account').getSelectionModel()
							.getSelected();
					var records = Ext.getCmp('account').getSelectionModel()
							.getSelections();
					...
					Ext.Ajax.request({
						...
						success : function(response, options) {
							...
							for (var i = 0; i < records.length; i++) {
								...
								accountTypes[i] = records[i].get('accTypeEN');//modify by liuying at 20100127
								Ext.Ajax.request({
									url : webContext
									...
									params : {
										...
										accountType : accountTypes[i]//modify by liuying at 20100127
									},
									timeout : 100000,
									success : function(response) {
										var r = Ext
				...
				handler : function() {
					...
					if (records.length < 1) {
						Ext.MessageBox.alert('警告', '你还不存在可删除的临时人员邮件帐号!');
						return false;
					}
					...
					Ext.getCmp("submit").disable();
					Ext.getCmp("back").disable();
					var isVPN=false;//判断删除的帐号中是否包括临时人员远程接入帐号，且帐号类型是硬令牌的
					for (var i = 0; i < records.length; i++) {
						if(records[i].get('accTypeEN')=='TempVPNAccount'&&records[i].get('vpnType')!='1'){
							isVPN=true;
						}
					}
					if(isVPN){
						Ext.MessageBox.alert("提示",
							"若要删除临时人员远程接入帐号,请将令牌卡归还到指定的平台工程师处,谢谢您的合作!",
							function(btn) {
								if (btn == 'ok') {
									var curDataId = this.dataId;
									var info = Ext.encode(getFormParam('panel_SpecailAccountApply_Input'));
											...
											Ext.Ajax.request({
												url : webContext
														//+ '/accountWorkflow_apply.action'
														  + '/accountWorkflow_applyTempMailDelete.action',//modify by liuying at 20100127 for 改变流程
												params : {
													dataId : curId,
													userInfo : userInfo,
	...
	initComponent : function() {
		...
		if (this.dataId == "0" || this.dataId == "null") {
			this.store1 = new Ext.data.JsonStore({
				...
				fields : ['id', 'accountName', 'applyReason', 'accountNowUser',
						'accountManger','accType','accTypeEN','vpnType'],
				root : 'data',
				totalProperty : 'rowCount',
				sortInfo : {
					field : "id",
			...
			this.store1 = new Ext.data.JsonStore({
				...
				totalProperty : 'rowCount',
				sortInfo : {
					field : "accType",//modify by liuying at 20100127
					direction : "DESC"
				},
				baseParams : {
					dataId : this.dataId
		...
		this.grid2 = new Ext.grid.EditorGridPanel({
			...
			columns : [sm, {
				...
				header : '<font color=red>删除原因</font>',
				...
				header : '<font color=red>帐号所有者ITcode</font>',
				dataIndex : 'accountNowUser',
				align : 'center',
				sortable : true,
				hidden : false
			}, {
				header : '<font color=red>帐号管理员</font>',
				dataIndex : 'accountManger',
				align : 'center',
				sortable : true,
				hidden : false
			},
			//add by liuying at 20100121 for 页面调整start
			{
			...
			//add by liuying at 20100121 for 页面调整end
			],
			sm : sm,
			...
			width : 665,
			autoHeight : true,
			tbar : [new Ext.Toolbar.TextItem('<font color=red>提示：</font><font color=blue>双击删除原因单元格可以进行编辑</font>')],
			bbar : [new Ext.Toolbar.TextItem('<font color=blue>如果用户需要删除多个帐号,可以将帐号前面的复选框选中!</font>')]

		});

		this.store1.load();
		//add by liuying at 20100127 for 页面调整 start 

		sm.addListener('rowselect',function(selModel,num,record){
			var accName=record.get('accountName');
			var records = Ext.getCmp('account').getStore();
			if(record.get('accType')=='临时人员邮件/域帐号'){
							for (var i = 0; i < records.getTotalCount(); i++) {
								name = records.getAt(i).get('accountName');
									if(accName==name){
										if(records.getAt(i).get('accType')!='临时人员邮件/域帐号'){
									...
									if(accName==name){
										if(records.getAt(i).get('accType')=='临时人员邮件/域帐号'){
											if(!selModel.isSelected(i)){
												Ext.Msg.show({
												   title:'提示',
												   msg: "请选择帐号类型为‘临时人员邮件/域帐号’的帐号!",
												   buttons: Ext.Msg.OK,
												   icon: Ext.MessageBox.INFO
												});
												selModel.deselectRow(num);
											}
										}
									}
								}
		...
		sm.addListener('rowdeselect',function(selModel,num,record){
				var accName=record.get('accountName');
				var records = Ext.getCmp('account').getStore();
				if(record.get('accType')=='临时人员邮件/域帐号'){
							for (var i = 0; i < records.getTotalCount(); i++) {
									...
									if(accName==name){
										if(records.getAt(i).get('accType')=='临时人员邮件/域帐号'){
											if(selModel.isSelected(i)){
												Ext.Msg.show({
												   title:'提示',
												   msg: "删除‘临时人员邮件/域帐号’时必须删除同名的其他临时帐号!",
												   buttons: Ext.Msg.OK,
												   icon: Ext.MessageBox.INFO
												});
												selModel.selectRow(num,true);
											}
										}
									}
								}
		...
		//add by liuying at 20100127 for 页面调整 end 
