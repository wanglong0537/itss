				...
				items : [{
					...
					listeners : {
						...
						'select' : function(combo, record, index) {
							var userInfo = Ext
									.getCmp('itil_ac_SpecialAccount$accountOldUserCombo')
									.getValue();
							Ext.Ajax.request({
								url : webContext
								...
								success : function(response, options) {
									...
									Ext
											.getCmp('itil_ac_SpecialAccount$telephone')
					...
					listeners : {
						...
						'select' : function(combo, record, index) {
							var userInfo = Ext
									...
									.getValue();
						    var old=Ext.getCmp("itil_ac_SpecialAccount$accountOldUserCombo").getValue();
						    if(userInfo==old){
			...
			buttons : [{
				...
				handler : function() {
					...
					if(applyUser==''||applyUser==null){
					 Ext.MessageBox.alert("提示","帐号新所有者选择错误,请重新选择！");
		                return false;
					}
					if(OldUser==''||OldUser==null){
					 Ext.MessageBox.alert("提示","帐号原所有者选择错误,请重新选择！");
		                return false;
					}
				...
				handler : function() {
					...
					if(OldUser==applyUser){
					 Ext.MessageBox.alert("提示","帐号原所有者不能和新所有者相同,请确认后再保存！");
		                return false;
					}
					if(applyUser==confirmUser){
					 Ext.MessageBox.alert("提示","帐号新所有者不能和审批人相同,请确认后再保存！");
							...
							Ext.Ajax.request({
								...
								params : {
									...
									bzparam : "{dataId :'"
											...
											+ curName
											+ "',applyType: 'acproject',"
											+ "applyTypeName: '临时人员WWW帐号所有者变更申请',"
	...
	initComponent : function() {
		...
		this.model = "ac_DeptSMailOwnerChange";
			...
			this.store1 = new Ext.data.JsonStore({
				url : webContext
						+ "/accountAction_listSAOwnerChangeAccount.action",
       ...
       this.store1 = new Ext.data.JsonStore({
				url : webContext
						+ "/accountAction_listSAOwnerUseerChoose.action",
				fields : ['id', 'accountName', 'accountNowUser', 'applyReason'],
				root : 'data',
				totalProperty : 'rowCount',
				sortInfo : {
		...
		this.grid2 = new Ext.grid.EditorGridPanel({
			...
			tbar : [new Ext.Toolbar.TextItem('<font color=red>提示：</font><font color=blue>双击变更原因单元格可以进行编辑</font>')],
                       bbar : [new Ext.Toolbar.TextItem('<font color=red>若变更多个帐号,可以选中帐号前面的复选框</font>')],
			viewConfig : {
				forceFit : true
			},
			columns : [new Ext.grid.RowNumberer(), sm, {
				...
				header : '<font color=red>变更原因(双击单元格可以编辑)</font>',
