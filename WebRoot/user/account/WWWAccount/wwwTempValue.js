	...
	getFormpanel_PersonAccountApply_Input : function() {
		...
		this.formpanel_PersonAccountApply_Input = new Ext.form.FormPanel({
...

			{
				...
				items : [{
					html : '<font color=red>*</font>WWW临时变更原因:',
					cls : 'common-text',
					style : 'width:135;text-align:right'
					...
					html : '<font color=red>*</font>实际申请额度:',
					cls : 'common-text',
					style : 'width:135;text-align:right'
				}, new Ext.form.ComboBox({forceSelection:true,
					xtype : 'combo',
					...
					html : '<font color=red>*</font>超出部分(元):',
					cls : 'common-text',
					style : 'width:135;text-align:right'
				}, new Ext.form.NumberField({
					fieldLabel : '<font color=red>*</font>超出部分(元)',
					xtype : 'textfield',
					colspan : 0,
					rowspan : 0,
			...
			buttons : [{
				...
				handler : function() {
					...
					var bm=Ext.getCmp('itil_ac_PersonFormalAccount$beyondMoney').getValue();
					if(bm<0){
					Ext.MessageBox.alert("提示",
								"超出部分不能填写负值。谢谢您合作！");
						return false;
				...
				handler : function() {
					...
					Ext.Ajax.request({
						...
						success : function(response, options) {
							...
							Ext.Ajax.request({
								url : webContext
										+ '/accountWorkflow_applyWWWTempValue.action',
								params : {
									dataId : curId,
									userInfo : userInfo,
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
							 ...
							 if(responseArray.success){	
							 	Ext.MessageBox.alert("提示","申请人已存在审批中的WWW临时额度调整申请,不能够再提该申请！",function(btn){
		                      	window.history.back(-1);
		                      	});
							 }else{
			Ext.Ajax.request({
				...
				success : function(response, options) {
					var responseArray = Ext.util.JSON
							.decode(response.responseText);
					if (!responseArray.success) {
						Ext.MessageBox.alert("提示", "申请人不存在可调整的WWW帐号,请确认后再提申请！",
								function(btn) {
									window.history.back(-1);
								});
					} else {
