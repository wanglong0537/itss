										...
										success : function(response, options) {
											...
											if (!r.success) {
												...
												Ext
														...
														.setValue(r.employeeCode);
												Ext
														.getCmp('sUserInfos$sameMailDept')
														.setValue(r.name);
					...
					if(delegateApplyUser==confirmUser){
					 Ext.MessageBox.alert("提示","代申请人不能和审批人相同,请确认后再保存！");
		                return false;
								...
								var info = Ext
										.encode(getFormParam('panel_PersonAccountApply_Input'));
								Ext.Ajax.request({
									url : webContext
											+ '/accountAction_saveAccountDeleteDraft.action',
		...
		if (this.dataId == "0" || this.dataId == "null") {
			...
			Ext.Ajax.request({
				...
				success : function(response, options) {
					var r = Ext.decode(response.responseText);
					if (!r.success) {
						Ext.MessageBox.alert("提示", "您不存在可删除的MSN帐号！");
