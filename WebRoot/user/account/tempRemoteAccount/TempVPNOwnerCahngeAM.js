			...
			
		{
		 ...
		 buttons : [{
				...
				handler : function() {
					...
					Ext.Ajax.request({
						url : webContext+ '/accountAction_saveSAOwnerChangeInfo.action',
						params : {
							rightDesc : rightDesc,
							dataId:dataId
				...
				success : function(action, form) {
					 Ext.getCmp("AccountApplyMainTable$delegateApplyUserCombo")
						 ...
						 Ext.getCmp("AccountApplyMainTable$confirmUserCombo")
							.initComponent();
							Ext.getCmp('itil_ac_SpecialAccount$rightsDesc').setValue("");
				}
			});
	...
	initComponent : function() {
		...
		this.store1 = new Ext.data.JsonStore({
	                ...
	                this.grid2 = new Ext.grid.EditorGridPanel({
	                   id:'account',
	                   name:'account',
	                   style:'margin:0px 0px 0px 25px; border:1px solid gray;',
	                 
			           store : this.store1,
			           tbar : [new Ext.Toolbar.TextItem('<font color=red>提示：</font><font color=blue>用户要变更的帐号信息</font>')],
			...
			columns : [new Ext.grid.RowNumberer(), {header : '自动编号',dataIndex : 'id',align : 'left',sortable : true,hidden : true}, 
						...
						{header : '<font color=red>变更原因(双击单元格可以编辑)</font>',dataIndex : 'applyReason',align : 'center',sortable : true,hidden : false,editor:new Ext.grid.GridEditor(new Ext.form.TextField({allowBlank:true}))}
