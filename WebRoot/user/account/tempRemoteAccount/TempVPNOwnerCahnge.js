...
//			}),
			...
			,
				...
				handler : function() {
					...
					var records = Ext.getCmp('account').getStore().getRange(0,
					              ...
					              if (records.length <1) {
			                      Ext.MessageBox.alert('警告', '不存在可变更的帐号!');
			                      return false;
					     }
				   var record =  Ext.getCmp('account').getSelectionModel().getSelected();
		           var records =  Ext.getCmp('account').getSelectionModel().getSelections();
							...
							Ext.Ajax.request({
								...
								params : {
									...
									bzparam : "{dataId :'" + curId
											...
											+ "',accountName:'" + curName
											+ "',applyType: 'acproject'," 
											+ "applyTypeName: '临时人员远程接入帐号所有者变更申请',"
	...
	initComponent : function() {
		...
		//modify by liuying for 修改加载用户帐号信息时的错误 at 20100518 start
		if(this.dataId == "0" || this.dataId == "null"){
		...
		this.store1 = new Ext.data.JsonStore({
		 ...
		 //modify by liuying for 修改加载用户帐号信息时的错误 at 20100518 end 
