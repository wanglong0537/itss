	...
	getFormpanel_PersonAccountApply_Input : function() {
		...
		this.formpanel_PersonAccountApply_Input = new Ext.form.FormPanel({
...

			{
				...
				{
					html : '<font color=red>*</font> 注册服务权限描述：为了能准确反馈您需要的权限，准备了一个权限注册描述事例，请按照该方式填写：<br>(例如：我在基础网络事业部从事销售岗位，需要查询系统中采购信息，以及库存分配信息。控制范围：北京平台)',
					cls : 'common-text',
					colspan : 2,
					rowspan : 0,
					style : 'border:1px dotted #b0acac;width:620;text-align:left;margin:0px 0px 0px 65px'
			...
			Ext.Ajax.request({
				...
				success : function(response, options) {
					...
					if (responseArray.success) {
						Ext.MessageBox.alert("提示",
								"申请人已存在审批中的SCM帐号申请,不能够再提该申请！", function(btn) {
									window.history.back(-1);
								});
					} else {
						Ext.Ajax.request({
