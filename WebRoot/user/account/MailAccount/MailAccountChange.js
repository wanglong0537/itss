	...
	getFormpanel_PersonAccountApply_Input : function() {
		...
		this.formpanel_PersonAccountApply_Input = new Ext.form.FormPanel({
...

			{
				...
				items : [{
					...
					listeners : {
						...
						'select' : function(combo, record, index) {
							...
							if (mailvalue == '无') {
								Ext.MessageBox.alert("提示",
										"请您选择具体的邮箱容量,谢谢您的合作！", function(btn) {
											Ext
													.getCmp('itil_ac_PersonFormalAccount$mailValueCombo')
													.setValue("");
										});
				...
				{
					html : '<font color=red>*</font>邮箱额度变更原因:',
					cls : 'common-text',
					style : 'width:135;text-align:right'
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
							 ...
							 if(responseArray.success){	
							 	Ext.MessageBox.alert("提示","申请人已存在审批中的邮件额度变更申请,不能够再提该申请！",function(btn){
						...
						success : function(response, options) {
							...
							Ext.Ajax.request({
								...
								failure : function(response, options) {
									...
									Ext.MessageBox
											.alert("提示",
													"提交申请失败,请您检查审批人是否选择正确!<br>注:审批人选择方法:请在要选择的审批人选项上单击");
									Ext.getCmp("save").enable();
									Ext.getCmp("submit").enable();
									Ext.getCmp("back").enable();
								}
			...
			Ext.Ajax.request({
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
									.decode(response.responseText);
							 if(responseArray.success){	
							 	Ext.MessageBox.alert("提示","申请人已存在审批中的邮件额度变更帐号申请,不能够再提该申请！",function(btn){
		                      	window.history.back(-1);
		                      	});
							 }else{
			         Ext.Ajax.request({
				     ...
				     success : function(response, options) {
								...
								.load({
									...
									success : function(action, form) {
										...
										Ext
												...
												.setValue("");
												Ext.getCmp("sUserInfos$mailServerCombo")
										.initComponent();
									},
									failure : function(response, options) {
										Ext.MessageBox.alert("提示",
												"申请人不存在可变更的邮件帐号,请确认后在提申请！",
												function(btn) {
													window.history.back(-1);
												});
									}
						...
						Ext.MessageBox.alert("提示", "申请人不存在可变更的邮件帐号,请确认后在提申请！",
								function(btn) {
									window.history.back(-1);
								});
					}
