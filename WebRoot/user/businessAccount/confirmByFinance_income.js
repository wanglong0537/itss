					...
					beforeload : function(store, opt) {
						if (opt.params['relationUser'] == undefined) {
							opt.params['userName'] = Ext
									.getCmp('relationUserCombo').defaultParam;
						}
					}
				},
			...
			initComponent : function() {
				this.store.load({
					...
					callback : function(r, options, success) {
						Ext.getCmp('relationUserCombo').setValue(Ext
								.getCmp('relationUserCombo').getValue());
					}
				});
			}
		}),/* {
...
function getIncomeGridPanel() {
   ...
   this.store = new Ext.data.JsonStore({ 
	   ...
	   this.store = new Ext.data.JsonStore({ 
		...
		this.allIncomeGrid = new Ext.grid.GridPanel({
			...
			columns:[sm,
					...
					{header:'自动编号',dataIndex:'itil_Realncome$id',align : 'left',sortable : true,hidden:true},
					{header:'收款成本中心',dataIndex:'itil_Realncome$costCenter',align : 'left',sortable : true,hidden:false},
					{header:'实际收款钱数',dataIndex:'itil_Realncome$realIcomeMoney',align : 'left',sortable : true,hidden:false},
					{header:'实际收款时间',dataIndex:'itil_Realncome$realIcomeDate',align : 'left',sortable : true,hidden:false}],
	...
	var incomeGrid = new Ext.grid.EditorGridPanel({
		id : 'incomeGrid',
		store : this.store,
		sm : sm,
		columns:[sm,
			{header:'自动编号',dataIndex:'SRIncomePlan$id',align : 'left',sortable : true,hidden:true},
			{header:'收款名称',dataIndex:'SRIncomePlan$name',align : 'left',sortable : true,hidden:false},
			{header:'收款描述',dataIndex:'SRIncomePlan$descn',align : 'left',sortable : true,hidden:true},
			...
			//{header:'实际收款时间',dataIndex:'itil_Realncome$realIcomeDate',align : 'left',sortable : true,hidden:false}],
			{header:'<font color=red>实际收款时间</font>',
		...
		title : "<font color=red>收款条目</font>",
		trackMouseOver : false,
		loadMask : true,
		clicksToEdit : 1,
		collapsible : true,
		...
		frame : true,
		tbar : [
				new Ext.Toolbar.TextItem("<font color=red>请选择收款条目并填写相应的收款时间</font>"),
				button],
		listeners : {
			afteredit : function(obj) {
				var r = obj.record;
				var id = r.get("itil_RealIncome$id");
				var date = r.get("itil_RealIncome$realDate");
				date = new Date(date).dateFormat('Y-m-d');
				var store = this.store;
				Ext.Ajax.request({
			        url:webContext+'/businessAccountAction_saveIncomeDate.action?id='+id+'&date='+date,
			        method:'post',
...
function getFormpanel_SpecialRequire3_Input() {
	...
	var data = da.getSingleFormPanelElementsForEdit(
			"panel_SpecialRequire_Input", this.requireId);
...
function getTabPanel() {
	...
	var tab = new Ext.TabPanel({
		...
		items : [{
			...
			title : "<font color=red>收款申请</font>",
			width : "auto",
