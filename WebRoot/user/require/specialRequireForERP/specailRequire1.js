PageTemplates = Ext.extend(Ext.Panel, {
	...
	save : function() {
		...
		if(!Ext.getCmp('panel_erpSR_enter_input').form.isValid()){
		...
		var formParam = Ext.encode(getFormParam('panel_erpSR_enter_input',false));
		Ext.Ajax.request({
			url : webContext + '/requireAction_saveApplyDraft.action',
			params : {
				pagePanel:'panel_erpSR_enter_input',
				info : formParam,
 ...
 getFormpanel_erpSR_enter_input: function() {
 	var pName = this.processName;
 	  ...
 	  var da = new DataAction();
		...
		// 判断是新增还是修改
		var biddata = "";
		data = da.getRequireFormPanelElementsForEdit("panel_erpSR_enter_input",this.dataId);
		...
		var histroyForm = new HistroyForm({
	     ...
	     this.getFormpanel_erpSR_enter_input();
		  ...
		  this.tab = new Ext.TabPanel({
			...
			items : [this.formpanel_erpSR_enter_input, histroyForm]
		});

		if (this.status != 0) {
			this.items = [this.tab];
		} else {
			this.items = [this.formpanel_erpSR_enter_input];
		}
