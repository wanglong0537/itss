		...
		if(tempNeed==""){
			...
			success : function(response, options) {
				...
				if (tempNeed == "1" ) {
					...
					window.parent.auditContentWin.specialAudit();//modify by zhangzy for 拒绝流程出错 in 2009 11 16
				}
 ...
 getFormpanel_SpecialRequire4_Input: function() {
      var da = new DataAction();
		  var data = null;
			...
			if (this.dataId != '0') {
				data = da.getPanelElementsForEdit("sr2_issueByEngineer", "panel_SpecialRequire4_Input", this.dataId);// 这是要随时变得
 ...
 getFormpanel_SRServiceProviderInfo_Input: function() {
      var da = new DataAction();
		  var data = null;
			...
			if (this.dataId != '0') {
				data = da.getPanelElementsForEdit("sr2_issueByEngineer", "panel_SRServiceProviderInfo_Input", this.dataId);// 这是要随时变得
				for (i = 0; i < data.length; i++) {
				if (data[i].xtype == 'combo' || data[i].xtype == 'datefield') {
					data[i].id = data[i].id + 8;// 改变Combobox的id
					data[i].readOnly = true;
 ...
 getFormpanel_SRAnalyse2: function() {
      var da = new DataAction();
		  var data = null;
			...
			if (this.dataId != '0') {
				data = da.getPanelElementsForEdit("sr2_issueByEngineer", "panel_SRAnalyse2", this.dataId);// 这是要随时变得
				for (i = 0; i < data.length; i++) {
				if (data[i].xtype == 'combo' || data[i].xtype == 'datefield') {
					data[i].id = data[i].id + 8;// 改变Combobox的id
					data[i].readOnly = true;
...
//			if (this.dataId != '0') {
//				data = da.getPanelElementsForEdit("sr2_issueByEngineer", "panel_SRProjectPlan", this.dataId);// 这是要随时变得
//				biddata = da.split(data);
//			} else {
 ...
 getFormpanel_SRTransmisEngineer: function() {
      var da = new DataAction();
		  var data = null;
			...
			if (this.dataId != '0') {
				data = da.getPanelElementsForEdit("sr2_issueByEngineer", "panel_SRTransmisEngineer", this.dataId);// 这是要随时变得
				biddata = da.split(data);
			} else {
