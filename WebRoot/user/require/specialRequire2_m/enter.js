PageTemplates = Ext.extend(Ext.Panel, {
	...
	save : function() {
		...
		if(!Ext.getCmp('panel_SpecialRequire2_m_Input').form.isValid()){
		...
		var curUrl = webContext
				...
				+ 'pagePanel=panel_SpecialRequire2_m_Input'
		...
		Ext.Ajax.request({
			...
			success : function(response, options) {
				...
				window.location = webContext//modify by musicbear for 保存草稿后跳转 in 2009 11 19
 ...
 getFormpanel_SpecialRequire2_m_Input: function() {
 	var pName = this.processName;
 	  var da = new DataAction();
		...
		// 判断是新增还是修改
		var biddata = "";
		data = da.getModifyRequireFormPanelElementsForEdit("panel_SpecialRequire2_m_Input",this.dataId,this.oldId);
		...
		this.formpanel_SpecialRequire2_m_Input = new Ext.form.FormPanel({
			...
			title : "解决方案设计开发服务",
			items : biddata,
			tbar : curbuttons //modify bu musicbear for "申请服务"和"申请变更"的按钮位置不统一
		});
