PageTemplates = Ext.extend(Ext.Panel, {
	...
	getOldApply : function(){
		...
		var oldApply = new Ext.form.ComboBox({
			...
			store:new Ext.data.JsonStore({
				url:webContext+'/extjs/comboDataAction?clazz=com.digitalchina.itil.require.entity.ERP_NormalNeed&serviceItem='+curSi+"&status=1",
				fields:['id','name'],
				listeners:{
	...
	save : function() {
		...
		if(!Ext.getCmp('panel_ERP_NormalNeed1_m_Input').form.isValid()){
		...
		var formParam = Ext.getCmp('panel_ERP_NormalNeed1_m_Input').form.getValues(false);
		...
		//remove by lee for 更换为统一的验证规则结果后台配置必填项 in 20091103 begin
...
//		}
		...
		var curUrl = webContext
				...
				+ 'pagePanel=panel_ERP_NormalNeed1_m_Input'
	...
	saveAndSubmit : function() {
		...
		//add by lee for 更换为统一的验证规则结果后台配置必填项 in 20091103 end
		//add by zhangzy for 不用下拉列表选择，而是手动输入也可以提交的bug in 2009 11 23 start
		if(Ext.getCmp('ERP_NormalNeed$oldApplyCombo').getValue()==""){
			Ext.MessageBox.alert("提示","您要变更的流程不存在,请确认！");	
			return false;
		}
		//add by zhangzy for 不用下拉列表选择，而是手动输入也可以提交的bug in 2009 11 23 end
		Ext.getCmp('saveButton').disable();
		Ext.getCmp('workFlowButton').disable();
		var curscid = this.serviceItemId;
...
//		}
		...
		formParam = Ext.getCmp('panel_ERP_NormalNeed1_m_Input').form
				.getValues(false);
		var vp = null;
		if (formParam != null) {
			vp = Ext.apply(formParam, vp, {});
	...
	getFormpanel_ERP_NormalNeed1_m_Input : function() {
		...
		data = da.getModifyRequireFormPanelElementsForEdit(
				"panel_ERP_NormalNeed1_m_Input", this.dataId, this.oldId);
...
//add by zhangzy for 变更前申请下拉列表不可用  in 2009 11 23 start			
			if(idStr=='ERP_NormalNeed$oldApplyCombo'){
//				data[i].store = new Ext.data.JsonStore({
//						url:webContext+'/extjs/comboDataAction?clazz=com.digitalchina.itil.require.entity.ERP_NormalNeed',
//						fields:['id','name'],
//						listeners:{
//							beforeload : function(store, opt){
...
//add by zhangzy for 变更前申请下拉列表不可用  in 2009 11 23 end				
...
//modify by zhangzy for “我审批的” 无法打开 in 2009 11 25 start
		if (this.status != 0) {
			...
			items : [this.formpanel_ERP_NormalNeed1_m_Input, histroyForm]
		});
			this.items = [this.tab];
			
		} else {
			this.items = [this.formpanel_ERP_NormalNeed1_m_Input];
		}
