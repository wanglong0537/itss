			 ...
			 for(var i=0;i<data.length;i++){
				...
				/*if(data[i].id=='SpecialLine$startLocationCombo'){
					data[i].store=new Ext.data.JsonStore({
						url:webContext+"/configItemAction_findConfigItemExtendInfo.action?clazz=com.digitalchina.itil.config.extci.entity.OfficeLocation"+"&fuzzyQuery=locationName",
				...
				if(data[i].id=='ServiceEngineerOut$serviceProviderOutCombo'){
					data[i].store=new Ext.data.JsonStore({
						url:webContext+"/configItemAction_findConfigItemExtendInfo.action?clazz=com.digitalchina.itil.actor.entity.ServiceProviderOut"+"&fuzzyQuery=name",
						fields:['id','name'],
						totalProperty:'rowCount',
						root:'data'});
					data[i].initComponent();
				...
				if(data[i].id=='ServiceEngineerIn$serviceProviderInCombo'){
					data[i].store=new Ext.data.JsonStore({
						url:webContext+"/configItemAction_findConfigItemExtendInfo.action?clazz=com.digitalchina.itil.actor.entity.ServiceProviderIn"+"&fuzzyQuery=name",
						fields:['id','name'],
						totalProperty:'rowCount',
						root:'data'});
					data[i].initComponent();
