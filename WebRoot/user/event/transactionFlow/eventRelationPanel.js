EventRelationPanel = Ext.extend(Ext.Panel, {
	...
	opeate : function(eventReId,parentEventId,eventRelTypeId) {
		...
		var eventRelTypeCombo = new Ext.form.ComboBox({
			...
			store : new Ext.data.JsonStore({
				url : webContext
						+ '/extjs/comboDataAction?clazz=com.zsgj.itil.event.entity.EventRelationType',
				fields : ['id', 'name'],
				listeners : {
					beforeload : function(store, opt) {
		...
		var formpanel_event_suppGroupEngineer = new Ext.form.FormPanel({
			...
			new Ext.form.ComboBox({
				...
				store : new Ext.data.JsonStore({
					url : webContext
							+ '/extjs/comboDataAction?clazz=com.zsgj.itil.event.entity.EventFrequency',
					fields : ['id', 'name'],
					listeners : {
						beforeload : function(store, opt) {
				...
				store : new Ext.data.JsonStore({
					url : webContext
							+ '/extjs/comboDataAction?clazz=com.zsgj.itil.event.entity.EventPonderance',
					fields : ['id', 'name'],
					listeners : {
						beforeload : function(store, opt) {
			...
			items:[
			{
				...
				if(attachmentFlag==''||attachmentFlag=='null'){
					...
					ud.getUpDownLoadFileSu(attachmentFlag,'2454','com.zsgj.info.appframework.metadata.entity.SystemFile','appendix');
				}else{
					var ud=new UpDownLoadFile();
					ud.getUpDownLoadFileSu(attachmentFlag,'2454','com.zsgj.info.appframework.metadata.entity.SystemFile','appendix');
				}}
...
//						var ud = new UpDownLoadFile();
//						ud.getUpDownLoadFileSu(attachmentFlag, '2454','com.zsgj.info.appframework.metadata.entity.SystemFile');
//					} else {
//						var ud = new UpDownLoadFile();
//						ud.getUpDownLoadFileSu(attachmentFlag, '2454','com.zsgj.info.appframework.metadata.entity.SystemFile');
//					}
		...
		if (eventReId != "") {
			formpanel_event_suppGroupEngineer.load({
				...
				success : function(action, form) {
					...
					var url=webContext
						+ '/eventAction_getFileList.action?clazzName=com.zsgj.itil.event.entity.Event&dataId='
...
//					alert(value.file);
	...
	modifyRelationEvent : function(){
         var clazz = "com.zsgj.itil.event.entity.EventRelation";
