...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
			...
			'created' : {
				...
				'window-render' : [{
					action : [this.createForm, this.createProgressBar,
							this.createProgressBar, this.createGrid],
					state : 'rendering'
				}],
				'destroy' : [{
	...
	createForm : function() {
		...
		this.form = Ext.DomHelper.append(this.body, {
			...
			// 100px; height: 100px'
			style : 'position: absolute; left: 0px; top: 22px; width: 400px; height: 100px'
		});
	...
	isPermittedFile : function(browse_btn) {
		...
		var q = fname.lastIndexOf("/");
		// alert(p+" ,"+q);
		var shortName = fname;
		if (p > q && p > 0) {
			shortName = fname.substring(p + 1);
		} else if (q > p && q > 0) {
			shortName = fname.substring(q + 1);
		}
		// var pattern = /^[a-zA-Z]\.[a-zA-Z]{0,20}$/;
		// if (!pattern.exec(shortName)) {
		// Ext.MessageBox.alert("提示信息", "文件名[" + shortName
		// + "]不符合要求！应该以一个英文字母命名文件（订单小号），如：a.xml");
		// return false;
		// }
	...
	updateProgressBar : function() {
		if (this.is_uploading) {
			...
			this.progress_bar.updateProgress(0, this.i18n.note_upload_success);
		}
	},
	...
	updateToolbar : function() {
		...
		if (this.is_uploading) {
			...
			if (this.grid_panel.getStore().getCount() > 0) {
				tb.x_buttons.reset.enable();
				tb.x_buttons.add.disable();

			} else {
				tb.x_buttons.reset.disable();
				tb.x_buttons.add.enable();
			}
		}
	},
	...
	onAjaxSuccess : function(response, options) {
		...
		var json_response = {
			'success' : this.i18n.note_upload_success,
			'error' : false
		}
		...
		// by yang
		//json_response.bizmessage = Ext.decode(response.responseText).bizmessage;
		// by yang
		var data = {
...
// //
/*
 * var p = Ext.ux.UploadDialog.Dialog.prototype; p.i18n = { title: 'File upload
 * dialog', state_col_title: 'State', state_col_width: 70, filename_col_title:
 * 'Filename', filename_col_width: 230, note_col_title: 'Note', note_col_width:
 * 150, add_btn_text: 'Add', add_btn_tip: 'Add file into upload queue.',
 * remove_btn_text: 'Remove', remove_btn_tip: 'Remove file from upload queue.',
 * reset_btn_text: 'Reset', reset_btn_tip: 'Reset queue.',
 * upload_btn_start_text: 'Upload', upload_btn_stop_text: 'Abort',
 * upload_btn_start_tip: 'Upload queued files to the server.',
 * upload_btn_stop_tip: 'Stop upload.', close_btn_text: 'Close', close_btn_tip:
 * 'Close the dialog.', progress_waiting_text: 'Waiting...',
 * progress_uploading_text: 'Uploading: {0} of {1} files complete.',
 * error_msgbox_title: 'Error', permitted_extensions_join_str: ',',
 * err_file_type_not_permitted: 'Selected file extension isn\'t permitted.<br/>Please
 * select files with following extensions: {1}', note_queued_to_upload: 'Queued
 * for upload.', note_processing: 'Uploading...', note_upload_failed: 'Server is
 * unavailable or internal server error occured.', note_upload_success: 'OK.',
 * note_upload_error: 'Upload error.', note_aborted: 'Aborted by user.' }
 */
