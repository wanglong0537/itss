 ...
 *By passing in a dataIndex and a store, it can pre-check (and select)
 *rows after it renders. Included are all the standard navigation
 *options of a RowSelectionModel, including Up/Down arrow keyMaps and
 ...
 *@copyright June 4, 2008 <last updated: June 24, 2008>
...
Ext.grid.SmartCheckboxSelectionModel = Ext.extend(Ext.grid.RowSelectionModel, {
	/**
	 *@header (string) Any valid text or HTML fragment to display in the header
	 *cell for the checkbox column The default CSS (defaults to
	 *'&lt;div class="x-grid3-hd-checker"> &lt;/div>'). class of
	 *'x-grid3-hd-checker' displays a checkbox in the header and
	 *provides support for automatic check all/none behavior on header
	 *click. This string can be replaced by any valid HTML fragment,
	 *including a simple text string (e.g., 'Select Rows'), but the
	...
	width : 30,
	/**
	 *@sortable (bool) Set to true if you want the checkbox column to be
	 *sortable.
	 */
	fixed: true,
	sortable : false,
	/**
	 *@email (bool) Will mimic email client functionality by separating out the
	 *selection of rows with the checking of rows, similar to how Yahoo!
	 *or Gmail works. One could then apply different actions on checked
	 *rows vs. selected rows. Defaults to false.
	 */
	email : false,
	/**
	 *@excel (bool) Mimics excel functionality when clicking on rows or
	 *checkboxes. If set to true, all other rows will be deselected and
	 *unchecked except the row you most recently clicked. If set to
	 *false, all previous selections will remain selected and/or checked
	 *as you click around the grid (as if you were holding down CTRL and
	 *clicking). Defaults to false.
	 */
	excel : false,
	/**
	 *@alwaysSelectOnCheck (bool) If set to true, clicking a checkbox will
	 *always select the row, working in conjunction * with
	 *the email option. 
	 *Defaults to false.
	 */
	alwaysSelectOnCheck : false,
	// private menuDisabled:true, fixed:true, id: 'checker', dataIndex: '',
	// Define the dataIndex when you construct the selectionModel, not here
	// private
	initEvents : function() {
		// Call the parent
		...
		// Disable the rowNav created in our parent object, otherwise pressing
		// DOWN will go down two rows!
		this.rowNav.disable();
		...
		// If we sent a store to the selModel, auto-select rows based on
		// dataIndex
		if (this.grid.store) {
			this.grid.store.on('load', function(p) {
				// This block of code checks the status of the checkbox header,
				// and if checked, will check all other checkboxes (but not on
				// the initial load)
				...
				// This block of code will pre-select checkboxes based on the
				// dataIndex supplied,
				// but only on the initial load.
				var dataIndex = this.grid.getSelectionModel().dataIndex;
				// the dataIndex for the selectionModel
				var count = this.grid.store.getCount();
				for (var i = 0, len = count; i < len; i++) {
					var dataIndexValue = p.data.items[i].data[dataIndex];
					// the value of the dataIndex for each row
					var isSelected = this.isSelected(i);
					if ((dataIndexValue == true || isSelected)
							&& !Ext.state.Manager.loaded) {
						// This code will only run the first time a grid is
						// loaded
	 ...
	 * @param (bool) c optional flag set to either true (to check) or false (touncheck) if no second param, the checkbox will toggle itself
	 */
	...
	/**
	 * private functions that toggles all checkboxes on or off depending on
	 * param
	 * 
	 * @param (bool)
	 *            c true to check all checkboxes, false to uncheck all
	 *            checkboxes
	 * @param (int)
	 *            e (optional) if an exception is given, all rows will be
	 *            checked/unchecked except this row
	 */
	...
	/**
	 * private function that clears all checkboxes specifically used to deal
	 * with shift+arrow keys, but can also be called with fast param to quickly
	 * uncheck everything
	 * 
	 * @param (bool)
	 *            fast true to quickly deselect everything with no exceptions
	 */
	...
	/**
	 * private function used in conjuction with the shift key for checking
	...
	/**
	 * private function that executes when you click on any row will keep other
	 * row selections active as you click around
	 */
	...
	onHdMouseDown : function(e, t) {
		if (t.className == 'x-grid3-hd-checker') {
			...
			if (isChecked) {
				...
				// the true param enables fast mode
			} else {
				hd.addClass('x-grid3-hd-checker-on');
				if (!this.email || this.alwaysSelectOnCheck) {
					this.selectAll();
	...
	/**
	 *private function that renders the proper checkbox based on your dataIndex
	 *variable
	 *@param (varchar)
	 *v the dataIndex passed into the selectionModel that contains
	 *whether a row is checked by default or not
	 */
	renderer : function(v, p, record) {
		p.css += ' x-grid3-check-col-td';
		return '<div class="x-grid3-check-col' + (v ? '-on' : '')
				+ ' x-grid3-cc-' + this.id + '"> </div>';
//		return '<div id="x-grid3-hd-checker" class="x-grid3-hd-checker"> </div>';
	}
});
