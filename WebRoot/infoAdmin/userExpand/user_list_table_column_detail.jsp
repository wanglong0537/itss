...
function initTableSelect(cobj, fselect){
  var j=1; 
  for(var i=fselect.options.length-1;i>=0;--i){ 
	fselect.options[i]=null;  
  }
...
function findTableByModule(){ 
   ...
   var foreignTable = Ext.getDom("foreignTable");
   var moduleSelectedId = module.options[module.selectedIndex].value;
  
  
   Ext.Ajax.request({
      url: serverPath+"/infoAdmin/userMainTableColumn.do?methodCall=findTableByModule", 
      params:{
      ...
      success:function(response){
        ...
        var obj = result.data;
        initTableSelect(obj, foreignTable);
...
function returnTableDetail(){
  window.location.href="${pageContext.request.contextPath}/infoAdmin/userListMainTable.do?methodCall=toForm&id=${param.tableId}";
} 
...
<TABLE cellSpacing=1 cellPadding=3 width="100%" bgColor=#6298e1 border=0 height="56">
  ...
  <TR>
    ...
    用户自定义主表名称：${smt.tableCnName }
    </TD></TR></TBODY></TABLE><BR>
 <form action="${pageContext.request.contextPath}/infoAdmin/userListTableColumn.do?methodCall=save" method=post  name="editForm" id="editForm">
字段属性
