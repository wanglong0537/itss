...
TreePanel = function(config){
	...
	this.iconPath = 'Images/TreeImages/';
	this.clickListeners = [];
	...
	initEvent : function(){
		...
		this.element.onclick=function(event){
			...
			if(_type == 'clip'){
				...
				//在这里添加是否监听判断
				for(var i=0; i < _this.clickListeners.length; i++){
					_this.clickListeners[i](node,_type);
				}
				node.onCheck();
			}
		};
...
TreePanel.prototype.icon = {
	...
	server				: 'server.gif',
	gateway				: 'terminal.gif',
	gduser				: 'user.gif',
	wirelessRoot		: 'wireless.gif'					
};
...
TreeNode.prototype={
	...
	paintText : function(){
		...
		if(title!=null&&title.length>0){
			this['html-element']['text'].title=title;
		}else{
	...
	//触发点击选择事件,只要this.checked==0的选上,不为0的,就不管
	onCheckByTreeBack : function(){
		if(this.checked!==false){
			if(this.checked==1){
				//this.cascade((function(checked){
				//	this.setCheck(checked);
  ...
  //private
	...
	//查找节点下的所有匹配子节点（级联查找所有子节点）
	findMyChilds : function(attribute, value){
		var cs = this.childNodes;
		var childs = new Array();
		var childss = new Array();
		var m = 0 ;
    for(var i = 0, len = cs.length; i < len; i++) {
			if(cs[i].attributes[attribute] == value){
				childs[m] = cs[i];
     		...
     		if(cs[i].hasChildNodes()){
     			childss = cs[i].findMyChilds(attribute, value);
     			for(var n=0;n<childss.length;n++){
     				childs[m] =childss[n];
     				m++;
     			}
     		}
