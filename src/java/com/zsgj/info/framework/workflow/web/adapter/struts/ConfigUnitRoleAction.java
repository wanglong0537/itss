...
public class ConfigUnitRoleAction extends BaseAction {
	...
	public String queryCobom() throws Exception{
		...
		if (id != null && !"".equals(id)) {
			...
			Map<Object, Object> queryParamValues = metaDataManager.genQueryParams(clazz, requestParams);
	...
	public String findRoleByDept(){
		...
		Department department = (Department)super.getService().find(Department.class, deptId);
		List list = super.getService().find(Role.class, "department", department);
		...
		for(int i=0; i< list.size(); i++){
			...
			json += "{\"id\":"+id+",\"descn\":\""+descn+"\",\"name\":\""+name+"\"},";
		}
		if(json.endsWith(",")){
	...
	public String configUnitRoleSave(){
		...
		String virtualDefinitionId = super.getRequest().getParameter("virtualDefinitionId");
		String nodeId = super.getRequest().getParameter("nodeId");
		String definitonName = HttpUtil.ConverUnicode(super.getRequest().getParameter("definitonName"));
		...
		String auditPerson = HttpUtil.ConverUnicode(super.getRequest().getParameter("auditPer"));
		if("".equals(checkCreate)||checkCreate==null||"false".equals(checkCreate)){
			checkCreate="0";
		}else{
			checkCreate="1";
		}
		...
		if(configUnitRoleTableId==null||"".equals(configUnitRoleTableId)){
			...
			ConfigUnitRole unitRole = configUnitService.findConfigUnitRole(virtualDefinitionId, nodeId);
			if(unitRole==null||"".equals(unitRole)){//这时里面还没有相应配置单元角色
				...
				if(roleIds!=null && roleIds.length>0){
					for(int i=0; i<roleIds.length; i++){
						...
						if(!"".equals(roleId)){
							Role role = (Role)super.getService().find(Role.class,roleId);
							//**********************************************************
			...
			if(roleIds!=null && roleIds.length>0){
				...
				for(int i=0; i<roleIds.length; i++){
					...
					if(!"".equals(roleId)){//LIST就是当前修改角色信息								
						ConfigUnitRoleTable configUnitRoleTable = (ConfigUnitRoleTable)super.getService().find(ConfigUnitRoleTable.class, configUnitRoleTableId);
						List<ConfigUnitRoleTable> list = super.getService().find(ConfigUnitRoleTable.class, "configUnitRole", configUnitRoleTable.getConfigUnitRole());
						for(ConfigUnitRoleTable table : list){
							roleList.add(table.getRole());
						}
						...
						if(roleList.contains(role)){//如果当前角色有重复值
							...
							configUnitRoleTable.setConfigUnitRole(configUnitRoleTable.getConfigUnitRole());
							configUnitRoleTable.setRole(role);
							configUnitRoleTable.setFlag(Integer.parseInt(roleType));
							configUnitRoleTable.setWorkflowBrowsePerson(auditPerson);
	...
	public String modifyRoleMessage(){
		...
		String json = "";
		String configUnitRoleTableId = super.getRequest().getParameter("id");
		ConfigUnitRoleTable rolesTable = (ConfigUnitRoleTable)super.getService().find(ConfigUnitRoleTable.class, configUnitRoleTableId);
		//List<ConfigUnitRoleTable> rolesTable = super.getService().find(ConfigUnitRoleTable.class, "configUnitRole", configUnitRole);
...
//		while(ite.hasNext()){
		//for(int i=0;i<rolesTable.size();i++){
			...
			json += "{\"id\":"+role.getId()+",\"descn\":\""+role.getDescn()+"\",\"name\":\""+role.getName()+"\"},";
		//}
	...
	public String configUnitRoleDelete(){
		...
		String[] configUnitRoleTableId = super.getRequest().getParameterValues("removeIds");
		String virtualId = super.getRequest().getParameter("virtualDefinitionId");
		String nodeId = super.getRequest().getParameter("nodeId");
		ConfigUnitRole configUnitRole = configUnitService.findConfigUnitRole(virtualId, nodeId);
		if(configUnitRole!=null&&!"".equals(configUnitRole)){
			...
			if(creatorFlag==0){//当说明没有加入申请人时候，全部删除即可
				for(int i=0;i<configUnitRoleTableId.length;i++){
					ConfigUnitRoleTable configUnitTable = (ConfigUnitRoleTable)super.getService().find(ConfigUnitRoleTable.class, configUnitRoleTableId[i]);		
					super.getService().remove(configUnitTable);
				}
				List<ConfigUnitRoleTable> table = super.getService().find(ConfigUnitRoleTable.class, "configUnitRole", configUnitRole);
				if(table.size()==0){
	...
	public String queryProNameAndnodeName(){
		...
		ConfigUnitRole unitRole = configUnitService.findConfigUnitRole(virtualDefinitionInfoId, nodeId);
		VirtualDefinitionInfo vitualDef = (VirtualDefinitionInfo) service.findUnique(VirtualDefinitionInfo.class, "id", Long.valueOf(virtualDefinitionInfoId));
		List<VirtualNodeInfo> virturalNodes = service.find(VirtualNodeInfo.class,"virtualDefinitionInfo", vitualDef);
		List listProperty = new ArrayList();
		HashMap<String , Object> relation = new HashMap<String , Object>();
		if(tableId!=null&&!"".equals(tableId)){
			ConfigUnitRoleTable unitTable = (ConfigUnitRoleTable)super.getService().find(ConfigUnitRoleTable.class, tableId);
	...
	public String getConfigUnitRole() throws Exception {
		...
		String roleType = "";
		String nodeType = "";
		String processId = super.getRequest().getParameter("virtualDefinitionInfoId");
		String nodeId = super.getRequest().getParameter("nodeId");
		VirtualDefinitionInfo virtualDefinitionInfo = (VirtualDefinitionInfo)service.findUnique(VirtualDefinitionInfo.class, "id",Long.valueOf(processId) );
		//VirtualNodeInfo  virtualNodeInfo = (VirtualNodeInfo)service.findUnique(VirtualNodeInfo.class, "nodeId", Long.valueOf(nodeId));//?
		VirtualNodeInfo  virtualNodeInfo = configUnitService.findVirtualNodeInfo(virtualDefinitionInfo, nodeId);
		...
		ConfigUnitRole unitRole = configUnitService.findConfigUnitRole(processId, nodeId);
		...
		List<ConfigUnitRoleTable> unitTable = super.getService().find(ConfigUnitRoleTable.class, "configUnitRole", unitRole);
		...
		if(unitRole!=null){
			...
			for(int i=0;i<unitTable.size();i++){//保存table表的id
				...
				json += "{id:" + unitTable.get(i).getId()+",nodeType:'"+nodeType+ "',virtualDesc:'" + desc
				+ "',nodeName:'" + nodeName +"',roleType:'"+roleType+ "',roleName:'"
				+ unitTable.get(i).getRole().getName() +"',nodeDesc:'"+nodeDesc+"'},";	
			}	
			if(roles.size()!=0){
			json = "{data:[" + json.substring(0, json.length()-1) + "]}";
			}
			...
			try {
			super.getResponse().setCharacterEncoding(this.getProperties(
	...
	public String findRolesByDept() throws Exception{
		String json = "";
		String virtualId = super.getRequest().getParameter("virtualDefinitonId");
		VirtualDefinitionInfo virtualDefinitonInfo = (VirtualDefinitionInfo)super.getService().find(VirtualDefinitionInfo.class, virtualId);
		Department dept = virtualDefinitonInfo.getDept();
		List<Role> roles = super.getService().find(Role.class, "department", dept);
	...
	public String getConfigUnitCreateFlag() throws Exception{
		...
		int flag = 0;
		int sendMailFlag = 0;
		String virProcessId = super.getRequest().getParameter("virtualDefinitionInfoId");
		String nodeId = super.getRequest().getParameter("nodeId");
		ConfigUnitRole unitRole = configUnitService.findConfigUnitRole(virProcessId, nodeId);
		if(unitRole!=null&&!"".equals(unitRole)){
		...
		try {
			super.getRequest().setCharacterEncoding("utf-8");
			PrintWriter pw = super.getResponse().getWriter();
			pw.write("{success:" + true + ",sendMailFlag:"+sendMailFlag+",flag:"+ flag + "}");
			pw.flush();
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	/**
	 * 保存是否不需要发送邮件
	 * 首先判断如果configUnitRole已经存在，则只需添加上相应的属性值；
	...
	public String configUnitRoleSendMailCheckSave()throws Exception{
		...
		String nodeName = super.getRequest().getParameter("nodeName");
		String virtualDefinitionInfoId = super.getRequest().getParameter("virtualDefinitionId");
		String nodeId = super.getRequest().getParameter("nodeId");
		String nodeType = super.getRequest().getParameter("nodeType");
		...
		ConfigUnitRole unitRole = configUnitService.findConfigUnitRole(virtualDefinitionInfoId, nodeId);
		VirtualDefinitionInfo virtualDefinitionInfo = (VirtualDefinitionInfo)service.find(VirtualDefinitionInfo.class, virtualDefinitionInfoId);
		VirtualNodeInfo nodeInfo = configUnitService.findVirtualNodeInfo(virtualDefinitionInfo, nodeId);
...
//		DefinitionServiceImpl dsi = new DefinitionServiceImpl();
		...
		mapBizz.put("workflowHistory", "com.digitalchina.itil.config.entity.ConfigItemAuditHis");
		mapBizz.put("applyType","cproject");
		mapBizz.put("addDynCounterSign", "repeatTaskNodeTwo:gaowen,guoxl");
		//mapBizz.put("dynCounterSign", "repeatTaskNodeTwo:1+guangsa,haowc$repeatTaskNodeThree:0+guoxl,sujs");
		...
		try{
			Long processId = processService.createProcess("TestMulitpleFlow9876541250501630687", "guangsa", mapBizz);
		}catch(RuntimeException e){
...
//子流程1----公告管理审批流程的业务参数
//mapBizz.put("noticeManager1237967838187", "dataId=85,applyId=85,applyType=nproject,applyTypeName=公告审批,customer=");
