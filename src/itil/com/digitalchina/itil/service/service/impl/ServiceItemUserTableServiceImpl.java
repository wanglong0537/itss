...
//		begin
		...
		c.add(Restrictions.ilike("systemMainTable.className", "com.digitalchina.itil.config.entity.ConfigItem"));
		...
		c.setFetchMode("systemMainTable", FetchMode.JOIN);
		c.add(Restrictions.ne("systemMainTableColumnType.id", Long.valueOf(8)));
		c.add(Restrictions.ilike("systemMainTable.className", "com.digitalchina.itil.config.entity.ConfigItemFinanceInfo"));
		smtcs = c.list();
		int cifcsize = 0;
		for(int i=0; i<smtcs.size(); i++){
			SystemMainTableColumn smtc = smtcs.get(i);
