	...
	public String getDycyList(String userId, String passwd) {
		...
		for(TaskInfo task:tasks){
			if(task.getTaskName().equals("承办传阅")){
				...
				map.put("createDate", task.getCreateTime()!=null?task.getCreateTime().toString().substring(0, 19):"");
				dycylist.add(map);
			}
		}
	   ...
	   public String getDbsxDetail(String userId, String passwd,String activityName,String taskId){
		   ...
		   ProDefinition proDefinition = getProDefinition();
			...
			if (this.taskId != null) {
				...
				Map processRunVars = processFormService
			...
			//”0”,(0发文流程，1收文流程，2请假流程)
			if(processName.equals("发文流程")||processName.equals("请示报告")||processName.equals("发文流程-市局发文")){
				...
				Archives archives=archivesService.get(Long.parseLong(id));
					...
					for(ProcessForm pf:pformlist){
						...
						for(;foemdates.hasNext();){
							...
							if(fd.getIsShowed()==1){
								json+="{label:\""+fd.getFieldLabel()+"\",value:\""+fd.getVal()+"\"},";
							}
						}
					}
					...
					for(;iterator.hasNext();){
						doc = iterator.next();
						json+="{name:\""+doc.getDocName() + "\",\"url\":\"" + doc.getFileAttach().getFilePath() +"\",type:\""+getType(doc.getFileAttach().getExt())+"\"}";
						if(iterator.hasNext()){
							json+=",";
						}
					}		
				...
				Archives archives=archivesService.get(Long.parseLong(id));
					...
					json+="]}";
			}else if(processName.equals("请假流程")){
