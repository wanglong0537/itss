...
public class OrderController
{
  ...
  private void prepareData(Model m, Cart cart, String uid)
  {
    ...
    for (CartItem item : cart.getItemList()) {
      ...
      item.setStoreCount("" + this.productListService.getProDetailQuantity(item.getSid(), item.getPromotionPrice()));
    }
    ...
    List addresslist = uid == null ? new ArrayList() : this.memberService.getAdressList(uid);
    ...
    List availtypes = expressTypes;
    boolean existCollect = false;
    if (order.getInceptCitySid() != null) {
      availtypes = this.freightService.getExpressTypeByCity(Integer.valueOf(Integer.parseInt(order.getInceptCitySid())));
    }
    ...
    if ((order.getInvoiceBit() == null) || ("".equals(order.getInvoiceBit()))) {
    ...
    double sendCost = order.getSendCost() == null ? 0.0D : Double.parseDouble(order.getSendCost());
    ...
    m.addAttribute("validate_sendtype", Boolean.valueOf(order.getSendType() != null));
    m.addAttribute("validate_paytype", Boolean.valueOf(order.getPaymentModeSid() != null));
    m.addAttribute("validate_productlist", Boolean.valueOf(!list.isEmpty()));
  }
  @MethodSecurity
  @RequestMapping(value={"/order/confirm"}, method={org.springframework.web.bind.annotation.RequestMethod.GET})
  public String confirmOrder(Model m, @CookieValue(value="shopin_sid", required=false) String cartId, @CookieValue(value="referer", required=false) String referer, @CookieValue(value="userTicket", required=false) String uid, @CookieValue(value="cid", required=false) String cid, HttpServletRequest req) {
  ...
  public String selectAddress(Model m, @CookieValue(value="userTicket", required=false) String uid, @CookieValue(value="shopin_sid", required=false) String cartId, HttpServletRequest req, String addressSID) {
    if (!validateCartId(cartId)) {
    ...
    List expressTypes = this.freightService.getExpressTypeByCity(new Integer(address.getDeliCitySid()));
  ...
  public String selectNewAddress(Model m, @CookieValue(value="userTicket", required=false) String uid, @CookieValue(value="shopin_sid", required=false) String cartId, HttpServletRequest req, MemberAddressVO address, String flag) {
    if (!validateCartId(cartId)) {
      m.addAttribute("result", new AjaxMessageVO("0", "很抱歉，当前会话已经失效，请刷新页面"));
      return null;
    }
    if ("1".equals(flag)) {
      if (!validateUid(uid)) {
        m.addAttribute("result", new AjaxMessageVO("0", "您还没有登录,不能将地址添加到地址簿"));
        return null;
      }
  ...
  public String editInvoice(Model m, @CookieValue(value="shopin_sid", required=false) String cartId, OrderVO order) {
    if (!validateCartId(cartId)) {
      m.addAttribute("result", new AjaxMessageVO("0", "很抱歉，当前会话已经失效，请刷新页面"));
      return null;
    }
  ...
  public String editPayType(Model m, @CookieValue(value="shopin_sid", required=false) String cartId, @CookieValue(value="userTicket", required=false) String uid, OrderVO order) {
    if (!validateCartId(cartId)) {
      m.addAttribute("result", new AjaxMessageVO("0", "很抱歉，当前会话已经失效，请刷新页面"));
      return null;
    }
    ...
    if ((!validateRequired(order.getPaymentModeSid())) || (!validateRequired(order.getPaymentModeName())))
    {
    ...
    if ("会员账户".equals(order.getPaymentModeName())) {
      if (!validateUid(uid)) {
        m.addAttribute("result", new AjaxMessageVO("0", "您还没有登录，不能使用账户余额支付，请选择其他支付方式"));
        return null;
      }
      String memSid = this.securityService.getLoginUserId(uid).getMembersSid();
      MemberAccountVO account = this.memberAccountService.getAccount(memSid);
  ...
  public String editSendType(Model m, @CookieValue(value="shopin_sid", required=false) String cartId, OrderVO order) {
    if (!validateCartId(cartId)) {
      m.addAttribute("result", new AjaxMessageVO("0", "很抱歉，当前会话已经失效，请刷新页面"));
      return null;
    }
    ...
    if ((!validateRequired(order.getSendType())) || (!validateRequired(order.getSendTypeName()))) {
      m.addAttribute("result", new AjaxMessageVO("0", "请选择配送方式"));
      return null;
    }
    String freight = this.freightService.getFreight(cityId, order.getSendType());
    if (!validateRequired(freight)) {
      m.addAttribute("result", new AjaxMessageVO("0", "选择配送方式失败，很抱歉，暂不支持" + old.getInceptCity() + "的" + order.getSendTypeName()));
  ...
  public String commitOrder(Model m, @CookieValue(value="userTicket", required=false) String uid, @CookieValue(value="shopin_sid", required=false) String cartId, @CookieValue(value="cid", required=false) String cid, @CookieValue(value="LTYPE", required=false) String ltype, @CookieValue(value="LTYPES", required=false) String ltypes, @CookieValue(value="LTINFO", required=false) String ltinfo, HttpServletRequest req) {
    ...
    if (!validateUid(uid)) {
      m.addAttribute("result", new AjaxMessageVO("01", "您还没有登录，请先登录<br/>如果您不是会员，也可以使用快速购买"));
      return null;
    }
    String membersSID = this.securityService.getLoginUserId(uid).getMembersSid();
    ...
    try
    {
      OrderVO newOrder = this.orderService.saveOrder(order);
  ...
  public String quickBuy(Model m, @CookieValue(value="userTicket", required=false) String uid, @CookieValue(value="shopin_sid", required=false) String cartId, String email, HttpServletRequest req) {
    if (!validateCartId(cartId)) {
      m.addAttribute("result", new AjaxMessageVO("0", "很抱歉，当前会话已经失效，请刷新页面"));
      return null;
    }
    if (validateUid(uid)) {
      m.addAttribute("result", new AjaxMessageVO("10", "您已经登录，不需要申请快速购买<br/>即将转至订单确认页面，请再次确认订单"));
      return null;
    }
    ...
    memberVO = this.memberService.validate(memberVO.getMemEmail(), memberVO.getMemPwd());
    if (memberVO != null) {
      ...
      if (!validateProduct(cart.getItemList())) {
        m.addAttribute("result", new AjaxMessageVO("01", "申请快速购买成功，您目前还没有购买商品，请尽快选购商品"));
        return null;
      }
      m.addAttribute("result", new AjaxMessageVO("1", "申请快速购买成功，即将转至订单确认页面，请再次确认订单"));
    }
    else {
  ...
  public String orderPay(Model m, @CookieValue(value="userTicket", required=false) String uid, @PathVariable("orderId") String orderId, HttpServletRequest req) {
    if (!validateUid(uid)) {
      return "redirect:/index.html";
    }
    ...
    m.addAttribute("offPriceRecord", this.orderService.getOffPriceRecord(order.getOrderId()));
  ...
  public String payOrder(Model m, @CookieValue(value="userTicket", required=false) String uid, @PathVariable("orderId") String orderId, HttpServletRequest req) {
    if (!validateUid(uid)) {
      return "redirect:/index.html";
    }
    if (!validateRequired(orderId)) {
