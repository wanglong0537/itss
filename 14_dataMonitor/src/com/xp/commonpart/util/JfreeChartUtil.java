	...
	//1、饼状图数据集
	 ...
	 public static CategoryDataset getDataSetBar(List<String[]> values) {
			...
			for(String[] val:values){
				...
				dataset.addValue(Double.parseDouble(val[0].toString()), val[1], val[2]);
			}
			return dataset;
		}
	 ...
	 public static DefaultValueDataset getDataSetDial(String value) {
		 	...
		 	dataset.setValue(Double.parseDouble(value));
			return dataset;
		}
	 public static String generatePieChart(HttpSession session, PrintWriter pw, int w,
				int h,Map maps,PieDataset dataset,String weburl) {
		...
		//	PieDataset dataset = getDataSet();
			...
			try {
				if(weburl==null||weburl.length()==0){
					...
					Long pname=new Date().getTime();
					filename=weburl+"/"+pname+".jpg";
					File file=new File(filename);
					ChartUtilities.saveChartAsJPEG(file, chart, w, h);
				}
			} catch (IOException e) {
				e.printStackTrace();
			}	
		...
		public static String generateBarChart(HttpSession session, PrintWriter pw, int w,
				int h,Map maps,CategoryDataset dataset,String weburl) {
			// TODO Auto-generated method stub
			String filename = null;
