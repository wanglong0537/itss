    ...
    creatRole : function() {
       var department = new Ext.form.ComboBox({
		...
		var roleStore2 = new Ext.data.JsonStore({
			root : 'roles',
            url: webContext+ '/sysManage/userMangeAction.do?methodCall=findRoleByDept',
            baseParams: {
                  deptCode:'1101'
             },    
...
//			},
	...
	roleListGrid : function(UserId) {
		...
		conn.open("POST", webContext+ '/sysManage/userMangeAction.do?methodCall=findRoles', false);
		conn.send(null);
		var data = Ext.util.JSON.decode(conn.responseText);*/
        var roleStore = new Ext.data.JsonStore({
			root : 'roles',
			url : webContext+ '/sysManage/userMangeAction.do?methodCall=findRolesByUserId',
			baseParams : {
				userId : UserId
			},
			fields : ['name', 'department', 'descn', 'id'],
  ...
  // 新建用户窗口++++++++++++++++++++++++++++++++++++++++++++++++++(方法6)
	...
	initComponent : function() {
		...
		this.store = new Ext.data.JsonStore({
			id : "id",
			url : webContext + '/sysManage/userMangeAction.do?methodCall=queryAllUserByParameter',
			root : "users",
			totalProperty : "rowCount",
			remoteSort : false,
			timeout : 3000000,
