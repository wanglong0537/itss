UserManagePanel = Ext.extend(Ext.Panel, {
	...
	lastValisateUserMessage : function() {
        ...
        if (email != 0) {
				...
				if (index1 == 0 || index1 == -1 || index2 == -1
						|| index1 > index2 || index2 - index1 == 1
						|| index2 == len - 1) {
    ...
    creatRole : function() {
       var department = new Ext.form.ComboBox({
			...
			store : new Ext.data.JsonStore({
				url : webContext
						+ '/sysManage/userOuterMangeAction.do?methodCall=findDeptForCombo',
		...
		var roleStore2 = new Ext.data.JsonStore({
			root : 'roles',
            url: webContext+ '/sysManage/userOuterMangeAction.do?methodCall=findRoleByDept',
            baseParams: {
                  deptCode:'1101'
             },    
		...
		var userDetailWin = new Ext.Window({
			title : '选择角色【<font color=red>通过相应的部门选择相应的角色，默认部门是上品公司</font>】',
			modal : true,
			height : 320,
			width : 520,
			resizable : false,
			...
			buttons : [{
				...
				handler : function() {
                   ...
                   var gP = Ext.getCmp("roleGrid").getStore().getRange(0,Ext.getCmp("roleGrid").getStore().getCount());
                    var tempName="";
					for (var i = 0; i < records.length; i++) {                                  
                        ...
                        if(flag==0){Ext.Msg.alert("提示","角色：<font color=red>"+tempName+"</font>已经存在不能重复添加");continue;}
						var data = [{
							id : 'id',
							mapping : 'id'
						}, {
...
//			},
			store : new Ext.data.JsonStore({
				url : webContext
						+ '/sysManage/userOuterMangeAction.do?methodCall=findCustOutForCombo',
				fields : ['id', 'departName'],
				totalProperty : 'rowCount',
				root : 'data',
				id : 'id',
		...
		this.form = new Ext.form.FormPanel({
         ...
         if (userId != "0") {
			this.form.load({
				url : webContext
						+ '/sysManage/userOuterMangeAction.do?methodCall=findUserForm&id='
						+ userId,
				timeout : 30,
	...
	roleListGrid : function(UserId) {
		...
		conn.open("POST", webContext+ '/sysManage/userOuterMangeAction.do?methodCall=findRoles', false);
		conn.send(null);
		var data = Ext.util.JSON.decode(conn.responseText);
        var roleStore = new Ext.data.JsonStore({
			root : 'roles',
			url : webContext+ '/sysManage/userOuterMangeAction.do?methodCall=findRolesByUserId',
			baseParams : {
				userId : UserId
			},
			fields : ['name', 'department', 'descn', 'id'],
							...
							if (btn == 'yes') {
								if (records) {
									for (var i = 0; i < records.length; i++) {
                                        ...
                                        this.grid.editRemovedIds += records[i].get("id")+ ",";                                                           
									}
								}
							}
						}, this)
		...
		var userDetailWin = new Ext.Window({
			...
			buttons : [{
				...
				handler : function() {
					...
					Ext.Ajax.request({
						url : webContext
								+ '/sysManage/userOuterMangeAction.do?methodCall=saveUsers',
						method : 'POST',
						params : request,
						success : function(response, options) {
                           var flagSign = eval("(" + response.responseText+ ")").flagSign;
							if (flagSign == "exitIsLock") {
								var userNameTring = eval("("+ response.responseText + ")").userNameTring;
								ExtExt.MessageBox.alert("警告", userNameTring
										+ "已经被锁定");
                                  Ext.getCmp("saveNewUserButton").enable();                                                                   
							} else if (flagSign == "exit") {
								var userNameTring = eval("("+ response.responseText + ")").userNameTring;
								ExtExt.MessageBox.alert("警告", userNameTring+ "已经存在");
                                      Ext.getCmp("saveNewUserButton").enable();                                           
							} else {
       ...
       var userDetailWin = new Ext.Window({
			...
			buttons : [{
				...
				handler : function() {
					...
					Ext.Ajax.request({
						url : webContext
								+ '/sysManage/userOuterMangeAction.do?methodCall=modifyUser',
						method : 'POST',
						params : request,
						success : function(response, options) {
		...
		var m = Ext.MessageBox.confirm("删除提示", "是否真的要删除数据？一旦删除,不可恢复!",
				function(ret) {
					if (ret == "yes") {
						Ext.Ajax.request({
							url : webContext
									+ '/sysManage/userOuterMangeAction.do?methodCall=removeUsers',
							params : {
								'id' : ids
							},
							method : 'POST',
...
//		this.pageBar.formValue = this.formValue;
	...
	initComponent : function() {
		...
		this.storeMapping = ['realName', 'userName', 'password', 'email',
				'enabled', 'specialUser','isLock', 'department', 'id'];
		...
		this.store = new Ext.data.JsonStore({
			id : "id",
			url : webContext + '/sysManage/userOuterMangeAction.do?methodCall=queryAllUserByParameter',
			root : "users",
			totalProperty : "rowCount",
			remoteSort : false,
			timeout : 3000000,
