PagePanel = Ext.extend(Ext.Panel, {
	...
	create : function() {
		var clazz = "com.digitalchina.info.framework.security.entity.SameMailDept";//指定实体类型
		...
		var firm  =Ext.Msg.confirm('提示框', '您确定要进行删除操作吗?', function(button) {//删除提示
			if (button == 'yes') {
				...
				Ext.Ajax.request({//用dataAction的方法完成删除
						url : webContext
								+ '/accountSystemAdminAction_deleteSameMailDept.action',
						params : {id:id[i]},
						timeout:100000, 
...
//						
	...
	initComponent : function() {//初始化面板
		...
		this.store = new Ext.data.JsonStore({//设置下方列表的格式
			url : webContext + "/accountSystemAdminAction_listsSameMailDeptAdmin.action",
			fields: ['id','name'],
			root:'data',
		...
		this.grid = new Ext.grid.GridPanel({//设定grid
			...
			columns : [sm, {header : '编号',dataIndex : 'id',align : 'left',sortable : true,hidden : true}, 
						{header : '部门名称',dataIndex : 'name',align : 'left',sortable : true,hidden : false,width:200} 
						], 
