...
<TABLE cellSpacing=1 cellPadding=3 width="100%" bgColor=#6298e1 border=0>
  ...
  <TR>
    <TD noWrap height=24><FONT color=#ffffff><IMG height=18 
      ...
      border=0>&nbsp;<STRONG>服务项发布管理：查询，添加、修改服务项发布</STRONG></FONT></TD></TR>
    <TR>
  ...
  <FORM name="formSearch" action="${pageContext.request.contextPath}/serviceItem_list.action" method="post">
         
          <TD align="center"> 
         服务项名称：<INPUT name="seviceItemName" type="text" value="${param.seviceItemName}">&nbsp;
        系统主表名称：<INPUT name="tableName" type="text" value="${param.tableName}">
         ...
         <INPUT class="button" type="button" onclick="query(1);" value="检索" > 
          ...
          <TD noWrap align=right>
            <A href="javascript:query(1);">全部服务项发布</A>
...
<TABLE cellSpacing=1 cellPadding=0 width="100%" border=0>
 ...
 <TR>
 <TD height=30>系统后台管理&nbsp;-&gt;&nbsp;
  ${not empty requestScope.serviceItemType? requestScope.serviceItemType.name : '全部'}服务项发布
  </TD></TR></TBODY></TABLE>


<TABLE cellSpacing=1 cellPadding=3 width="100%" bgColor=#6298e1 border=0>
  <FORM name="formDel" action="${pageContext.request.contextPath}/infoAdmin/serviceItemUserTableAction.do" method="post">
  ...
  <TR>
    ...
    <STRONG><FONT color=#ffffff>服务项名称</FONT></STRONG>
    </TD>
    <TD noWrap bgColor=#8db5e9 >
    <STRONG><FONT color=#ffffff>服务项类型</FONT></STRONG>
    </TD>
    <TD noWrap  bgColor=#8db5e9>
    <FONT color=#ffffff><STRONG>系统主表</STRONG></FONT>
    </TD>
    <TD noWrap bgColor=#8db5e9>
    <FONT color=#ffffff><STRONG>中文表名</STRONG></FONT>
    </TD>
 ...
 <c:forEach  var="item" items="${requestScope.page.data}" varStatus="status">   
  ...
  <TR onmouseover="this.style.backgroundColor = '#FFFFFF'" style="CURSOR: hand" 
    ...
    <TD noWrap title="${item.serviceItem.name}">
    <A onclick='changeAdminFlag("修改导航栏目")' 
      href="${pageContext.request.contextPath}/infoAdmin/serviceItemUserTableAction.do?methodCall=toForm&id=${item.id}">
      ${fn:substring(item.serviceItem.name, 0, 20)}
      
    </A> 
    </TD>
    <TD noWrap>${item.serviceItemType.name}</TD>
    <TD noWrap>${item.systemMainTable.tableName}</TD>
  ...
  <TR>
    ...
    <INPUT class=button  onclick="createNew();" type=button value=添加新服务项发布 >&nbsp;
