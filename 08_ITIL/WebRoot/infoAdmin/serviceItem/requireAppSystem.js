PagePanel = Ext.extend(Ext.Panel, {
	...
	search : function() {//查询方法
		var tempApp = Ext.getCmp('searchAppCombo').getValue();
		var tempManager = Ext.getCmp('searchManagerCombo').getValue();
		var param = {
					appConfigItem : tempApp,
					appManager : tempManager,
					start: 0
					};
	...
	create : function() {
		var clazz = "com.digitalchina.itil.require.entity.RequireAppSystem";//指定实体类型
...
//		}
	...
	modify:function(){
		...
		var clazz = "com.digitalchina.itil.require.entity.RequireAppSystem";//指定修改实体类型
...
//						
		...
		this.panel = new Ext.form.FormPanel({
		   ...
		   items :[{
					...
					new Ext.form.ComboBox({
						...
						minChars : 50,
						queryDelay : 700,
						store : new Ext.data.JsonStore({//定制store 
							url:webContext+'/SRAction_getAppConfigItemComboData.action?appTypeId=1',
							fields : ['id', 'name'],//指定数据存储格式
							listeners : {
								beforeload : function(store, opt) {//添加beforeload状态
									if (opt.params['searchApp'] == undefined) {//如果没有数据，那么显示默认值
										opt.params['name'] = Ext.getCmp('searchAppCombo').defaultParam;
									}
								}
							},
						...
						pageSize : 10,
						listeners : {
							'beforequery' : function(queryEvent) {//添加beforequery事件，进行查询前的数据处理
					...
					items : [new Ext.form.ComboBox({
						xtype : 'combo',
						hiddenName : 'searchManager',
						id : 'searchManagerCombo',
						width : 200,
						style : '',
						fieldLabel : '管理员',
						colspan : 0,
						...
						queryDelay : 700,
						store : new Ext.data.JsonStore({//定制store 
							url : webContext//指定url
									+ '/extjs/comboDataAction?clazz=com.digitalchina.info.framework.security.entity.UserInfo',
							fields : ['id', 'userName'],//指定数据存储格式
							listeners : {
								beforeload : function(store, opt) {//添加beforeload状态
									if (opt.params['searchManager'] == undefined) {//如果没有数据，那么显示默认值
										opt.params['userName'] = Ext
												.getCmp('searchManagerCombo').defaultParam;
									}
								}
							},
							totalProperty : 'rowCount',
						...
						initComponent : function() {//组建初始化
							this.store.load({
								params : {
									id : Ext //设置管理员ID
											.getCmp('searchManagerCombo')
											.getValue(),
									start : 0
								},
	...
	initComponent : function() {//初始化面板
		...
		this.store = new Ext.data.JsonStore({//设置下方列表的格式
			url : webContext + "/requireAction_listRequireApp.action",
			
			fields: ['id','appConfigItem','appManager','serviceProviderIn','engineer'],
			root:'data',
			totalProperty : 'rowCount',
		...
		this.grid = new Ext.grid.GridPanel({//设定grid
			...
			store : this.store,
			// cm : this.cm,
			columns : [sm, {header : '应用系统编号',dataIndex : 'id',align : 'left',sortable : true,hidden : true}, 
						{header : '应用系统名称',dataIndex : 'appConfigItem',align : 'left',sortable : true,hidden : false}, 
						{header : '应用系统管理员',dataIndex : 'appManager',align : 'left',sortable : true,hidden : false},
						{header : '交付团队',dataIndex : 'serviceProviderIn',align : 'left',sortable : true,hidden : false},
						{header : '交付经理',dataIndex : 'engineer',align : 'left',sortable : true,hidden : false}], 
