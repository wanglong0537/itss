				...
				success:function(response){
					if(response.responseText.trim()!=''){
						...
						if(result.noneNecessaryRel!=undefined){
							Ext.MessageBox.alert("提示","没有可维护的必要关系!",function(){
							});
							return;
						}
					}
...
PageTemplates = Ext.extend(Ext.Panel, {
 ...
 getFormpagePanel_CIBatchModify: function() {
      var da = new DataAction();
		...
		if(data[i].name=="CIBatchModify$modifyNo"){
			data[i].readOnly=true;
		}
		if(data[i].name=="CIBatchModify$applyUser"){
			data[i].readOnly=true;
			data[i].hideTrigger=true;
		};
		if(data[i].name=="CIBatchModify$applyDate"){
			data[i].readOnly=true;
		}
		}
	...
	saveModify:function(){
		...
		Ext.Ajax.request({
						...
						success : function(response, options) {
							...
							Ext.MessageBox.alert("提示","保存变更申请成功!",function(){
								Ext.getCmp("modifySave").enable();
							});
						},
						failure : function(response, options) {
  ...
  items : this.items,
	...
	tabPanel.doLayout();
	tabPanel.activate('HistroyForm');
	this.buttons=[{text :'确认',
			id:'submit',
			handler : function(){
							...
							handler : function() {
								  var submit=function(){
										auditForm.getForm().submit({
											...
											failure : function(response, options) {
												Ext.MessageBox.alert("提示信息：", "提交失败");
											},
											scope : this,
								...
								if(!Ext.getCmp("reSubmit").getValue()){
									submit();
									return;
								}
								Ext.Ajax.request({
										...
										if(result.trim()!=""){
											...
											if(result.error!=undefined){
												...
												new Ext.Window({
													...
													modal:true,
													buttonAlign:'center',
													html :result.confirm+'<font color=red ><p>如果配置项或关系已存在或已被变更,继续提交视为在已存在或已被变更的配置项或关系的基础之上做变更!</p><p>如果原配置项或关系已被删除,继续提交视为新建!</p></font>',
													buttons:[{
														text:'是',
														handler:function(){
															Ext.getCmp('alertWin').close();
