PageTemplates = Ext.extend(Ext.Panel, {
	...
	getFormpanel_MailGroup : function() {
		...
		this.formpanel_MailGroup = new Ext.form.FormPanel({
			...
			})]},
		   {
			...
			{
				html : '<font color=red>*</font> 群组名称:(多个群组申请请使用分号(半角)分隔，最多十个群组，群组管理员将相同 )',
				cls : 'common-text',
				colspan : 4,
				rowspan : 0,
				style : 'border:1px dotted #b0acac;width:600;text-align:left;margin:0px 0px 0px 75px'
			...
			//add by liuying at 20100401 for 修改群组名称不可手写 start
			{
					html : ' <font color=red>群组名:</font>',
					cls : 'common-text',
					style : 'margin:0px 0px 0px 75p'
				}, new Ext.form.TextField({
			...
			new Ext.Button({
					...
					handler : function() {
						...
						if (mangerGroup == "") {
							Ext.getCmp('MailGroup$groupName').setValue(newName);

						} else {
							Ext.getCmp('MailGroup$groupName').setValue(mangerGroup+';'+newName);
 //                         Ext.Msg.alert("提示", "一次申请只能变更一个群组帐号");
			...
			//add by liuying at 20100401 for 修改群组名称不可手写 end
			{
			...
			}), 
			{
		     ...
		     new Ext.Button({
				...
				handler : function() {
					...
					if(mangerGroup==""){
						Ext.getCmp('MailGroup$groupManger').setValue(tmp);
						
					}else{
						
						Ext.getCmp('MailGroup$groupManger').setValue(mangerGroup+";"+tmp);
					}
			...
			buttons : [{
				...
				handler : function() {
					...
					var groupName=Ext.getCmp('MailGroup$groupName').getValue();
					Ext.Ajax.request({
						url : webContext
								+ '/accountAction_saveMailGroupDraft.action',
						params : {
							info : info,
				...
				if(!Ext.getCmp('panel_MailGroup').form.isValid()){
					...
					var info = Ext.encode(getFormParam('panel_MailGroup'));
					var groupName=Ext.getCmp('MailGroup$groupName').getValue();
					var groupManger=Ext.getCmp('MailGroup$groupManger').getValue();
					Ext.Ajax.request({
						url : webContext
								+ '/accountAction_saveMailGroupDraft.action',
						params : {
