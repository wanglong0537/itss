PageTemplates = Ext.extend(Ext.Panel, {
	...
	getFormpanel_SpecailAccountApply_Input : function() {
		...
		this.formpanel_SpecailAccountApply_Input = new Ext.form.FormPanel({
		 ...
		 {
			xtype : 'fieldset',
		    title : '帐号信息(加注<font color=red>*</font>的为必填项)',
			layout : 'table',
		    anchor : '100%',
		    animCollapse:true,
			collapsible :true,
		...
		buttons : [{
				...
				handler : function() {
					...
					Ext.Ajax.request({
						url : webContext
								+ '/accountAction_saveSPARightChangeApplyDraft.action',
						params : {
							info : info,
				...
				handler : function() {
					...
					var accountName=Ext.getCmp('itil_ac_SpecialAccount$accountNameCombo').getValue();
					if(accountName==''){
					Ext.MessageBox.alert("提示","页面中带红色星号的为必填项,请填写完整。谢谢您合作！");	
					}
					var info = Ext.encode(getFormParam('panel_SpecailAccountApply_Input'));
					Ext.Ajax.request({
						url : webContext + '/accountAction_saveSPARightChangeApplyDraft.action',
						params : {
							info : info,
							serviceItemId : curscid,
							processInfoId:processInfoId,
							...
							.initComponent();
				             ...
				             failure : function(response, options) {
		                    	...
		                    	Ext.MessageBox.alert(
									"提示",
									"申请人不存在可变更的临时人员EB帐号,请核实后再提交申请。谢谢您的合作！",
									function(btn) {
											window.history.back(-1);
										})
									}
	...
	initComponent : function() {
		...
		this.model = "ac_TempEBAccountRightChange";
