PageTemplates = Ext.extend(Ext.Panel, {
	...
	getFormpanel_PersonAccountApply_Input : function() {
		...
		this.formpanel_PersonAccountApply_Input = new Ext.form.FormPanel({
			...
			title : Ext.getCmp('PageTemplates').serviceName+"变更申请",
			items : [{
				xtype : 'fieldset',
				title : '申请人信息',
				layout : 'table',
							...
							Ext.Ajax.request({
								...
								params : {
									...
									bzparam : "{dataId :'" + curId
											...
											+ "',accountName:'" + curName
											+ "',applyType: 'acproject',"
											+ "applyTypeName: '"+Ext.getCmp('PageTemplates').serviceName+"权限变更申请',"
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
							 ...
							 if(responseArray.success){	
							 	Ext.MessageBox.alert("提示","申请人已存在审批中的此帐号权限变更申请,不能够再提该申请！",function(btn){
		                      	window.history.back(-1);
		                      	});
							 }else{
			Ext.Ajax.request({
...
//												.setValue("");
						...
						Ext.MessageBox.alert("提示", "申请人不存在可变更的帐号,请确认后在提申请！",
								function(btn) {
									window.history.back(-1);
								});
					}
