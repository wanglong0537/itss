PageTemplates = Ext.extend(Ext.Panel, {
	...
	getFormpanel_PersonAccountApply_Input : function() {
		...
		this.formpanel_PersonAccountApply_Input = new Ext.form.FormPanel({
			...
			title : Ext.getCmp('PageTemplates').serviceName+"重置申请",
			items : [{
				xtype : 'fieldset',
				title : '申请人信息',
				layout : 'table',
				...
				{
					html : '<font color=red>*</font>部门经理审批人:',
					cls : 'common-text',
					style : 'width:135;text-align:right'
				}, new Ext.form.ComboBox({forceSelection:true,
					xtype : 'combo',
				...
				xtype : 'fieldset',
				title : '帐号重置信息(加注<font color=red>*</font>为必填项)',
				layout : 'table',
				anchor : '100%',
				animCollapse : true,
				collapsible : true,
...
//							.getValue();
					...
					Ext.Ajax.request({
						...
						params : {
							...
							accountType : Ext.getCmp('PageTemplates').serviceName,
...
//					}
					...
					Ext.Ajax.request({
						...
						success : function(response, options) {
							...
							Ext.Ajax.request({
								url : webContext
										+ '/accountWorkflow_commonapply.action',
								params : {
									dataId : curId,
									userInfo : userInfo,
									bzparam : "{dataId :'" + curId
											...
											+ "',accountName:'" + curName
											+ "',applyType: 'acproject',"
											+ "applyTypeName: '"+Ext.getCmp('PageTemplates').serviceName+"重置申请',"
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
							 ...
							 if(responseArray.success){	
							 	Ext.MessageBox.alert("提示","申请人已存在审批中的此帐号重置申请,不能够再提该申请！",function(btn){
		                      	window.history.back(-1);
		                      	});
							 }else{
			Ext.Ajax.request({
...
//												.setValue("");
						...
						Ext.MessageBox.alert("提示", "申请人不存在可重置的帐号,请确认后在提申请！",
								function(btn) {
									window.history.back(-1);
								});
					}
