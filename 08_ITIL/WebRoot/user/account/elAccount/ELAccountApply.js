						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
							 ...
							 if(responseArray.success){	
							 	Ext.MessageBox.alert("提示","申请人已存在审批中的EL帐号申请,不能够再提该申请！",function(btn){
			...
			Ext.Ajax.request({
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
									.decode(response.responseText);
						    if(responseArray.success){
						    	Ext.MessageBox.alert("提示","申请人已存在可使用的E-Logistics帐号,请确认后在提申请！",function(btn){
		                      	window.history.back(-1);
		                      	});
		                      }else{
