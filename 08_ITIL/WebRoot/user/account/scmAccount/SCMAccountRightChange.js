PageTemplates = Ext.extend(Ext.Panel, {
	...
	getFormpanel_PersonAccountApply_Input : function() {
		...
		this.formpanel_PersonAccountApply_Input = new Ext.form.FormPanel({
			...
			items:[
			 {
				html : '<font color=red>*</font> SCM服务权限变更原因:',
				cls : 'common-text',
				colspan : 2,
				rowspan : 0,
				style : 'border:1px dotted #b0acac;width:610;text-align:left;margin:0px 0px 0px 75px'
			...
			{
				html : '<font color=red>*</font> 注册服务权限变更描述：为了能准确反馈您需要的权限，准备了一个权限注册描述事例，请按照该方式填写：<br>(例如：我在科技发展公司北京物流中心工作，希望查看物流交货单信息等数据)',
				cls : 'common-text',
				colspan : 2,
				rowspan : 0,
				style : 'border:1px dotted #b0acac;width:610;text-align:left;margin:0px 0px 0px 75px'
			 ...
			 {
				html : ' 如果需要申请的权限与现有的某一个SCM帐号权限一致请填写该帐号:<br> （例如：我需要申请与现有SCM帐号it-manage一致的权限）',
				cls : 'common-text',
				colspan : 2,
				rowspan : 0,
				style : 'border:1px dotted #b0acac;width:610;text-align:left;margin:0px 0px 0px 75px'
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
							 ...
							 if(responseArray.success){	
							 	Ext.MessageBox.alert("提示","申请人已存在审批中的SCM帐号权限变更申请,不能够再提该申请！",function(btn){
		                      	window.history.back(-1);
		                      	});
							 }else{
			Ext.Ajax.request({
						...
						success : function(response, options) {
							...
							Ext.getCmp("itil_ac_PersonFormalAccount$sameRightAccount").setValue("");
				             ...
				             failure : function(response, options) {
							   Ext.MessageBox.alert("提示","申请人不存在可变更的SCM帐号,请确认后在提申请！",function(btn){
		                      	window.history.back(-1);
		                      	});
				              }
		                      })
						...
						failure : function(response, options) {
							Ext.MessageBox.alert("提示","申请人不存在可变更的E-Logistics帐号,请确认后在提申请！",function(btn){
		                      	window.history.back(-1);
		                      	});
						}
					}, this);
