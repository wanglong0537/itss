	...
	getFormpanel_PersonAccountApply_Input : function() {
		...
		this.formpanel_PersonAccountApply_Input = new Ext.form.FormPanel({
			...
			buttons : [{
				...
				id : 'save',
				handler : function() {
					// var rightType=Ext.getCmp('sap').getValue();
					// alert(rightType);
					// return false;
...
//					}
					...
					var rightType = Ext.Ajax.request({
						url : webContext
...
//					}
					...
					var info = Ext.encode(getFormParam('panel_PersonAccountApply_Input'));
			//modify by liuying 20100111 for 验证草稿状态提交时和正常提交时是否存在相同申请和是否存在远程接入账号 start
			Ext.Ajax.request({
						url : webContext
								+ '/accountAction_getUserApply.action',
						params : {
							serviceItemProcess:'191',
							processType:'1'
						} ,
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
							 ...
							 if(responseArray.success){	
							 	Ext.MessageBox.alert("提示","申请人已存在审批中的远程接入帐号权限变更申请,不能够再提该申请！",function(btn){
								...
								Ext.Ajax.request({
									...
									success : function(response, options) {
										...
										Ext.Ajax.request({
											...
											Ext.MessageBox.alert("提示",
													"申请人不存在可变更的远程接入帐号,请确认后在提申请！", function(btn) {
														window.history.back(-1);
													});
										}
									...
									failure : function(response, options) {
										Ext.MessageBox.alert("提示", "申请人不存在可变更的远程接入帐号,请确认后在提申请！",
												function(btn) {
													window.history.back(-1);
												});
									}
	...
	//modify by liuying 20100111 for 验证草稿状态提交时和正常提交时是否存在相同申请和是否存在远程接入账号 end
				}
			}, {
				text : '返回',
				iconCls : 'back',
...
//												.initComponent();
			...
			this.formpanel_PersonAccountApply_Input.load({
				url : webContext
						+ '/accountAction_getVPNRightChangeDraftData.action?panelName=panel_PersonAccountApply_Input&dataId='
						+ this.dataId,
				timeout : 30,
				success : function(action, form) {
