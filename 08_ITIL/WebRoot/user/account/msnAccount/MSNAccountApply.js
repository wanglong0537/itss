PageTemplates = Ext.extend(Ext.Panel, {
	...
	getTabpanel : function(tab, tabTitle) {
		this.tabPanel = new Ext.TabPanel({
			...
			items : tab,
			tbar : [new Ext.Toolbar.TextItem('<font color=red>提示：</font><font color=blue>1.页面中带红色<font color=red>*</font>号的必填项，请在填写完整后再提交申请！<font color=red>2.管理员审批后30分钟左右生效!</font>&nbsp3.审批人必须从下拉列表中选择.</font>')]
	...
	getFormpanel_PersonAccountApply_Input : function() {
		...
		this.formpanel_PersonAccountApply_Input = new Ext.form.FormPanel({
								...
								success : function(response, options) {
									...
									if (!r.success) {
										...
										Ext.MessageBox
												.confirm(
														"提示",
														"申请人还没有WWW帐号,请先提交WWW帐号申请,是否创建该申请？",
														function(button, text) {
															if (button == 'yes') {
																window.location = webContext
																		+ "/requireAction_toProcessEnterPage.action?serviceItemProcessId=156";
															} else {
																var userInfo = Ext
							...
							.initComponent();
								...
								var userInfo = Ext
									   .getCmp('AccountApplyMainTable$applyUserCombo').setValue("");
										...
										Ext.MessageBox.alert("提示","申请人已经存在可用的MSN帐号,不能够在申请！");
										var userInfo = Ext
									.getCmp('AccountApplyMainTable$applyUserCombo').setValue("");
										return false;
			...
			}), 
           {
			...
			items:[
			
			 {
				...
				html : '<font color=red>*</font>对方联系单位:',
				cls : 'common-text',
				style : 'width:135;text-align:right'
			}, new Ext.form.TextArea({
				fieldLabel : '对方联系单位',
				...
				colspan : 0,
				rowspan : 0,
				id : 'itil_ac_PersonFormalAccount$otherLinkCompany',
				name : 'itil_ac_PersonFormalAccount$otherLinkCompany',
				style : '',
				width : 530,
		   ...
		   defaults : {
			...
			items:[ {
				html : '1、MSN帐号用于神州数码内部IT用户与外部供应商/厂商、代理商、客户等业务合作伙伴进行协同办公。<br>2、根据公司IT资源配置原则，MSN的使用实行月租制，每帐号月租成本20元，每月分摊至MSN帐号管理者所在部门。<br>3、禁止使用MSN发送附件，尤其是大附件。<br>4、禁止共用、借用、盗用MSN帐号。<br>5、管理员审批后30分钟左右生效!',
				cls : 'common-text',
			...
			buttons : [{
				...
				Ext.Ajax.request({
						...
						success : function(response, options) {
							var responseArray = Ext.util.JSON
							 ...
							 if(responseArray.success){	
							 	Ext.MessageBox.alert("提示","申请人已存在审批中的MSN帐号申请,不能够再提该申请！",function(btn){
					...
					Ext.Ajax.request({
						...
						failure : function(response, options) {
								Ext.MessageBox.alert("提示", "提交申请失败,请检查审批人或申请人是否选择正确!");
							Ext.getCmp("save").enable();
					                Ext.getCmp("submit").enable();
					                Ext.getCmp("back").enable();
						}
			...
			Ext.Ajax.request({
						...
						failure : function(response, options) {
							...
							Ext.MessageBox.confirm(
														"提示",
														"使用人还没有WWW帐号,请先提交WWW帐号申请,是否创建该申请？",
														function(button, text) {
															if (button == 'yes') {
																window.location = webContext
																		+ "/requireAction_toProcessEnterPage.action?serviceItemProcessId=156";
