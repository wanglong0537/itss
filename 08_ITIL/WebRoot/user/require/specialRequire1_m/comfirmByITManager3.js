...
//add by musicbear for 暂存按钮 in 2009 11 04 end	
	saveAndSubmit : function() {
		...
		Ext.Ajax.request({
			...
			success : function(response, options) {
				...
				this.spid = responseArray.contractId;
				Ext.getCmp('SRprojectContracts$id').setValue(this.spid);
				window.parent.auditContentWin.specialAudit();//modify by musicbear for 审批意见对话框去掉拒绝选项 in 2009 11 20
			},
			failure : function(response, options) {
				Ext.MessageBox.alert("保存失败");
			}
	...
	getFormpanel_SpecialRequire3_m_Input : function() {
		...
		if (this.dataId != '0') {
			data = da.getPanelElementsForEdit("nsr1_m_comfirmByITManager2",
...
//	},
	getFormpanel_SRprojectContracts : function() {
		...
		//add by lee for 将暂存按钮放入合同面板 in 20091112 begin
	...
	initComponent : function() {
		...
		this.model = "nsr1_m_comfirmByITManager2";
		var buttonUtil = new ButtonUtil();
		this.mybuttons = buttonUtil.getButtonForModel(
				"nsr1_m_comfirmByITManager2", this);
		if (this.mybuttons != "") {
			this.buttons = {
				layout : 'table',
				height : 'auto',
