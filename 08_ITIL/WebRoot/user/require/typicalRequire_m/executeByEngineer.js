PageTemplates = Ext.extend(Ext.Panel, {
	...
	getFormpanel_typicalRequire_m_Input : function() {
		...
		if (this.dataId != '0') {
			data = da.getPanelElementsForEdit("tr_m_executeByEngineer","panel_typicalRequire_m_Input", this.dataId);// 这是要随时变得
	...
	getFormpanel_SRAnalyse : function() {
		...
		this.getFormButtons = buttonUtil.getButtonForPanel("panel_SRAnalyse",
				this);
		if (this.dataId != '0') {
			data = da.getPanelElementsForEdit("tr_m_executeByEngineer","panel_SRAnalyse", this.dataId);// 这是要随时变得
			biddata = da.splitForReadOnly(data);
		} else {
	...
	initComponent : function() {
		...
		this.model = "tr_m_executeByEngineer";
		var buttonUtil = new ButtonUtil();
		this.mybuttons = buttonUtil.getButtonForModel("tr_m_executeByEngineer",
				this);
		if (this.mybuttons != "") {
			this.buttons = {
				layout : 'table',
