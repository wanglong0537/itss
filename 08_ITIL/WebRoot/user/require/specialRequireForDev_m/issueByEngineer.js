...
//remove by zhangzy for‘用户要求去掉动态指派传输工程师操作，改为后台预指派（也就是在后台工作流配置角色） end
	...
	getFormpanel_SpecialRequireDevConfirm_m_Input : function() {
		...
		var biddata = "";
		if (this.dataId != '0') {
			data = da.getPanelElementsForEdit("devsr_m_issueByEngineer",
	...
	initComponent : function() {
		...
		var items = new Array();
		var temp = new Array();
		this.model = "devsr_m_issueByEngineer";
		var buttonUtil = new ButtonUtil();
		this.mybuttons = buttonUtil.getButtonForModel("devsr_m_issueByEngineer",this);
