...
//	title :this.description,
	...
	save : function() {
		...
		if(!Ext.getCmp('panel_swrequire_input').form.isValid()){
		...
		var formParam = Ext.encode(getFormParam('panel_swrequire_input',false));
		Ext.Ajax.request({
			url : webContext + '/requireAction_saveApplyDraft.action',
			params : {
				pagePanel　:　'panel_swrequire_input',
				info : formParam,
      ...
      var da = new DataAction();
		  var data = null;
			...
			var biddata = "";
			  ...
			  this.getFormButtons = buttonUtil.getButtonForPanel("panel_swrequire_input",this);
			if (this.dataId == '0') {
				data = da.getRequireFormPanelElementsForEdit("panel_swrequire_input","");
			}else{
				data = da.getRequireFormPanelElementsForEdit("panel_swrequire_input",this.dataId);
			}
...
//modify by zhangzy for “我审批的” 无法打开 in 2009 11 21 start		 
		if (this.status != 0) {
			...
			items : [this.formpanel_swrequire_input, histroyForm]
		});
			this.items = [this.tab];
			
		} else {
			this.items = [this.formpanel_swrequire_input];
		}
