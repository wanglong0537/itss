PageTemplates = Ext.extend(Ext.Panel, {
	...
	getFormpanel_SpecialRequire4_Input : function() {
		...
		if (this.dataId != '0') {
			data = da.getPanelElementsForEdit("sr2_executeByEngineer",
...
//add by lee for add attachmentfile in 20090806 end
	...
	//			if (this.dataId != '0') {
	//				data = da.getPanelElementsForEdit("sr2_executeByEngineer", "panel_SRProjectPlan", this.dataId);// 这是要随时变得
	//				biddata = da.split(data);
	//			} else {
			...
			{fieldLabel:'附件',
				...
				Ext.Ajax.request({
					...
					success : function(response, options) {
						...
						var ud = new UpDownLoadFile();
						ud.getUpDownLoadFileSu(attachmentFlag, '8016',
							'com.digitalchina.info.appframework.metadata.entity.SystemFile','DeptMailattachment');
					},
					failure : function(response, options) {
						Ext.MessageBox.alert("获取附件失败");
					}
	...
	getTestPanel : function(panel1, panel2) {
	...
	initComponent : function() {
		...
		this.model = "sr2_executeByEngineer";
		var buttonUtil = new ButtonUtil();
		this.mybuttons = new Ext.Button({
			text : '提交测试',//btnName,	//modify by lee for 修改为默认名称，mantis号IT项目0000821 in 20090830
			pressed : true,
			iconCls : 'submit',
			handler : function() {
