...
//			String subject="IT温馨提示:"+event.getCreateUser().getRealName()+"/"+event.getCreateUser().getUserName()+"创建的编号为"+event.getEventCisn()+",名称为："+event.getSummary()+"的事件在提交时发生了异常，请查看并重新提交。";//"异常通知";
			...
			String url = PropertiesUtil.getProperties("system.web.url","itss.digitalchina.com") + "/user/event/eventResubmit/eventResubmitForException.jsp?dataId="+dataId;
			SendExceptionMailThred smthread =new SendExceptionMailThred(ms,event.getCreateUser().getEmail()+";"+"liming1@digitalchina.com", "", null, subject, this.eventHtmlContent(event.getCreateUser(), url, event), null);
