SpAuthorizePbcForm = Ext.extend(Ext.Window, {
	constructor : function(a) {
		...
		Ext.apply(this, a);
		this.initComponents();
		SpAuthorizePbcForm.superclass.constructor.call(this, {
			id : "SpAuthorizePbcFormWin",
			title : "授权操作界面",
			width : 830,
			height : 500,
			modal : true,
	...
	initComponents : function() {
		...
		this.store = new Ext.data.JsonStore({
			url : __ctxPath + "/shop/authorListSpPaKpiitem2user.do?pbcId=" + this.pbcId,
			id : "id",
			root : "result",
			remoteSort : true,
			fields : [
	...
	authorize : function(a) {
		var e = Ext.getCmp("SpAuthorizePbcFormGrid");
		...
		for(var i = 0; i < c.length; i++) {
			authorItems += c[i].data.id + " ";
		}
		Ext.Ajax.request({
			url : __ctxPath + "/shop/saveSpPaAuthorizepbc.do",
			params : {
